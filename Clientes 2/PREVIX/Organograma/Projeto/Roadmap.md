...
- ⚪ **STORY-019 — Defesa total contra ciclos no `secondary_manager_id`.** Em backlog. Após 2 incidentes em 2026-05-08 (Katia com `secondary == primary`, Adriano com ciclo transitivo via Ana), 4 camadas de proteção foram aplicadas (UI/schema/CHECK/render). Esta story cobre os caminhos não-cobertos: TRIGGER no DB com CTE recursiva pra detectar ciclos transitivos no INSERT/UPDATE, e Edge Function `validate-and-set-secondary-manager` espelhando `validate-and-set-manager`. JG decidiu deixar em backlog — risco residual baixo pro fluxo da Previx.
