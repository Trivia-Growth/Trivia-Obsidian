# [P0] Missing TRIVIAIOX Core module: utils/repository-detector - Blocks Linux Installation
...
The TRIVIAIOX installer fails immediately on Linux systems with a module not found error, preventing any installation or testing. The installer cannot locate the TRIVIAIOX Core module `utils/repository-detector`, blocking all Linux platform validation for Story 1.10c.
...
```
Error: Cannot find TRIVIAIOX Core module: utils/repository-detector
Searched: ${PROJECT_ROOT}/.triviaiox-core/utils/repository-detector
Please ensure triviaiox-core is installed correctly.
    at loadTRIVIAIOXCore (${PROJECT_ROOT}/bin/triviaiox-init.js:43:11)
```

### Stack Trace Location

- **File:** `bin/triviaiox-init.js`
- **Line:** 43
- **Function:** `loadTRIVIAIOXCore`
...
- Node.js v18.20.8+
- TRIVIAIOX-Fullstack repository cloned
...
2. Create test directory:
   ...
   ```bash
   mkdir -p /tmp/triviaiox-test-install
   cd /tmp/triviaiox-test-install
   ```

3. Execute installer:

   ```bash
   node ${PROJECT_ROOT}/bin/triviaiox-init.js --help
   ```
...
- Installer should locate TRIVIAIOX Core module successfully
...
2. **Missing File/Directory**
   - `.triviaiox-core/utils/repository-detector.js` may not exist in repository
...
- [ ] Verify `.triviaiox-core/utils/repository-detector.js` exists in repo
- [ ] Check `bin/triviaiox-init.js:43` for path construction logic
...
1. **Verify module exists:**
   ...
   ```bash
   ls -la .triviaiox-core/utils/repository-detector.js
   ```

2. **Fix path resolution in `bin/triviaiox-init.js:43`:**

   ```javascript
   // Before (suspected):
   const detector = require('./.triviaiox-core/utils/repository-detector');

   // After (cross-platform):
   const path = require('path');
   const detectorPath = path.resolve(__dirname, '..', '.triviaiox-core', 'utils', 'repository-detector');
...
3. **Add file existence check:**
   ...
   if (!fs.existsSync(detectorPath + '.js')) {
     console.error('TRIVIAIOX Core module not found.');
...
4. **Test in WSL Ubuntu:**
   ```bash
   wsl bash -c "cd /tmp/triviaiox-test-install && node /mnt/c/.../bin/triviaiox-init.js --help"
   ```
...
- **Installer:** `bin/triviaiox-init.js:43` (error location)
- **Missing Module:** `.triviaiox-core/utils/repository-detector` (expected location)
