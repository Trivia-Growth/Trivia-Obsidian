...
/**
 * TRIVIAIOX-FullStack Minimal Installation
 *
...
console.log('\n⚠️  DEPRECATION WARNING: triviaiox-minimal is deprecated.');
console.log('   The --minimal mode (squads) was replaced by Squads.');
console.log('   Running standard installation wizard instead.\n');

// Get the path to the main router (triviaiox.js)
const routerPath = path.join(__dirname, 'triviaiox.js');
...
child.on('error', (error) => {
  console.error('❌ Failed to start TRIVIAIOX:', error.message);
  process.exit(1);
});
