...
/**
 * TRIVIAIOX IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   triviaiox ids:query {intent}     — Query registry for matching artifacts
 *   triviaiox ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   triviaiox ids:health             — Run registry health check (self-healing)
 *   triviaiox ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   triviaiox ids:impact {entity-id} — Impact analysis for modifications
 *   triviaiox ids:stats              — Registry statistics
 *   triviaiox ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.triviaiox-core', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.triviaiox-core', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.triviaiox-core', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.triviaiox-core', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.triviaiox-core', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
TRIVIAIOX IDS — Incremental Development System
...
Examples:
  triviaiox ids:query "validate story drafts"
  triviaiox ids:query "template rendering engine" --json
  triviaiox ids:query "database migration" --type script
  triviaiox ids:create-review
  triviaiox ids:create-review --json
  triviaiox ids:health
  triviaiox ids:health --fix
  triviaiox ids:check "validate yaml schema" --type task
  triviaiox ids:impact create-doc
  triviaiox ids:stats
  triviaiox ids:stats --json
  triviaiox ids:register .triviaiox-core/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: triviaiox ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: triviaiox ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: triviaiox ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: triviaiox ids:register {file-path}');
    process.exit(1);
  }
