...
async function runWizard(options = {}) {
  ...
  if (!fs.existsSync(wizardPath)) {
    ...
    const legacyScript = path.join(__dirname, 'triviaiox-init.js');
    if (fs.existsSync(legacyScript)) {
      if (!options.quiet) {
        console.log('⚠️  Using legacy wizard (src/wizard not found)');
      }
      // Legacy wizard doesn't support options, pass via env vars
      process.env.TRIVIAIOX_INSTALL_FORCE = options.force ? '1' : '';
      process.env.TRIVIAIOX_INSTALL_QUIET = options.quiet ? '1' : '';
      process.env.TRIVIAIOX_INSTALL_DRY_RUN = options.dryRun ? '1' : '';
      require(legacyScript);
      return;
    }
    console.error('❌ Initialization wizard not found');
    console.error('Please ensure TRIVIAIOX-FullStack is installed correctly.');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
TRIVIAIOX-FullStack v${packageJson.version}
AI-Orchestrated System for Full Stack Development

USAGE:
  npx triviaiox-core@latest              # Run installation wizard
  npx triviaiox-core@latest install      # Install in current project
  npx triviaiox-core@latest init <name>  # Create new project
  npx triviaiox-core@latest update       # Update to latest version
  npx triviaiox-core@latest validate     # Validate installation integrity
  npx triviaiox-core@latest info         # Show system info
  npx triviaiox-core@latest doctor       # Run diagnostics
  npx triviaiox-core@latest --version    # Show version
  npx triviaiox-core@latest --version -d # Show detailed version info
  npx triviaiox-core@latest --help       # Show this help

UPDATE:
  triviaiox update                    # Update to latest version
  triviaiox update --check            # Check for updates without applying
  triviaiox update --dry-run          # Preview what would be updated
  triviaiox update --force            # Force update even if up-to-date
  triviaiox update --verbose          # Show detailed output

VALIDATION:
  triviaiox validate                    # Validate installation integrity
  triviaiox validate --repair           # Repair missing/corrupted files
  triviaiox validate --repair --dry-run # Preview repairs
  triviaiox validate --detailed         # Show detailed file list

CONFIGURATION:
  triviaiox config show                       # Show resolved configuration
  triviaiox config show --debug               # Show with source annotations
  triviaiox config diff --levels L1,L2        # Compare config levels
  triviaiox config migrate                    # Migrate monolithic to layered
  triviaiox config validate                   # Validate config files
  triviaiox config init-local                 # Create local-config.yaml

SERVICE DISCOVERY:
  triviaiox workers search <query>            # Search for workers
  triviaiox workers search "json" --category=data
  triviaiox workers search "transform" --tags=etl,data
  triviaiox workers search "api" --format=json

EXAMPLES:
  # Install in current directory
  npx triviaiox-core@latest

  # Install with minimal mode (only expansion-creator)
  npx triviaiox-core-minimal@latest
...
For more information, visit: https://github.com/Trivia-Growth/Triviaiox
`);
}
...
async function showVersion() {
  ...
  console.log(`TRIVIAIOX-FullStack v${packageJson.version}`);
  console.log('Package: triviaiox-core');

  // Check for local installation
  const localVersionPath = path.join(process.cwd(), '.triviaiox-core', 'version.json');
  ...
  if (fs.existsSync(localVersionPath)) {
    try {
      ...
      if (versionInfo.version !== packageJson.version) {
        ...
        console.log('  Run \'npx triviaiox-core update\' to update.');
      } else {
        console.log('\n✅ Up to date');
      }
    } catch (error) {
    ...
    console.log('  Run \'npx triviaiox-core install\' to install TRIVIAIOX in this project.');
  }
}

// Helper: Show system info
function showInfo() {
  console.log('📊 TRIVIAIOX-FullStack System Information\n');
  ...
  const triviaioxCoreDir = path.join(__dirname, '..', '.triviaiox-core');
  if (fs.existsSync(triviaioxCoreDir)) {
    ...
    const devDir = path.join(triviaioxCoreDir, 'development');
    const componentBase = fs.existsSync(devDir) ? devDir : triviaioxCoreDir;
    ...
    console.log('\n⚠️  TRIVIAIOX Core not found');
  }

  // Check TRIVIAIOX Pro status (Task 5.1)
  const proDir = path.join(__dirname, '..', 'pro');
  if (fs.existsSync(proDir)) {
    console.log('\n✓ TRIVIAIOX Pro installed');
...
async function runValidate() {
  ...
  try {
    ...
    const { createValidateCommand } = require('../.triviaiox-core/cli/commands/validate/index.js');
    ...
    await validateCmd.parseAsync(['node', 'triviaiox', ...validateArgs]);
  } catch (_error) {
...
async function runUpdate() {
  ...
  try {
    ...
    const { TRIVIAIOXUpdater, formatCheckResult, formatUpdateResult } = require(updaterPath);

    const updater = new TRIVIAIOXUpdater(process.cwd(), {
      verbose: isVerbose,
      force: isForce,
    });
...
async function runDoctor(options = {}) {
  const { runDoctorChecks } = require(path.join(__dirname, '..', '.triviaiox-core', 'core', 'doctor'));
...
// Helper: Remove TRIVIAIOX sections from .gitignore
function cleanGitignore(gitignorePath) {
  ...
  const newLines = [];
  let inTriviaioxSection = false;
  let removedLines = 0;

  for (const line of lines) {
    if (line.includes('# TRIVIAIOX') || line.includes('# Added by TRIVIAIOX')) {
      inTriviaioxSection = true;
      removedLines++;
      continue;
    }
    if (inTriviaioxSection && line.trim() === '') {
      inTriviaioxSection = false;
      continue;
    }
    if (inTriviaioxSection) {
      removedLines++;
      continue;
    }
    newLines.push(line);
...
function showUninstallHelp() {
  console.log(`
Usage: npx triviaiox-core uninstall [options]

Remove TRIVIAIOX from the current project.

Options:
  --force      Skip confirmation prompt
  --keep-data  Keep .triviaiox/ directory (settings and history)
  --dry-run    Show what would be removed without removing
  -h, --help   Show this help message

What gets removed:
  - .triviaiox-core/     Framework core files
  - docs/stories/   Story files (if created by TRIVIAIOX)
  - squads/         Squad definitions
  - .gitignore      TRIVIAIOX-added entries only

What is preserved (with --keep-data):
  - .triviaiox/          Project settings and agent history
...
Examples:
  ...
  npx triviaiox-core uninstall --force

  # See what would be removed
  npx triviaiox-core uninstall --dry-run
...
function showDoctorHelp() {
  console.log(`
Usage: npx triviaiox-core doctor [options]

Run health checks on your TRIVIAIOX installation.
...
Checks performed:
  • Required directories exist (.triviaiox-core/, .triviaiox/)
...
Examples:
  ...
  npx triviaiox-core doctor --fix

  # Preview what would be fixed
  npx triviaiox-core doctor --fix --dry-run
`);
}

// Uninstall TRIVIAIOX from project
async function runUninstall(options = {}) {
  ...
  // Items to remove
  const itemsToRemove = [
    { path: '.triviaiox-core', description: 'Framework core' },
    { path: 'squads', description: 'Squad definitions' },
  ];

  // Optionally remove .triviaiox
  if (!keepData) {
    itemsToRemove.push({ path: '.triviaiox', description: 'Project data and settings' });
  }
  ...
  if (existingItems.length === 0) {
    console.log('ℹ️  No TRIVIAIOX installation found in this directory.');
    return;
  }
  ...
  if (!quiet) {
    ...
    if (fs.existsSync(gitignorePath)) {
      const content = fs.readFileSync(gitignorePath, 'utf8');
      if (content.includes('# TRIVIAIOX') || content.includes('# Added by TRIVIAIOX')) {
        console.log('  • .gitignore TRIVIAIOX entries will be cleaned');
      }
    }
    console.log('');
  }
  ...
  if (!force) {
    ...
    const answer = await new Promise(resolve => {
      rl.question('⚠️  Are you sure you want to uninstall TRIVIAIOX? (y/N): ', resolve);
    });
    rl.close();
  ...
  // Perform removal
  if (!quiet) console.log('\n🗑️  Removing TRIVIAIOX components...\n');
  ...
  if (gitignoreResult.removed && !quiet) {
    console.log(`  ✓ Cleaned ${gitignoreResult.lines} TRIVIAIOX entries from .gitignore`);
  }

  // Summary
  if (!quiet) {
    console.log('\n✅ TRIVIAIOX has been uninstalled.');
    if (keepData) {
      console.log('   Your project data in .triviaiox/ has been preserved.');
    }
    console.log('\n   To reinstall: npx triviaiox-core install');
  }
}
...
function showInstallHelp() {
  console.log(`
Usage: npx triviaiox-core install [options]

Install TRIVIAIOX in the current directory.

Options:
  --force      Overwrite existing TRIVIAIOX installation
...
Smart Merge (Brownfield):
  ...
  TRIVIAIOX can merge new settings while preserving your customizations.

  - .env files: Adds new variables, preserves existing values
  - CLAUDE.md: Updates TRIVIAIOX sections, keeps your custom rules
...
Examples:
  ...
  npx triviaiox-core install --force

  # Brownfield: merge configs automatically
  npx triviaiox-core install --merge
...
function showInitHelp() {
  console.log(`
Usage: npx triviaiox-core init <project-name> [options]

Create a new TRIVIAIOX project with the specified name.
...
Examples:
  ...
  npx triviaiox-core init my-project --template minimal
  npx triviaiox-core init my-project --force --skip-install
  npx triviaiox-core init . --template enterprise
`);
}
...
async function initProject() {
  ...
  console.log(`Creating new TRIVIAIOX project: ${displayName}`);
  if (template !== 'default') {
...
async function main() {
  switch (command) {
    case 'workers':
      ...
      try {
        const { run } = require('../.triviaiox-core/cli/index.js');
    ...
    case 'pro':
      // TRIVIAIOX Pro License Management - Story PRO-6
      try {
        const { run } = require('../.triviaiox-core/cli/index.js');
        await run(process.argv);
      } catch (error) {
