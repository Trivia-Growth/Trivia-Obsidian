...
/**
 * TRIVIAIOX-FullStack Installation Wizard v5 (LEGACY)
 ...
 *
 * Migration path:
 * - Use `npx triviaiox-core` which routes through bin/triviaiox.js to the new wizard
...
/**
 * Smart path resolution for TRIVIAIOX Core modules
 */
function resolveTriviaioxCoreModule(modulePath) {
  const triviaioxCoreModule = path.join(__dirname, '..', '.triviaiox-core', modulePath);

  const moduleExists =
    fs.existsSync(triviaioxCoreModule + '.js') ||
    fs.existsSync(triviaioxCoreModule + '/index.js') ||
    fs.existsSync(triviaioxCoreModule);

  if (!moduleExists) {
    throw new Error(
      `Cannot find TRIVIAIOX Core module: ${modulePath}\n` +
        `Searched: ${triviaioxCoreModule}\n` +
        'Please ensure triviaiox-core is installed correctly.'
    );
  }

  return require(triviaioxCoreModule);
}

// Load TRIVIAIOX Core modules
const { detectRepositoryContext } = resolveTriviaioxCoreModule(
  'infrastructure/scripts/repository-detector'
);
// PM adapters imported but not used directly (loaded dynamically)
// const { ClickUpAdapter } = resolveTriviaioxCoreModule('utils/pm-adapters/clickup-adapter');
// const { GitHubProjectsAdapter } = resolveTriviaioxCoreModule('utils/pm-adapters/github-adapter');
// const { JiraAdapter } = resolveTriviaioxCoreModule('utils/pm-adapters/jira-adapter');
...
async function main() {
  ...
  if (!context) {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      ...
      const defaultPackage = {
        ...
        version: '1.0.0',
        description: 'TRIVIAIOX-FullStack project',
        main: 'index.js',
  ...
  const installedManifestPath = path.join(projectRoot, '.triviaiox-core', '.installed-manifest.yaml');
  const hasExistingInstall = fs.existsSync(installedManifestPath);

  if (hasExistingInstall && brownfieldUpgrader) {
    console.log(chalk.yellow('🔄 Existing TRIVIAIOX installation detected!'));
    console.log('');

    const sourceDir = path.join(context.frameworkLocation, '.triviaiox-core');
      ...
      if (upgradeChoice === 'upgrade') {
        ...
        if (result.success) {
          ...
          brownfieldUpgrader.updateInstalledManifest(
            projectRoot,
            sourceManifest,
            `triviaiox-core@${packageJson.version}`
          );
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using TRIVIAIOX-FullStack?'),
      choices: [
        {
          name: '  Using TRIVIAIOX in a project ' + chalk.gray('(Framework files added to .gitignore)'),
          value: 'project-development',
        },
        {
          name:
            '  Developing TRIVIAIOX framework itself ' + chalk.gray('(Framework files are source code)'),
  ...
  const configPath = path.join(context.projectRoot, '.triviaiox-installation-config.yaml');
  // INS-2 Performance: Use async write instead of sync
  ...
  // Step 4b: Copy TRIVIAIOX Core files
  console.log('');

  const sourceCoreDir = path.join(context.frameworkLocation, '.triviaiox-core');
  const targetCoreDir = path.join(context.projectRoot, '.triviaiox-core');

  if (fs.existsSync(sourceCoreDir)) {
    // INS-2 Performance: Add spinner for file copy (AC9)
    const copySpinner = ora('Installing TRIVIAIOX Core files...').start();
    await fse.copy(sourceCoreDir, targetCoreDir);
    copySpinner.succeed(
      'TRIVIAIOX Core files installed ' +
    ...
    console.error(chalk.red('✗') + ' TRIVIAIOX Core files not found');
    process.exit(1);
  }
  ...
  if (!ides.includes('none')) {
    ...
    const ideRulesMap = {
      ...
      'github-copilot': { source: 'copilot-rules.md', target: '.github/chatmodes/triviaiox-agent.md' },
    ...
    // Step 2: Install TRIVIAIOX CORE agents and tasks for Claude Code
    ...
    if (ides.includes('claude')) {
      ...
      // Create TRIVIAIOX README for Claude Code
      const triviaioxsReadme = path.join(
        context.projectRoot,
        '.claude',
        'commands',
        'TRIVIAIOX',
      ...
      await fse.ensureDir(path.dirname(triviaioxsReadme));
      await fse.writeFile(
        triviaioxsReadme,
        `# TRIVIAIOX Core Commands

This directory contains the core TRIVIAIOX-FullStack agents and tasks.
...
## Documentation
See .triviaiox-core/user-guide.md for complete documentation.
`
      );
    ...
    // Step 3: Install TRIVIAIOX CORE agents for Cursor
    ...
    if (ides.includes('cursor')) {
      ...
      // Create TRIVIAIOX README for Cursor
      const cursorReadme = path.join(context.projectRoot, '.cursor', 'rules', 'TRIVIAIOX', 'README.md');
      await fse.ensureDir(path.dirname(cursorReadme));
      await fse.writeFile(
        cursorReadme,
        `# TRIVIAIOX Core Rules

This directory contains the core TRIVIAIOX-FullStack agent rules for Cursor.
...
`
    ...
    // Step 4: Install TRIVIAIOX CORE agents for other IDEs (Gemini, AntiGravity)
    // v4: Agents are in development/ module
    // INS-2 Performance: Uses cached agent files list
    ...
    if (ides.includes('github-copilot')) {
      ...
      if (cachedAgentFiles.length > 0) {
        ...
        for (const agentFile of cachedAgentFiles) {
          ...
          const targetPath = path.join(copilotModesDir, `triviaiox-${agentName}.md`);
          await fse.copy(sourcePath, targetPath);
        }
  ...
  const possibleSquadsDirs = [
    ...
    path.join(context.projectRoot, 'node_modules', 'triviaiox-core', 'squads'),
    path.join(context.projectRoot, 'node_modules', '@triviaiox', 'fullstack', 'squads'),
  ];
  ...
  try {
    ...
    const validator = new PostInstallValidator(context.projectRoot, context.frameworkLocation, {
      ...
      // For production integrity checks, users should run `triviaiox validate` which
    ...
    if (
      ...
      console.log(
        chalk.yellow('   Run ') +
          chalk.cyan('triviaiox validate --repair') +
    ...
    if (process.env.DEBUG || process.env.TRIVIAIOX_DEBUG) {
      console.log(chalk.dim(`   Stack: ${validationError.stack}`));
    }
    console.log(chalk.dim('   Run `triviaiox validate` to check installation integrity'));
  }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ TRIVIAIOX-FullStack installation complete! 🎉'));
  console.log('');
  ...
  console.log('  ' + chalk.dim('.triviaiox-core/') + '           - Framework core files');

  if (ides.includes('claude')) {
    console.log('  ' + chalk.dim('.claude/'));
    console.log('    ' + chalk.dim('├─ CLAUDE.md') + '        - Main configuration');
    console.log('    ' + chalk.dim('└─ commands/'));
    console.log('      ' + chalk.dim('  ├─ TRIVIAIOX/') + '         - Core agents & tasks');
  ...
  if (ides.includes('cursor')) {
    ...
    console.log('      ' + chalk.dim('  ├─ TRIVIAIOX/') + '         - Core agent rules');
    if (selectedSquads && selectedSquads.length > 0) {
      selectedSquads.forEach((squad) => {
  ...
  if (ides.includes('claude')) {
    ...
    console.log('    • Browse: .claude/commands/TRIVIAIOX/agents/ for all available agents');
  }
  ...
  if (ides.includes('antigravity')) {
    ...
    console.log('    • Browse: .antigravity/rules/TRIVIAIOX/agents/ for all available agents');
  }

  console.log('  ' + chalk.yellow('General:'));
  console.log('    • Run ' + chalk.yellow('triviaiox validate') + ' to verify installation integrity');
  console.log('    • Run ' + chalk.yellow('triviaiox validate --repair') + ' to fix any missing files');
  console.log('    • Check .triviaiox-core/user-guide.md for complete documentation');
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# TRIVIAIOX-FullStack Framework Files (auto-managed - do not edit)',
      '.triviaiox-core/',
      'node_modules/@triviaiox/',
      'outputs/minds/',
      '.triviaiox-installation-config.yaml',
      '# End TRIVIAIOX-FullStack auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# TRIVIAIOX-FullStack Framework Files');
...
async function savePMConfig(pmTool, config, projectRoot) {
  ...
  const configPath = path.join(projectRoot, '.triviaiox-pm-config.yaml');
  // INS-2 Performance: Use async write
 ...
 * @param {string} sourceCoreDir - Path to installed .triviaiox-core directory
 */
