/**
 * TRIVIAIOX-FullStack Environment Configuration Module
...
async function generateCoreConfigYAML(projectPath, wizardState) {
  try {
    const triviaioxCoreDir = path.join(projectPath, '.triviaiox-core');
    const yamlPath = path.join(triviaioxCoreDir, 'core-config.yaml');

    // Ensure .triviaiox-core directory exists
    await fs.ensureDir(triviaioxCoreDir);
