/**
 * TRIVIAIOX-FullStack MCP Installation Module
...
async function installProjectMCPs(options = {}) {
  ...
  // Create .triviaiox directory for logs
  const triviaioxDir = path.join(projectPath, '.triviaiox');
  await fse.ensureDir(triviaioxDir);

  const logPath = path.join(triviaioxDir, 'install-log.txt');
  const errorLogPath = path.join(triviaioxDir, 'install-errors.log');
...
function displayInstallationStatus(results) {
  ...
  console.log(chalk.gray('📋 Installation log: .triviaiox/install-log.txt'));
  console.log('');
}
