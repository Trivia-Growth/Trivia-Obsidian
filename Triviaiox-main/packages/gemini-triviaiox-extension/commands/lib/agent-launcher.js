...
const AGENT_INFO = {
  'triviaiox-master': { icon: '🧠', role: 'Master Orchestrator' },
...
function listAvailableAgents(projectRoot = process.cwd()) {
  const sourceDir = path.join(projectRoot, '.triviaiox-core', 'development', 'agents');
...
function commandNameForAgent(agentId) {
  if (agentId.startsWith('triviaiox-')) {
    return `/triviaiox-${agentId.replace(/^triviaiox-/, '')}`;
  }
  return `/triviaiox-${agentId}`;
}

function hasAgent(projectRoot, agentId) {
  const canonical = path.join(projectRoot, '.triviaiox-core', 'development', 'agents', `${agentId}.md`);
  const gemini = path.join(projectRoot, '.gemini', 'rules', 'TRIVIAIOX', 'agents', `${agentId}.md`);
  return fs.existsSync(canonical) || fs.existsSync(gemini);
}

function renderGreeting(projectRoot, agentId) {
  const scriptPath = path.join(projectRoot, '.triviaiox-core', 'development', 'scripts', 'generate-greeting.js');
...
function buildActivationPrompt(agentId) {
  return [
    `Ative o agente ${agentId} usando .triviaiox-core/development/agents/${agentId}.md`,
    `(fallback: .gemini/rules/TRIVIAIOX/agents/${agentId}.md),`,
    `renderize o greeting via node .triviaiox-core/development/scripts/generate-greeting.js ${agentId}`,
...
function runAgentLauncher(agentId, projectRoot = process.cwd()) {
  if (!agentId) {
    console.log('Uso: /triviaiox-agent <agent-id>');
    return 1;
  }
  ...
  console.log(`${info.icon} TRIVIAIOX Agent Selected: ${agentId}`);
...
function runAgentMenu(projectRoot = process.cwd()) {
  ...
  console.log('🤖 TRIVIAIOX Quick Agent Menu (Gemini)');
  console.log('');

  if (agents.length === 0) {
    console.log('No TRIVIAIOX agents found. Run: npm run sync:ide:gemini');
    return 1;
  }
  ...
  console.log('\nTip: run /triviaiox-<agent-id> to prepare activation prompt quickly.');
  return 0;
}
