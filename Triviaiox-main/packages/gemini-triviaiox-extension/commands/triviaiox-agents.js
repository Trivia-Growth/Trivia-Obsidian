#!/usr/bin/env node
/**
 * TRIVIAIOX Agents Command - List available agents
 */
...
async function main() {
  ...
  const agentsPath = path.join(projectDir, '.triviaiox-core', 'development', 'agents');

  console.log('🤖 TRIVIAIOX Agents\n');
  console.log('━'.repeat(50));

  if (!fs.existsSync(agentsPath)) {
    console.log('No agents found. Run: npx triviaiox-core install');
    return;
  }
  ...
  console.log('\n' + '━'.repeat(50));
  console.log('Quick launch with: /triviaiox-menu or /triviaiox-<agent-id>');
  console.log('Alternative: /triviaiox-agent <agent-id>');
}
