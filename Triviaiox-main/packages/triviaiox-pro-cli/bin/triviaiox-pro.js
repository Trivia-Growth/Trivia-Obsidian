 ...
 * triviaiox-pro CLI
 *
 * Thin CLI wrapper for @triviaiox-fullstack/pro.
 * Provides a clean npx interface: npx triviaiox-pro install
 *
 * Commands:
 *   install             Install @triviaiox-fullstack/pro in the current project
...
const PRO_PACKAGE = '@triviaiox-fullstack/pro';
...
function isProInstalled() {
  try {
    const pkgPath = path.join(process.cwd(), 'node_modules', '@triviaiox-fullstack', 'pro', 'package.json');
...
function findTriviaioxCli() {
  // Check local node_modules first
  const localBin = path.join(process.cwd(), 'node_modules', '.bin', 'triviaiox');
  if (fs.existsSync(localBin) || fs.existsSync(localBin + '.cmd')) {
    return 'npx triviaiox';
  }

  // Check global
  try {
    execSync('triviaiox --version', { stdio: 'pipe' });
    return 'triviaiox';
  } catch {
    return null;
  }
...
function delegateToTriviaiox(subcommand) {
  const triviaiox = findTriviaioxCli();
  if (!triviaiox) {
    console.error('triviaiox-core CLI not found.');
    console.error('Install it first: npm install triviaiox-core');
    process.exit(1);
  }

  const spawnArgs = ['pro', subcommand, ...args.slice(1)];
  const result = spawnSync(triviaiox, spawnArgs, { stdio: 'inherit' });
...
function runProWizard(key) {
  ...
  try {
    ...
    console.error('Ensure triviaiox-core installer is available.\n');
    process.exit(1);
  }
...
function showHelp() {
  console.log(`
triviaiox-pro v${VERSION} — TRIVIAIOX Pro CLI

Usage:
  npx triviaiox-pro <command> [options]
...
Examples:
  ...
  npx triviaiox-pro setup
  npx triviaiox-pro wizard --key PRO-XXXX-XXXX-XXXX-XXXX
  npx triviaiox-pro activate --key PRO-XXXX-XXXX-XXXX-XXXX
  npx triviaiox-pro status
  npx triviaiox-pro recover

Documentation: https://triviagrowth.ai/pro/docs
`);
}
...
function installPro() {
  ...
  console.log('  npx triviaiox-pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('  npx triviaiox-pro status');
  console.log('');
}
...
if (command === '--version' || command === '-v') {
  console.log(`triviaiox-pro v${VERSION}`);
  process.exit(0);
}
...
switch (command) {
  ...
  case 'validate':
    if (!isProInstalled()) {
      ...
      console.error('Run first: npx triviaiox-pro install\n');
      process.exit(1);
    }
    delegateToTriviaiox(command);
    break;
