/**
 * TRIVIAIOX Installer - Main installation logic
 *
 * Orchestrates the complete TRIVIAIOX installation flow:
 * 1. OS detection
...
/**
 * Check if a config-resolver is available (triviaiox-core installed)
 * @param {string} projectRoot - Project root
 * @returns {Object|null} Config resolver module or null
 */
function tryLoadConfigResolver(projectRoot) {
  const possiblePaths = [
    path.join(projectRoot, '.triviaiox-core/core/config/config-resolver.js'),
    path.join(projectRoot, 'node_modules/triviaiox-core/.triviaiox-core/core/config/config-resolver.js'),
  ];
...
async function createUserConfigDirect(profile, logger, dryRun) {
  const userConfigDir = path.join(os.homedir(), '.triviaiox');
...
/**
 * Check if this is a brownfield installation (existing TRIVIAIOX)
 * @param {string} projectRoot - Project root
...
function detectBrownfield(projectRoot) {
  ...
  const legacyConfigPath = path.join(projectRoot, '.triviaiox-core/core-config.yaml');
  ...
  const frameworkConfigPath = path.join(projectRoot, '.triviaiox-core/framework-config.yaml');
  if (fs.existsSync(frameworkConfigPath)) {
    result.isBrownfield = true;
    result.hasLayeredConfig = true;
  }

  // Check for .triviaiox-core directory
...
/**
 * Run the TRIVIAIOX doctor command
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx triviaiox-core doctor');
    return;
  }

  const spinner = ora('Running TRIVIAIOX doctor...').start();

  try {
    const { stdout } = await execa('npx', ['triviaiox-core', 'doctor'], {
      cwd: projectRoot,
      timeout: 60000,
    });

    spinner.succeed('TRIVIAIOX doctor completed');

    if (logger.verbose) {
      console.log(chalk.dim(stdout));
    }
  } catch (error) {
    spinner.warn('TRIVIAIOX doctor had warnings');
    if (logger.verbose) {
...
/**
 * Install triviaiox-core package
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function installTriviaioxCore(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npm install triviaiox-core --save-dev');
    return;
  }

  const spinner = ora('Installing triviaiox-core...').start();

  try {
    await execa('npm', ['install', 'triviaiox-core', '--save-dev'], {
      cwd: projectRoot,
      timeout: 300000, // 5 minutes
    });

    spinner.succeed('triviaiox-core installed');
  } catch (error) {
    spinner.fail('Failed to install triviaiox-core');
    throw error;
  }
}

/**
 * Initialize TRIVIAIOX in the project
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function initializeTriviaiox(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx triviaiox-core install');
    return;
  }

  const spinner = ora('Initializing TRIVIAIOX...').start();

  try {
    await execa('npx', ['triviaiox-core', 'install'], {
      cwd: projectRoot,
      timeout: 120000, // 2 minutes
    });

    spinner.succeed('TRIVIAIOX initialized');
  } catch (error) {
    spinner.fail('Failed to initialize TRIVIAIOX');
    throw error;
  }
}
...
async function runInstaller(options = {}) {
  ...
  // Introduction
  intro(chalk.bgCyan(' TRIVIAIOX Installer '));
  ...
  if (!profile) {
    ...
    const profileSelection = await select({
      message: 'Select your TRIVIAIOX profile:',
      options: [
        {
          value: 'bob',
  ...
  if (brownfield.isBrownfield) {
    logger.info('Existing TRIVIAIOX installation detected');
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['triviaiox-core', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
        ...
        logger.action('Run: npx triviaiox-core config migrate');
      }
    } else {
    ...
    // Install triviaiox-core
    await installTriviaioxCore(projectRoot, logger, options.dryRun);

    // Initialize TRIVIAIOX
    await initializeTriviaiox(projectRoot, logger, options.dryRun);
  }
  ...
  if (options.dryRun) {
    ...
    outro(chalk.green(`TRIVIAIOX installed successfully in ${elapsed}!`));
    console.log('');
    console.log(chalk.dim('Next steps:'));
    console.log(chalk.dim('  1. Run `npx triviaiox-core info` to see your configuration'));
