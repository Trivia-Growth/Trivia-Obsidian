 ...
 * node_modules/@triviaiox-fullstack/pro/ into the user's project after
 * license activation.
 *
...
const SCAFFOLD_ITEMS = [
  ...
  },
  {
    ...
    dest: path.join('.triviaiox-core', 'pro-config.yaml'),
  ...
  },
  {
    ...
    dest: path.join('.triviaiox-core', 'feature-registry.yaml'),
 ...
 * @param {string} proSourceDir - Path to pro package content (node_modules/@triviaiox-fullstack/pro)
...
async function scaffoldProContent(targetDir, proSourceDir, options = {}) {
  ...
  if (!await fs.pathExists(proSourceDir)) {
    result.errors.push(
      `Pro package not found at ${proSourceDir}. Run "npm install @triviaiox-fullstack/pro" first.`
    );
    return result;
  }
...
async function mergeProConfig(targetDir) {
  ...
  const proConfigPath = path.join(targetDir, '.triviaiox-core', 'pro-config.yaml');
