...
/**
 * TRIVIAIOX Installer Performance Benchmark
...
/**
 * Get the .triviaiox-core directory for benchmarking
 */
function getTriviaioxCoreDir() {
  const projectRoot = path.resolve(__dirname, '../../../../');
  return path.join(projectRoot, '.triviaiox-core');
}
...
async function runBenchmarks() {
  const triviaioxCoreDir = getTriviaioxCoreDir();

  if (!fs.existsSync(triviaioxCoreDir)) {
    console.error(`Error: .triviaiox-core directory not found at ${triviaioxCoreDir}`);
    process.exit(1);
  }

  log(`Starting benchmark with ${CONFIG.runs} runs`);
  log(`Using .triviaiox-core at: ${triviaioxCoreDir}`);

  // Collect files for benchmarking
  log('Collecting files...');
  const allFiles = collectFiles(triviaioxCoreDir, CONFIG.testProjectSize);
  ...
  for (let run = 1; run <= CONFIG.runs; run++) {
    ...
    const agentsDir = path.join(triviaioxCoreDir, 'development', 'agents');
  ...
  console.error('\n' + '='.repeat(60));
  console.error('TRIVIAIOX Installer Performance Baseline');
