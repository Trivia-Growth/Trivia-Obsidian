 ...
 * 1. EXISTING_TRIVIAIOX - .triviaiox-core/ directory exists
 ...
 * @returns {string} 'GREENFIELD' | 'BROWNFIELD' | 'EXISTING_TRIVIAIOX' | 'UNKNOWN'
...
function detectProjectType(targetDir = process.cwd()) {
  try {
    ...
    // Check for TRIVIAIOX installation markers (use path.join for security)
    const hasTriviaioxCore = fs.existsSync(path.join(normalizedDir, '.triviaiox-core'));
    
    // If TRIVIAIOX already installed, return immediately (highest priority)
    if (hasTriviaioxCore) {
      return 'EXISTING_TRIVIAIOX';
    }
