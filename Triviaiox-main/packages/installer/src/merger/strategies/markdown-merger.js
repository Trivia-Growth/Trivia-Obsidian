/**
 * @fileoverview Merge strategy for Markdown files with TRIVIAIOX-MANAGED sections
...
const { parseMarkdownSections, hasTriviaioxMarkers } = require('../parsers/markdown-section-parser.js');

/**
 * Merge strategy for Markdown files (CLAUDE.md, rules.md).
 * Uses TRIVIAIOX-MANAGED markers to identify sections that can be updated.
...
class MarkdownMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, options = {}) {
    ...
    // If existing file has no TRIVIAIOX markers, do legacy migration
    if (!existing.hasTriviaioxMarkers) {
      return this.migrateLegacy(existingContent, template, options);
    }

    const stats = createEmptyStats();
    const changes = [];
    const processedTriviaioxSections = new Set();
    ...
    for (const section of existing.sections) {
      if (section.managed) {
        // TRIVIAIOX-managed section - update with template content if available
        ...
        if (templateSection) {
          ...
          merged += `<!-- TRIVIAIOX-MANAGED-START: ${section.id} -->\n`;
          merged += templateSection.lines.join('\n');
          if (!merged.endsWith('\n')) {
            merged += '\n';
          }
          merged += `<!-- TRIVIAIOX-MANAGED-END: ${section.id} -->\n\n`;
          processedTriviaioxSections.add(section.id);
          changes.push({
            type: 'updated',
    ...
    // Add new TRIVIAIOX sections that don't exist in current file
    const newSections = template.sections.filter(
      s => s.managed && !processedTriviaioxSections.has(s.id),
    );

    if (newSections.length > 0) {
      merged += '---\n\n';
      merged += '<!-- New TRIVIAIOX sections added -->\n\n';
  ...
  /**
   * Migrate a legacy file (without TRIVIAIOX markers) by appending TRIVIAIOX sections
  ...
  async migrateLegacy(existingContent, template, _options = {}) {
    ...
    // Get all TRIVIAIOX-managed sections from template
    const triviaioxSections = template.sections.filter(s => s.managed);
    ...
    // Add separator and TRIVIAIOX sections
    if (triviaioxSections.length > 0) {
      merged += '\n---\n\n';
      merged += '<!-- TRIVIAIOX-MANAGED SECTIONS -->\n';
      merged += '<!-- These sections are managed by TRIVIAIOX. Edit content between markers carefully. -->\n';
      merged += '<!-- Your custom content above will be preserved during updates. -->\n\n';

      for (const section of triviaioxSections) {
        merged += `<!-- TRIVIAIOX-MANAGED-START: ${section.id} -->\n`;
        merged += section.lines.join('\n');
        if (!merged.endsWith('\n')) {
          merged += '\n';
   ...
   */
  getDescription() {
    return 'Merges Markdown files using TRIVIAIOX-MANAGED section markers';
  }
}
