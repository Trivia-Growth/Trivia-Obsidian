...
class EnvMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, _options = {}) {
    ...
    // Add TRIVIAIOX section if there are new vars or suggestions
    if (newVars.length > 0 || suggestions.length > 0) {
      const date = new Date().toISOString().split('T')[0];
      merged += `\n# === TRIVIAIOX Variables (added ${date}) ===\n`;

      // Add new variables
      for (const { key, value, comment } of newVars) {
        if (comment && !comment.includes('TRIVIAIOX')) {
          // Add original comment if it's not an TRIVIAIOX header
          merged += `${comment}\n`;
        }
        merged += `${key}=${value}\n`;
      }
      ...
      if (suggestions.length > 0) {
        merged += '\n# TRIVIAIOX_NOTE: The following variables already exist with different values:\n';
        for (const { key, suggestedValue } of suggestions) {
          merged += `# TRIVIAIOX_SUGGESTED: ${key}=${suggestedValue}\n`;
        }
      }
    }
