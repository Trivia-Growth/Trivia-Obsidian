/**
 * TRIVIAIOX Updater
 * Intelligent update system for TRIVIAIOX-Core installations
 *
...
/**
 * TRIVIAIOX Updater Class
 * Handles intelligent updates while preserving user customizations
 */
class TRIVIAIOXUpdater {
  /**
   * Create a new TRIVIAIOXUpdater instance
   *
  ...
  constructor(projectRoot, options = {}) {
    ...
    this.triviaioxCoreDir = path.join(this.projectRoot, '.triviaiox-core');
    this.triviaioxConfigDir = path.join(this.projectRoot, '.triviaiox');
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      if (!result.installed) {
        result.error = 'TRIVIAIOX not installed or version info not found';
        return result;
      }
      ...
      if (!result.latest) {
        ...
        if (!isOnline) {
          ...
          result.error = 'Package triviaiox-core not found on npm registry. This may be a local development installation.';
        }
        return result;
      }
  ...
  async getInstalledVersion() {
    ...
    const versionJsonPath = path.join(this.triviaioxCoreDir, 'version.json');
    ...
    const packageJsonPath = path.join(this.projectRoot, 'node_modules', 'triviaiox-core', 'package.json');
    ...
    if (fs.existsSync(localPackageJsonPath)) {
      try {
        ...
        if (pkg.name === '@trivia-growth/triviaiox-core' || pkg.name === 'triviaiox-core') {
  ...
  async getLatestVersion() {
    return new Promise((resolve) => {
      const request = https.get(
        'https://registry.npmjs.org/triviaiox-core/latest',
  ...
  async detectCustomizations() {
    ...
    for (const [relativePath, originalHash] of Object.entries(this.versionInfo.fileHashes)) {
      const absolutePath = path.join(this.triviaioxCoreDir, relativePath);
   ...
   */
  async createBackup() {
    this.backupDir = path.join(this.triviaioxConfigDir, 'backup', `pre-update-${Date.now()}`);
    ...
    for (const file of filesToBackup) {
      const src = path.join(this.triviaioxCoreDir, file);
  ...
  async rollback() {
    ...
    for (const file of backupFiles) {
      ...
      const dest = path.join(this.triviaioxCoreDir, file);
  ...
  async applyUpdate(targetVersion, _customizedFiles = []) {
    ...
    try {
      ...
      const cmd = `npm install triviaiox-core@${targetVersion} --save-exact`;
      ...
      // TODO: Copy new files from node_modules to .triviaiox-core
  ...
  log(message) {
    if (this.options.verbose) {
      console.log(`[TRIVIAIOXUpdater] ${message}`);
    }
  }
}
...
function formatCheckResult(result, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}🔍 TRIVIAIOX Update Check${c.reset}`);
  lines.push('');
  ...
  switch (result.status) {
    ...
    case UpdateStatus.UPDATE_AVAILABLE:
      ...
      lines.push(`  Run ${c.cyan}npx triviaiox-core update${c.reset} to update.`);
      break;
...
function formatUpdateResult(result, options = {}) {
  ...
  if (result.success) {
    ...
    lines.push('');
    lines.push(`Run ${c.cyan}npx triviaiox-core validate${c.reset} to verify installation.`);
  } else {
