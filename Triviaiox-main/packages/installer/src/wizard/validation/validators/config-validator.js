 ...
 * @private
 */
async function validateCoreConfig(results, configPath = '.triviaiox-core/core-config.yaml') {
...
async function validateGitignore(results) {
  ...
  try {
    ...
    const recommendedEntries = ['.env.local', '*.key', '*.pem', '.triviaiox/install-log.txt'];
