 ...
 * @param {string} options.projectType - 'BROWNFIELD' | 'GREENFIELD' | 'EXISTING_TRIVIAIOX'
...
async function promptFileExists(filePath, options = {}) {
  ...
  const isBrownfield = projectType === 'BROWNFIELD' || projectType === 'EXISTING_TRIVIAIOX';
...
function generateTemplateVariables(wizardState) {
  ...
  return {
    ...
    timestamp,
    triviaioxVersion: '2.1.0', // From package.json in real implementation
  };
}

/**
 * Copy agent files from .triviaiox-core/development/agents to IDE-specific agent folder
...
async function copyAgentFiles(projectRoot, agentFolder, ideConfig = null) {
  ...
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.triviaiox-core', 'development', 'agents');
  ...
  for (const file of agentFiles) {
    ...
    if (stat.isFile()) {
      if (isAntiGravity) {
        ...
        try {
          const agentParser = require('../../../../.triviaiox-core/infrastructure/scripts/ide-sync/agent-parser');
          const copilotTransformer = require('../../../../.triviaiox-core/infrastructure/scripts/ide-sync/transformers/github-copilot');
...
async function createAntiGravityConfigJson(projectRoot, ideConfig) {
  ...
  const config = {
    ...
    paths: {
      ...
      tasks: '.triviaiox-core/tasks',
      workflows: '.triviaiox-core/workflows',
    },
  };
  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // Load template from .triviaiox-core/product/templates/
        const templatePath = path.join(__dirname, '..', '..', '..', '..', '.triviaiox-core', 'product', 'templates', ide.template);
        ...
        if (ideKey === 'gemini') {
          ...
          if (settingsFile) {
            createdFiles.push(settingsFile);
            spinner.succeed('Created .gemini/settings.json with TRIVIAIOX hooks');
          } else {
            spinner.info('Skipped .gemini/settings.json (no hooks to register)');
          }

          spinner.start('Linking Gemini TRIVIAIOX extension...');
          const extensionResult = await linkGeminiExtension(projectRoot);
          if (extensionResult.status === 'linked') {
            spinner.succeed('Gemini extension "triviaiox" linked and enabled');
          } else if (extensionResult.status === 'already-linked') {
            spinner.succeed('Gemini extension "triviaiox" already linked');
          } else {
...
function showSuccessSummary(result) {
  ...
  console.log('  2. The IDE should automatically recognize TRIVIAIOX configuration');
...
/**
 * Copy .triviaiox-core/hooks/gemini folder into .gemini/hooks during installation
 * @param {string} projectRoot - Project root directory
 * @returns {Promise<string[]>} List of copied files
 */
async function copyGeminiHooksFolder(projectRoot) {
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.triviaiox-core', 'hooks', 'gemini');
...
/**
 * Create/merge .gemini/settings.json and register TRIVIAIOX hooks as enabled.
 * @param {string} projectRoot - Project root directory
...
async function createGeminiSettings(projectRoot) {
  ...
  const hookEntries = [
    ...
    },
    {
      ...
      matcher: '*',
      hook: {
        name: 'triviaiox-session-persist',
        type: 'command',
...
/**
 * Best-effort Gemini extension linking for TRIVIAIOX project.
...
async function linkGeminiExtension(projectRoot) {
  const extensionDir = path.join(projectRoot, 'packages', 'gemini-triviaiox-extension');
  ...
  if (output.includes('already installed')) {
    const uninstall = spawnSync('gemini', ['extensions', 'uninstall', 'triviaiox'], {
      cwd: projectRoot,
      encoding: 'utf8',
      timeout: 30000,
    });
...
async function copyExtraCommandFiles(projectRoot, _sourceRoot) {
  ...
  const DISTRIBUTABLE_ENTRIES = new Set([
    'TRIVIAIOX',       // Core agent/script commands (agents/ sub-dir excluded below)
    'synapse',    // SYNAPSE context engine commands
    'greet.md',   // Greeting skill
  ]);

  // Within TRIVIAIOX/, these sub-dirs are excluded (private or handled separately)
  const TRIVIAIOX_EXCLUDED = new Set([
    'TRIVIAIOX/agents',   // Already handled by copyAgentFiles()
    'TRIVIAIOX/stories',  // Project-specific story skills, not distributable
  ]);
  ...
  async function copyRecursive(src, dest, relativePath) {
    ...
    for (const entry of entries) {
      ...
      // Within TRIVIAIOX/, skip excluded sub-directories
      if (TRIVIAIOX_EXCLUDED.has(entryRelative) || [...TRIVIAIOX_EXCLUDED].some(ex => entryRelative.startsWith(ex + '/'))) {
        continue;
      }
