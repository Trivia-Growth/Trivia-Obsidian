/**
 * TRIVIAIOX Interactive Wizard - Main Entry Point
 *
...
const { colors } = require('../utils/triviaiox-colors');
const {
  getLanguageQuestion,
...
const { commandSync, commandValidate } = require('../../../../.triviaiox-core/infrastructure/scripts/ide-sync/index');
const {
  installTriviaioxCore,
  hasPackageJson,
} = require('../installer/triviaiox-core-installer');
const {
...
} = require('../../../../.triviaiox-core/infrastructure/scripts/llm-routing/install-llm-routing');
...
async function getExistingUserProfile(targetDir = process.cwd()) {
  const coreConfigPath = path.join(targetDir, '.triviaiox-core', 'core-config.yaml');
...
async function runWizard(options = {}) {
  try {
    ...
    // Show welcome message with TRIVIAIOX branding
    if (!options.quiet) {
      showWelcome();
    }
    ...
    // Story 1.4: Install TRIVIAIOX core framework (agents, tasks, workflows, templates)
    console.log('\n📦 Installing TRIVIAIOX core framework...');
    let triviaioxCoreResult = null;
    try {
      triviaioxCoreResult = await installTriviaioxCore({
      ...
      if (triviaioxCoreResult.success) {
        console.log(`✅ TRIVIAIOX core installed (${triviaioxCoreResult.installedFolders.length} folders)`);
        console.log(
          `   - Agents: ${triviaioxCoreResult.installedFolders.includes('agents') ? '✓' : '⨉'}`,
        );
        console.log(`   - Tasks: ${triviaioxCoreResult.installedFolders.includes('tasks') ? '✓' : '⨉'}`);
        console.log(
          `   - Workflows: ${triviaioxCoreResult.installedFolders.includes('workflows') ? '✓' : '⨉'}`,
        );
        console.log(
          `   - Templates: ${triviaioxCoreResult.installedFolders.includes('templates') ? '✓' : '⨉'}`,
        );
      }
      answers.triviaioxCoreInstalled = true;
      answers.triviaioxCoreResult = triviaioxCoreResult;
    } catch (error) {
      console.error('\n⚠️  TRIVIAIOX core installation failed:', error.message);
      answers.triviaioxCoreInstalled = false;
    }
    ...
    if (answers.selectedTechPreset && answers.selectedTechPreset !== 'none') {
      ...
      try {
        ...
        const possiblePresetDirs = [
          path.join(__dirname, '..', '..', '.triviaiox-core', 'data', 'tech-presets'),
          path.join(process.cwd(), '.triviaiox-core', 'data', 'tech-presets'),
        ];
        ...
        if (sourcePresetDir) {
          ...
          if (fse.existsSync(presetFile)) {
            // Copy preset to project's .triviaiox-core/data/tech-presets/
            const targetPresetDir = path.join(process.cwd(), '.triviaiox-core', 'data', 'tech-presets');
            await fse.ensureDir(targetPresetDir);

            // BUG-5 fix (INS-1): Guard against source === dest (e.g., running inside triviaiox-core repo)
            ...
            console.log(
              `   📁 Location: .triviaiox-core/data/tech-presets/${answers.selectedTechPreset}.md`,
            );
    ...
    // Story INS-4.3: Wire settings.json boundary generator after .triviaiox-core/ copy
    console.log('\n🔒 Generating boundary rules...');
    try {
      const settingsGenerator = require('../../../../.triviaiox-core/infrastructure/scripts/generate-settings-json');
      ...
      console.warn(`⚠️  settings.json generation failed: ${error.message} — run 'triviaiox doctor --fix' post-install`);
    ...
    try {
      ...
      if (answers.ideSyncValidation === 'drift') {
        console.warn('⚠️  IDE sync validation: drift detected — run \'triviaiox doctor --fix\' post-install');
      }
    } catch (syncError) {
      console.warn(`⚠️  IDE sync failed: ${syncError.message} — run 'triviaiox doctor --fix' post-install`);
    ...
    // Bootstrap runs AFTER .triviaiox-core deps are installed (triviaiox-core-installer.js:324-345)
    // NODE_PATH ensures spawned scripts can resolve packages from .triviaiox-core/node_modules/
    console.log('\n📇 Bootstrapping entity registry...');
    try {
      const registryScript = path.join(process.cwd(), '.triviaiox-core', 'development', 'scripts', 'populate-entity-registry.js');
      if (fse.existsSync(registryScript)) {
        // INS-4.12 AC3: Guard — skip bootstrap if .triviaiox-core deps are not installed
        const triviaioxCoreNodeModules = path.join(process.cwd(), '.triviaiox-core', 'node_modules');
        if (!fse.existsSync(triviaioxCoreNodeModules)) {
          console.warn('⚠️  .triviaiox-core/node_modules/ not found — skipping entity registry bootstrap');
          console.warn('   Run: cd .triviaiox-core && npm install --production');
          answers.entityRegistryStatus = 'skipped-no-deps';
        } else {
        // INS-4.12 AC2: Set NODE_PATH so spawned scripts resolve deps from .triviaiox-core/node_modules/
          const parentNodeModules = path.join(process.cwd(), 'node_modules');
          const nodePath = [triviaioxCoreNodeModules, parentNodeModules].join(path.delimiter);
          ...
          const registryPath = path.join(process.cwd(), '.triviaiox-core', 'data', 'entity-registry.yaml');
          let entityCount = 0;
      ...
      console.warn(`⚠️  Entity registry bootstrap failed: ${error.message} — run 'triviaiox doctor' post-install`);
    ...
    try {
      ...
      if (envResult.envCreated && envResult.coreConfigCreated) {
        ...
        console.log('  - .triviaiox-core/core-config.yaml created');
    ...
    //       console.log('\n💡 Check .triviaiox/install-errors.log for details');
    //     }
    //
    ...
    if (!options.skipPro) {
      try {
        ...
        const hasProKey = !!process.env.TRIVIAIOX_PRO_KEY;

        const proOptions = { targetDir: process.cwd() };

        if (isCI && hasProKey) {
          // CI mode: auto-run if TRIVIAIOX_PRO_KEY is set
    ...
    try {
      const validation = await validateInstallation(
        {
          files: {
            ...
            env: '.env',
            coreConfig: '.triviaiox-core/core-config.yaml',
            mcpConfig: '.mcp.json',
          },
          configs: {
      ...
      console.log('Installation may be incomplete. Check logs in .triviaiox/ directory.');
    }
