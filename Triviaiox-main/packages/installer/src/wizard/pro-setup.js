 ...
 * Supports interactive mode, CI mode (TRIVIAIOX_PRO_KEY/TRIVIAIOX_PRO_EMAIL env vars), and lazy import.
 *
...
const { colors, status } = require('../utils/triviaiox-colors');
 ...
 */
const LICENSE_SERVER_URL = process.env.TRIVIAIOX_LICENSE_API_URL || 'https://triviaiox-license-server.vercel.app';

/**
 * Inline License Client — lightweight HTTP client for pre-bootstrap license checks.
 *
 * Used when @triviaiox-fullstack/pro is not yet installed (first install scenario).
...
class InlineLicenseClient {
    ...
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        headers: {
          ...
          'User-Agent': 'triviaiox-installer',
          ...headers,
        },
        timeout: 15000,
      };
   ...
   * @param {string} version - triviaiox-core version
 ...
 * 2. @triviaiox-fullstack/pro package (brownfield: node_modules/@triviaiox-fullstack/pro/license/{name})
 * 3. Absolute path via triviaiox-core in node_modules (brownfield upgrade)
 * 4. Absolute path via @triviaiox-fullstack/pro in user project (npx context)
 *
 * Path 4 is critical for npx execution: when running `npx triviaiox-core install`,
 * require() resolves from the npx temp directory, not process.cwd(). After
 * bootstrap installs @triviaiox-fullstack/pro in the user's project, only an
 * absolute path to process.cwd()/node_modules/@triviaiox-fullstack/pro/... works.
 *
...
function loadProModule(moduleName) {
  ...
  // 2. @triviaiox-fullstack/pro package (works when triviaiox-core is a local dependency)
  try {
    return require(`@triviaiox-fullstack/pro/license/${moduleName}`);
  } catch { /* not available */ }

  // 3. triviaiox-core in node_modules (brownfield upgrade from >= v4.2.15)
  try {
    const absPath = path.join(process.cwd(), 'node_modules', 'triviaiox-core', 'pro', 'license', moduleName);
    return require(absPath);
  } catch { /* not available */ }

  // 4. @triviaiox-fullstack/pro in user project (npx context — require resolves from
  //    temp dir, so we need absolute path to where bootstrap installed the package)
  try {
    const absPath = path.join(process.cwd(), 'node_modules', '@triviaiox-fullstack', 'pro', 'license', moduleName);
    return require(absPath);
  } catch { /* not available */ }
 ...
 *
 * Prefers the full LicenseApiClient from @triviaiox-fullstack/pro when available.
...
function getLicenseClient() {
  ...
  // Fallback: use inline client for pre-bootstrap (no @triviaiox-fullstack/pro yet)
 ...
 *
 * In CI mode, reads from TRIVIAIOX_PRO_EMAIL + TRIVIAIOX_PRO_PASSWORD or TRIVIAIOX_PRO_KEY env vars.
 ...
 *
 * Priority: TRIVIAIOX_PRO_EMAIL + TRIVIAIOX_PRO_PASSWORD > TRIVIAIOX_PRO_KEY
 *
 * @param {Object} options - Options with possible pre-provided credentials
 * @returns {Promise<Object>} Result with { success, key, activationResult }
 */
async function stepLicenseGateCI(options) {
  const email = options.email || process.env.TRIVIAIOX_PRO_EMAIL;
  const password = options.password || process.env.TRIVIAIOX_PRO_PASSWORD;
  const key = options.key || process.env.TRIVIAIOX_PRO_KEY;
...
async function stepLicenseGateWithEmail() {
  ...
  if (!checkResult.isBuyer) {
    ...
    showInfo('  Issues: https://github.com/Trivia-Growth/Triviaiox/issues');
    ...
    try {
      ...
      if (loginError.code === 'EMAIL_NOT_VERIFIED') {
        ...
        if (remaining > 0) {
          ...
          showInfo('Forgot your password? Visit https://triviaiox-license-server.vercel.app/reset-password');
        } else {
          spinner.fail('Maximum login attempts reached.');
          showInfo('Forgot your password? Visit https://triviaiox-license-server.vercel.app/reset-password');
          showInfo('Or open an issue: https://github.com/Trivia-Growth/Triviaiox/issues');
...
async function activateProByAuth(client, sessionToken) {
  ...
  try {
    ...
    // Read triviaiox-core version
    let triviaioxCoreVersion = 'unknown';
    try {
      const path = require('path');
      const fs = require('fs');
      const pkgPath = path.join(__dirname, '..', '..', '..', '..', 'package.json');
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
      triviaioxCoreVersion = pkg.version || 'unknown';
    } catch {
      // Keep 'unknown'
    }

    const activationResult = await client.activateByAuth(sessionToken, machineId, triviaioxCoreVersion);
...
async function validateKeyWithApi(key) {
  ...
  try {
    ...
    const activationResult = await client.activate(key, machineId, triviaioxCoreVersion);
...
async function stepInstallScaffold(targetDir, options = {}) {
  ...
  // 1. Bundled in triviaiox-core package (pro/ submodule — npx and local dev)
  // 2. @triviaiox-fullstack/pro in node_modules (legacy brownfield)
  const bundledProDir = path.resolve(__dirname, '..', '..', '..', '..', 'pro');
  const npmProDir = path.join(targetDir, 'node_modules', '@triviaiox-fullstack', 'pro');
...
async function runProWizard(options = {}) {
  ...
  // Step 1: License Gate (uses InlineLicenseClient if @triviaiox-fullstack/pro not yet installed)
  const licenseResult = await stepLicenseGate({
    key: options.key || process.env.TRIVIAIOX_PRO_KEY,
    email: options.email || process.env.TRIVIAIOX_PRO_EMAIL,
    password: options.password || process.env.TRIVIAIOX_PRO_PASSWORD,
  });
