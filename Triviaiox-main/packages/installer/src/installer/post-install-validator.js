 ...
 * Comprehensive validation of TRIVIAIOX-Core installation with security hardening
 */
class PostInstallValidator {
  /**
   * Create a new PostInstallValidator instance
   *
   * @param {string} targetDir - Directory where TRIVIAIOX was installed (project root)
  ...
  constructor(targetDir, sourceDir = null, options = {}) {
    ...
    this.triviaioxCoreTarget = path.join(this.targetDir, '.triviaiox-core');
    this.triviaioxCoreSource = this.sourceDir ? path.join(this.sourceDir, '.triviaiox-core') : null;
  ...
  _getRealTargetDir() {
    if (this._realTargetDirCache === null) {
      try {
        this._realTargetDirCache = fs.realpathSync(this.triviaioxCoreTarget);
      } catch {
  ...
  async loadManifest() {
    ...
    const sourceManifestPath = this.triviaioxCoreSource
      ? path.join(this.triviaioxCoreSource, 'install-manifest.yaml')
      : null;
    const targetManifestPath = path.join(this.triviaioxCoreTarget, 'install-manifest.yaml');
  ...
  async validateFile(entry) {
    ...
    const absolutePath = path.resolve(this.triviaioxCoreTarget, relativePath);
    ...
    if (!isPathContained(absolutePath, this.triviaioxCoreTarget)) {
      ...
      if (error.code === 'ENOENT') {
        result.issue = {
          ...
          remediation: this.sourceDir
            ? "Run 'triviaiox validate --repair' to restore"
            : 'Re-run installation',
          relativePath,
        };
    ...
    // allowing path traversal attacks (e.g., /install/.triviaiox-core/symlinked-dir/../../../etc/passwd)
    //
    ...
    try {
      ...
      const expectedRelative = path.relative(this.triviaioxCoreTarget, absolutePath);
  ...
  async detectExtraFiles() {
    ...
    await scanDir(this.triviaioxCoreTarget, this.triviaioxCoreTarget);
    return extraFiles;
  }
  ...
  async validate() {
    ...
    if (!fs.existsSync(this.triviaioxCoreTarget)) {
      this.issues.push({
        type: IssueType.MISSING_FILE,
        severity: Severity.CRITICAL,
        message: 'TRIVIAIOX-Core directory not found',
        details: `Expected at: ${this.triviaioxCoreTarget}`,
        remediation: 'Run `npx triviaiox-core install`',
        relativePath: null,
      });
  ...
  generateRecommendations() {
    ...
    if (this.stats.missingFiles > 0) {
      if (this.stats.missingFiles > 50) {
        ...
        recommendations.push(
          `${this.stats.missingFiles} file(s) missing. Run 'triviaiox validate --repair'.`,
        );
      }
    }
    ...
    if (this.stats.corruptedFiles > 0) {
      recommendations.push(
        `${this.stats.corruptedFiles} file(s) corrupted. Run 'triviaiox validate --repair'.`,
      );
    }
  ...
  async repair(options = {}) {
    ...
    if (!this.sourceDir || !fs.existsSync(this.triviaioxCoreSource)) {
      return {
        success: false,
    ...
    for (let i = 0; i < repairableIssues.length; i++) {
      ...
      const sourcePath = path.resolve(this.triviaioxCoreSource, relativePath);
      const targetPath = path.resolve(this.triviaioxCoreTarget, relativePath);

      onProgress(i + 1, repairableIssues.length, relativePath);

      // SECURITY: Path containment for source
      if (!isPathContained(sourcePath, this.triviaioxCoreSource)) {
        result.skipped.push({ path: relativePath, reason: 'Source path traversal blocked' });
        continue;
      }

      // SECURITY: Path containment for target
      if (!isPathContained(targetPath, this.triviaioxCoreTarget)) {
      ...
      try {
        ...
        // Walk each path component from triviaioxCoreTarget to targetDir
        // and verify none are symlinks
        let currentPath = this.triviaioxCoreTarget;
        const relativeParts = path.relative(this.triviaioxCoreTarget, targetDir).split(path.sep);
        ...
        for (const part of relativeParts) {
          ...
          try {
            ...
            if (componentStat.isSymbolicLink()) {
              result.skipped.push({
                path: relativePath,
                reason: `Symlink detected in path component: ${path.relative(this.triviaioxCoreTarget, currentPath)}`,
              });
              continue;
            }
          } catch (_statError) {
        ...
        // Final realpath verification: ensure resolved target stays within resolved triviaioxCoreTarget
        ...
        if (fs.existsSync(targetDir)) {
          ...
          const realTriviaioxCoreTarget = fs.realpathSync(this.triviaioxCoreTarget);

          if (!isPathContained(realTargetDir, realTriviaioxCoreTarget)) {
            result.skipped.push({
              path: relativePath,
              reason: `Realpath escapes target directory: ${realTargetDir} is outside ${realTriviaioxCoreTarget}`,
            });
            continue;
          }
        }
...
function formatReport(report, options = {}) {
  ...
  lines.push('');
  lines.push(`${c.bold}TRIVIAIOX-Core Installation Validation Report${c.reset}`);
