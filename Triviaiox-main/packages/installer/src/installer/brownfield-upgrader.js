/**
 * Brownfield Upgrader
 * Handles incremental upgrades for existing TRIVIAIOX-Core installations
 *
...
function loadInstalledManifest(targetDir) {
  return loadManifest(
    path.join(targetDir, '.triviaiox-core'),
 ...
 * @param {string} sourceDir - Source package directory (.triviaiox-core from npm)
...
function generateUpgradeReport(sourceManifest, installedManifest, targetDir) {
  ...
  for (const [filePath, sourceEntry] of sourceMap) {
    ...
    const absolutePath = path.join(triviaioxCoreDir, filePath);
 ...
 * @param {string} sourceDir - Source directory (npm package .triviaiox-core)
 * @param {string} targetDir - Target project directory
...
async function applyUpgrade(report, sourceDir, targetDir, options = {}) {
  ...
  // Ensure .triviaiox-core directory exists
  if (!dryRun) {
  ...
  for (const file of report.newFiles) {
    ...
    const targetPath = path.join(triviaioxCoreDir, file.path);
...
function updateInstalledManifest(targetDir, sourceManifest, sourcePackage) {
  const installedManifestPath = path.join(targetDir, '.triviaiox-core', '.installed-manifest.yaml');
  ...
  const header = `# TRIVIAIOX-Core Installed Manifest
...
function formatUpgradeReport(report) {
  ...
  lines.push('TRIVIAIOX-Core Upgrade Report');
  lines.push('═'.repeat(60));
  lines.push('');
