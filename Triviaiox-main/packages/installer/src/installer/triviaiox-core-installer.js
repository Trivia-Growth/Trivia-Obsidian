/**
 * TRIVIAIOX Core Installer Module
 *
 * Story 1.4/1.6: IDE Selection & Environment Configuration
 * Handles copying .triviaiox-core content (agents, tasks, workflows, templates, etc.)
 * to the target project directory.
 *
 * @module installer/triviaiox-core-installer
 */
...
/**
 * Get the path to the source .triviaiox-core directory in the package
 * @returns {string} Absolute path to .triviaiox-core source
 */
function getTriviaioxCoreSourcePath() {
  // Navigate from packages/installer/src/installer/ to project root/.triviaiox-core
  return path.join(__dirname, '..', '..', '..', '..', '.triviaiox-core');
}

/**
 * Folders to copy from .triviaiox-core
...
/**
 * Root files to copy from .triviaiox-core
 * @constant {string[]}
 */
const ROOT_FILES_TO_COPY = [
  ...
  'constitution.md',    // TRIVIAIOX fundamental principles
  'user-guide.md',
 ...
 * @param {string} rootPath - Replacement path (e.g., '.triviaiox-core')
 * @returns {string} Content with {root} replaced
 */
function replaceRootPlaceholder(content, rootPath = '.triviaiox-core') {
 ...
 *
 * @param {string} targetTriviaioxCore - Path to .triviaiox-core directory
 * @param {string[]} installedFiles - List of installed files (relative to .triviaiox-core)
 * @returns {Promise<Object>} Object mapping file paths to their sha256 hashes
 */
async function generateFileHashes(targetTriviaioxCore, installedFiles) {
  const fileHashes = {};

  for (const filePath of installedFiles) {
    const absolutePath = path.join(targetTriviaioxCore, filePath);
 ...
 * @param {string} options.targetTriviaioxCore - Path to .triviaiox-core directory
...
async function generateVersionJson(options) {
  ...
  const fileHashes = await generateFileHashes(targetTriviaioxCore, installedFiles);
  ...
  const versionJsonPath = path.join(targetTriviaioxCore, 'version.json');
...
async function copyFileWithRootReplacement(sourcePath, destPath, replaceRoot = true) {
  try {
    ...
    if (needsReplacement) {
      ...
      const updatedContent = replaceRootPlaceholder(content, '.triviaiox-core');
...
/**
 * Install .triviaiox-core content to target directory
 *
 ...
 *
 * @example
 * const result = await installTriviaioxCore({ targetDir: '/path/to/project' });
 * console.log(result.installedFiles); // List of installed files
 */
async function installTriviaioxCore(options = {}) {
  const {
  ...
  const spinner = ora('Installing TRIVIAIOX core framework...').start();

  try {
    const sourceDir = getTriviaioxCoreSourcePath();
    const targetTriviaioxCore = path.join(targetDir, '.triviaiox-core');

    // Check if source exists
    if (!await fs.pathExists(sourceDir)) {
      throw new Error('.triviaiox-core source directory not found in package');
    }

    // Create target .triviaiox-core directory
    await fs.ensureDir(targetTriviaioxCore);

    // Copy each folder
    for (const folder of FOLDERS_TO_COPY) {
      const folderSource = path.join(sourceDir, folder);
      const folderDest = path.join(targetTriviaioxCore, folder);
    ...
    for (const file of ROOT_FILES_TO_COPY) {
      ...
      const fileDest = path.join(targetTriviaioxCore, file);
    ...
    await fs.writeFile(
      path.join(targetTriviaioxCore, 'install-manifest.yaml'),
    ...
    // BUG-2 fix (INS-1): Install .triviaiox-core dependencies after copy
    // The copied .triviaiox-core/package.json has dependencies (js-yaml, execa, etc.)
    // that must be installed for the activation pipeline to work
    // INS-4.12: Track dep install success for bootstrap guard
    const triviaioxCorePackageJson = path.join(targetTriviaioxCore, 'package.json');
    result.triviaioxCoreDepsInstalled = false;
    if (await fs.pathExists(triviaioxCorePackageJson)) {
      spinner.text = 'Installing .triviaiox-core dependencies (js-yaml, fast-glob, etc.)...';
      try {
        const { exec } = require('child_process');
        const { promisify } = require('util');
        const execAsync = promisify(exec);
        await execAsync('npm install --production --ignore-scripts', {
          cwd: targetTriviaioxCore,
          timeout: 60000,
        });
        result.triviaioxCoreDepsInstalled = true;
        spinner.succeed('Installed .triviaiox-core dependencies');
        spinner.start('Finishing installation...');
      } catch (depError) {
        spinner.warn(`Could not install .triviaiox-core dependencies: ${depError.message}`);
    ...
    result.success = true;
    spinner.succeed(`TRIVIAIOX core installed (${result.installedFiles.length} files)`);

  } catch (error) {
    spinner.fail('TRIVIAIOX core installation failed');
...
/**
 * Create a basic package.json for TRIVIAIOX projects
 * @param {Object} options - Options
 * @param {string} [options.targetDir=process.cwd()] - Target directory
...
async function createBasicPackageJson(options = {}) {
  ...
  const packageJson = {
    ...
    version: '0.1.0',
    description: `TRIVIAIOX-powered ${projectType} project`,
    private: true,
    scripts: {
    ...
    keywords: ['triviaiox', projectType],
    license: 'MIT',
  };
