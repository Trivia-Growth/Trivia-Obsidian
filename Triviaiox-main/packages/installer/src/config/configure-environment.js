...
async function configureEnvironment(options = {}) {
  ...
  try {
    ...
    // Step 2: API keys are configured later via .env or triviaiox-master
    // Skipping prompts during installation for better UX
    const apiKeys = {};

    if (!skipPrompts) {
      console.log('\n💡 API keys can be configured later in .env file or via triviaiox-master');
    }
    ...
    const coreConfigDir = path.join(targetDir, '.triviaiox-core');
    ...
    console.log('✅ Created .triviaiox-core/core-config.yaml');
...
/**
 * Update .gitignore to include critical entries for TRIVIAIOX projects
 *
...
async function updateGitignore(targetDir) {
  ...
  const criticalEntries = {
    ...
    'TRIVIAIOX Local': ['.triviaiox-core/local/', '.claude/settings.local.json', '.triviaiox/install-log.txt'],
  };
