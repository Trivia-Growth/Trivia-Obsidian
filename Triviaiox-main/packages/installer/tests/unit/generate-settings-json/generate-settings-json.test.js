...
const {
  ...
  generatePermissions,
  writeSettingsJson,
} = require('../../../../../.triviaiox-core/infrastructure/scripts/generate-settings-json');
...
function createTempProject(boundary, existingSettings) {
  ...
  const triviaioxCoreDir = path.join(tmpDir, '.triviaiox-core');
  fs.mkdirSync(triviaioxCoreDir, { recursive: true });
  ...
  fs.writeFileSync(path.join(tmpDir, '.triviaiox-core', 'core-config.yaml'), yamlContent, 'utf8');

  // Create directory structure for expansion tests
  if (boundary.protected.includes('.triviaiox-core/core/**')) {
    const coreDir = path.join(tmpDir, '.triviaiox-core', 'core');
...
describe('generate-settings-json', () => {
  describe('readBoundaryConfig', () => {
    test('reads boundary config from core-config.yaml', () => {
      const tmpDir = createTempProject({
        ...
        protected: ['.triviaiox-core/core/**', 'bin/triviaiox.js'],
        exceptions: ['.triviaiox-core/data/**'],
      });
      ...
      try {
        ...
        expect(config.protected).toContain('.triviaiox-core/core/**');
        expect(config.protected).toContain('bin/triviaiox.js');
        expect(config.exceptions).toContain('.triviaiox-core/data/**');
      } finally {
  ...
  describe('generatePermissions — frameworkProtection: true', () => {
    test('generates deny rules covering all protected paths', () => {
      const tmpDir = createTempProject({
        frameworkProtection: true,
        protected: ['.triviaiox-core/core/**', '.triviaiox-core/infrastructure/**', 'bin/triviaiox.js'],
        exceptions: ['.triviaiox-core/data/**'],
      });

      // Create infrastructure dir (no expansion for non-core paths)
      fs.mkdirSync(path.join(tmpDir, '.triviaiox-core', 'infrastructure'), { recursive: true });

      try {
        const boundary = readBoundaryConfig(tmpDir);
        const permissions = generatePermissions(boundary, tmpDir);

        // Should have deny rules for core subdirs (events/**, utils/**, index.js) + infrastructure/** + bin/triviaiox.js
        expect(permissions.deny.length).toBeGreaterThan(0);

        // Core expansion: events/**, utils/**, index.js → 6 deny rules (3 paths x 2 tools)
        expect(permissions.deny).toContain('Edit(.triviaiox-core/core/events/**)');
        expect(permissions.deny).toContain('Write(.triviaiox-core/core/events/**)');
        expect(permissions.deny).toContain('Edit(.triviaiox-core/core/utils/**)');
        expect(permissions.deny).toContain('Write(.triviaiox-core/core/utils/**)');
        expect(permissions.deny).toContain('Edit(.triviaiox-core/core/index.js)');
        expect(permissions.deny).toContain('Write(.triviaiox-core/core/index.js)');

        // Non-core paths stay as globs
        expect(permissions.deny).toContain('Edit(.triviaiox-core/infrastructure/**)');
        expect(permissions.deny).toContain('Write(.triviaiox-core/infrastructure/**)');
        expect(permissions.deny).toContain('Edit(bin/triviaiox.js)');
        expect(permissions.deny).toContain('Write(bin/triviaiox.js)');

        // Allow rules from exceptions
        expect(permissions.allow).toContain('Edit(.triviaiox-core/data/**)');
        expect(permissions.allow).toContain('Write(.triviaiox-core/data/**)');
        expect(permissions.allow).toContain('Read(.triviaiox-core/**)');
      } finally {
        cleanupTempProject(tmpDir);
      }
    });
    ...
    test('all 9 protected paths from core-config produce deny rules', () => {
      ...
      const protectedRoots = [
        ...
        '.triviaiox-core/infrastructure/',
        '.triviaiox-core/constitution.md',
        'bin/triviaiox.js',
        'bin/triviaiox-init.js',
      ];
  ...
  describe('generatePermissions — frameworkProtection: false', () => {
    test('produces no boundary deny rules', () => {
      const boundary = {
        ...
        protected: ['.triviaiox-core/core/**', '.triviaiox-core/infrastructure/**'],
        exceptions: ['.triviaiox-core/data/**'],
      };
