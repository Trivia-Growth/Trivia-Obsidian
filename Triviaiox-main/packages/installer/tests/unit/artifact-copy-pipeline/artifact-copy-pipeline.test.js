...
describe('artifact-copy-pipeline (Story INS-4.3)', () => {
  ...
  describe('copyExtraCommandFiles', () => {
    test('copies .md files recursively excluding TRIVIAIOX/agents/ via real function', async () => {
      const sourceRoot = createTempDir();
      const targetRoot = createTempDir();
      ...
      try {
        ...
        // Create TRIVIAIOX/agents/dev.md (should be EXCLUDED)
        const agentsDir = path.join(cmdDir, 'TRIVIAIOX', 'agents');
        fs.mkdirSync(agentsDir, { recursive: true });
        fs.writeFileSync(path.join(agentsDir, 'dev.md'), '# Dev Agent', 'utf8');

        // Create TRIVIAIOX/stories/story.md (should be EXCLUDED — project-specific)
        const storiesDir = path.join(cmdDir, 'TRIVIAIOX', 'stories');
        ...
        // Verify TRIVIAIOX/agents/ was excluded (handled by copyAgentFiles)
        expect(fs.existsSync(path.join(targetCmd, 'TRIVIAIOX', 'agents', 'dev.md'))).toBe(false);

        // Verify TRIVIAIOX/stories/ was excluded (project-specific)
        expect(fs.existsSync(path.join(targetCmd, 'TRIVIAIOX', 'stories', 'story.md'))).toBe(false);
  ...
  describe('generator failure graceful degradation', () => {
    test('generate-settings-json throws on missing config — install should catch', () => {
      const { generate } = require('../../../../../.triviaiox-core/infrastructure/scripts/generate-settings-json');
