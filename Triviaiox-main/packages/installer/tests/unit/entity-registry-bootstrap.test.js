 ...
 * handles failures gracefully, and that triviaiox doctor can verify the registry.
 */
...
const POPULATE_SCRIPT = path.join(
  __dirname, '..', '..', '..', '..', '.triviaiox-core', 'development', 'scripts', 'populate-entity-registry.js'
);
const DOCTOR_CHECK = path.join(
  __dirname, '..', '..', '..', '..', '.triviaiox-core', 'core', 'doctor', 'checks', 'entity-registry.js'
);
const REGISTRY_PATH = path.join(
  __dirname, '..', '..', '..', '..', '.triviaiox-core', 'data', 'entity-registry.yaml'
);
const PRE_PUSH_HOOK = path.join(__dirname, '..', '..', '..', '..', '.husky', 'pre-push');
const IDS_PRE_PUSH = path.join(
  __dirname, '..', '..', '..', '..', '.triviaiox-core', 'hooks', 'ids-pre-push.js'
);
...
describe('Entity Registry Bootstrap (Story INS-4.6)', () => {
  ...
  describe('AC1: Bootstrap called during install', () => {
    ...
    test('bootstrap runs after .triviaiox-core/ copy (injection order)', () => {
      const triviaioxCoreIdx = wizardSource.indexOf('TRIVIAIOX core installed');
      ...
      // Bootstrap must come after triviaiox-core install
      expect(bootstrapIdx).toBeGreaterThan(triviaioxCoreIdx);
    ...
    test('bootstrap has try/catch — failure does not abort install', () => {
      ...
      expect(wizardSource).toContain("run 'triviaiox doctor' post-install");
    });
  ...
  describe('AC2: Sanity check (relative threshold)', () => {
    test('triviaiox doctor entity-registry check exists', () => {
  ...
  describe('AC5: Post-bootstrap verification', () => {
    ...
    test('triviaiox doctor entity-registry check passes on current registry', async () => {
