...
describe('CLAUDE.md Template v5 (Story INS-4.4)', () => {
  ...
  describe('template has 4 new sections with TRIVIAIOX-MANAGED markers', () => {
    test('template file exists and has TRIVIAIOX markers', () => {
      expect(templateContent).toBeTruthy();
      expect(hasTriviaioxMarkers(templateContent)).toBe(true);
    });

    test('contains framework-boundary section', () => {
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: framework-boundary -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-END: framework-boundary -->');
      expect(templateContent).toContain('## Framework vs Project Boundary');
    });

    test('contains rules-system section', () => {
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: rules-system -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-END: rules-system -->');
      expect(templateContent).toContain('## Rules System');
    });

    test('contains code-intelligence section', () => {
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: code-intelligence -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-END: code-intelligence -->');
      expect(templateContent).toContain('## Code Intelligence');
    });

    test('contains graph-dashboard section', () => {
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: graph-dashboard -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-END: graph-dashboard -->');
      expect(templateContent).toContain('## Graph Dashboard');
    });

    test('template has exactly 11 TRIVIAIOX-MANAGED sections total', () => {
      const startMatches = templateContent.match(/<!-- TRIVIAIOX-MANAGED-START:/g);
      const endMatches = templateContent.match(/<!-- TRIVIAIOX-MANAGED-END:/g);
  ...
  describe('section content quality', () => {
    ...
    test('graph-dashboard has CLI commands', () => {
      expect(templateContent).toContain('triviaiox graph --deps');
      ...
      expect(templateContent).toContain('triviaiox graph --stats');
    });
  });

  describe('existing sections preserved', () => {
    test('original 5 TRIVIAIOX-MANAGED sections still present', () => {
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: core-framework -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: agent-system -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: framework-structure -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: triviaiox-patterns -->');
      expect(templateContent).toContain('<!-- TRIVIAIOX-MANAGED-START: common-commands -->');
    });
  ...
  describe('upgrade: new sections added to existing CLAUDE.md', () => {
    ...
    test('upgrade adds 4 new sections to CLAUDE.md without them', async () => {
      const existingClaudeMd = `# TriviaGrowth Triviaiox
...
My custom testing rules`;
      ...
      expect(result.content).toContain('<!-- TRIVIAIOX-MANAGED-START: framework-boundary -->');
    });
  });

  describe('section order in template', () => {
    test('4 new sections come after framework-structure and before workflow-execution', () => {
      const frameworkStructureEnd = templateContent.indexOf('<!-- TRIVIAIOX-MANAGED-END: framework-structure -->');
      const frameworkBoundaryStart = templateContent.indexOf('<!-- TRIVIAIOX-MANAGED-START: framework-boundary -->');
      const graphDashboardEnd = templateContent.indexOf('<!-- TRIVIAIOX-MANAGED-END: graph-dashboard -->');
