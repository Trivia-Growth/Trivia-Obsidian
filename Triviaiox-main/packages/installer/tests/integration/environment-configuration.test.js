...
describe('Environment Configuration Integration', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `triviaiox-env-test-${testId}`);
  ...
  describe('configureEnvironment', () => {
    it('should create .env file with skip prompts', async () => {
      ...
      // Version-agnostic: check TRIVIAIOX_VERSION exists with valid semver format
      expect(content).toMatch(/TRIVIAIOX_VERSION=\d+\.\d+\.\d+/);
    });
    ...
    it('should create core-config.yaml', async () => {
      ...
      const configPath = path.join(testDir, '.triviaiox-core', 'core-config.yaml');
  ...
  describe('File Structure', () => {
    ...
    it('should create .triviaiox-core directory if missing', async () => {
      const triviaioxcoreDir = path.join(testDir, '.triviaiox-core');
      expect(await fs.pathExists(triviaioxcoreDir)).toBe(false);
      ...
      expect(await fs.pathExists(triviaioxcoreDir)).toBe(true);
    });
  });
});
