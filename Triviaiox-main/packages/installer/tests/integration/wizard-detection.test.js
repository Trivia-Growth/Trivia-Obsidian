...
describe('Wizard Integration with Project Type Detection', () => {
  ...
  describe('GREENFIELD Detection Flow', () => {
    test('wizard correctly detects and processes GREENFIELD project', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        ...
        // No markers exist
        if (checkPath.includes('.triviaiox-core')) return false;
  ...
  describe('BROWNFIELD Detection Flow', () => {
    test('wizard correctly detects and processes BROWNFIELD project with package.json', async () => {
      ...
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath === targetPath) return true;
        if (checkPath.includes('.triviaiox-core')) return false; // No TRIVIAIOX
  ...
  // ============================================================================
  // Task 1.3.5.4: Test full wizard flow with EXISTING_TRIVIAIOX detection
  // ============================================================================
  describe('EXISTING_TRIVIAIOX Detection Flow', () => {
    test('wizard correctly detects and processes EXISTING_TRIVIAIOX installation', async () => {
      // Setup: Directory with .triviaiox-core
      fs.existsSync.mockImplementation((checkPath) => {
      ...
      fs.readdirSync.mockReturnValue(['.triviaiox-core', 'package.json', '.git']);

      const result = await runWizard({ targetDir: '/test-existing' });

      expect(result.projectType).toBe('EXISTING_TRIVIAIOX');
      expect(result.targetDir).toBe('/test-existing');
      // Output shows "brownfield" since EXISTING_TRIVIAIOX is treated as brownfield update
      expect(console.log).toHaveBeenCalledWith(expect.stringContaining('brownfield'));
    });

    test('confirmProjectType shows update/reinstall message for EXISTING_TRIVIAIOX', async () => {
      const confirmed = await confirmProjectType('EXISTING_TRIVIAIOX');

      expect(confirmed).toBe('EXISTING_TRIVIAIOX');
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('update or reinstall'),
      );
    });

    test('getProjectType helper returns EXISTING_TRIVIAIOX when .triviaiox-core exists', () => {
      fs.existsSync.mockImplementation((checkPath) => {
        if (checkPath.includes('existing')) return true;
        if (checkPath.endsWith('.triviaiox-core')) return true;
        return false;
      });
      fs.readdirSync.mockReturnValue(['.triviaiox-core']);

      const type = getProjectType('/test/existing');

      expect(type).toBe('EXISTING_TRIVIAIOX');
    });
  });
  ...
  describe('User Override Functionality', () => {
    ...
    test('confirmProjectType shows appropriate message for each type', async () => {
      const types = ['GREENFIELD', 'BROWNFIELD', 'EXISTING_TRIVIAIOX', 'UNKNOWN'];
  ...
  describe('Default Wizard Behavior', () => {
    ...
    test('wizard displays welcome message', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Welcome to TRIVIAIOX Installer'),
      );
    });
