...
const { installTriviaioxCore, hasPackageJson } = require('../../packages/installer/src/installer/triviaiox-core-installer');
...
jest.mock('../../packages/installer/src/installer/triviaiox-core-installer');
...
describe('Wizard Integration - Story 1.7', () => {
  ...
  beforeEach(() => {
    ...
    // Mock TRIVIAIOX core installer
    installTriviaioxCore.mockResolvedValue({
      success: true,
  ...
  describe('Full Wizard Flow (AC Integration)', () => {
    ...
    it('should install TRIVIAIOX core before IDE configs', async () => {
      await runWizard();

      // Verify TRIVIAIOX core was installed
      expect(installTriviaioxCore).toHaveBeenCalled();

      // Verify order: TRIVIAIOX core before IDE configs
      const triviaioxCoreCallOrder = installTriviaioxCore.mock.invocationCallOrder[0];
      const ideConfigCallOrder = generateIDEConfigs.mock.invocationCallOrder[0];

      expect(triviaioxCoreCallOrder).toBeLessThan(ideConfigCallOrder);
    });
  ...
  describe('Wizard State Flow', () => {
    it('should maintain correct state through all steps', async () => {
      ...
      expect(answers.triviaioxCoreInstalled).toBeDefined(); // Story 1.4 - TRIVIAIOX core
    });
    ...
    it('should handle TRIVIAIOX core installation failure gracefully', async () => {
      installTriviaioxCore.mockRejectedValue(new Error('TRIVIAIOX core installation failed'));

      const answers = await runWizard();

      expect(answers.triviaioxCoreInstalled).toBe(false);
