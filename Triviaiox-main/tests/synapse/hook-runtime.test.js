...
const {
  resolveHookRuntime,
  buildHookOutput,
} = require('../../.triviaiox-core/core/synapse/runtime/hook-runtime');
...
describe('hook-runtime', () => {
  ...
  it('calls cleanStaleSessions on first prompt (prompt_count === 0)', () => {
    ...
    try {
      ...
      const smPath = path.join(cwd, '.triviaiox-core', 'core', 'synapse', 'session', 'session-manager.js');
      const engPath = path.join(cwd, '.triviaiox-core', 'core', 'synapse', 'engine.js');
