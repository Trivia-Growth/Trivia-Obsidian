...
const {
  ...
  formatNotFoundError,
  wrapText,
} = require('../../.triviaiox-core/cli/commands/workers/formatters/info-formatter');
const { findSuggestions, findRelatedWorkers } = require('../../.triviaiox-core/cli/commands/workers/info');
const { levenshteinDistance } = require('../../.triviaiox-core/cli/commands/workers/search-keyword');
...
describe('Info Formatter - Pretty Output', () => {
  ...
  test('formatInfoPretty includes usage example', () => {
    ...
    expect(output).toContain('triviaiox task run json-csv-transformer');
  });
...
describe('Not Found Error Formatter', () => {
  ...
  test('formatNotFoundError includes search hint', () => {
    ...
    expect(output).toContain('triviaiox workers search invalid');
  });
});
