...
describe('IDE Config Generator', () => {
  ...
  describe('generateTemplateVariables', () => {
    it('should generate default variables', () => {
      ...
      expect(variables).toHaveProperty('triviaioxVersion');
    });
    ...
    it('should include TRIVIAIOX version', () => {
      const wizardState = {};
      const variables = generateTemplateVariables(wizardState);

      expect(variables.triviaioxVersion).toBeDefined();
      expect(typeof variables.triviaioxVersion).toBe('string');
    });
  });
  ...
  describe('generateIDEConfigs', () => {
    ...
    it('should create config files for multiple IDEs', async () => {
      ...
      expect(await fs.pathExists(path.join(testDir, '.gemini', 'rules', 'TRIVIAIOX', 'agents'))).toBe(true);
    });
    ...
    it('should render template with variables', async () => {
      ...
      // v2.1 templates use static content from .triviaiox-core/templates/ide-rules/
      // They contain TriviaGrowth Triviaiox standard rules
      expect(content).toContain('TriviaGrowth Triviaiox');
    ...
    it('should create github-copilot config in .github directory', async () => {
      ...
      const content = await fs.readFile(configPath, 'utf8');
      // Should contain TRIVIAIOX rules content
      expect(content).toContain('TriviaGrowth Triviaiox');
    });
    ...
    it('should configure Gemini hooks and settings with active TRIVIAIOX hooks', async () => {
      ...
      const beforeAgentWrapper = settings.hooks.BeforeAgent.find(
        (w) => Array.isArray(w.hooks) && w.hooks.some((h) => h.name === 'triviaiox-context-inject'),
      );
      expect(beforeAgentWrapper).toBeDefined();
      const hook = beforeAgentWrapper.hooks.find((h) => h.name === 'triviaiox-context-inject');
