...
const { syncSkills } = require('../../.triviaiox-core/infrastructure/scripts/codex-skills-sync/index');
const { validateCodexSkills } = require('../../.triviaiox-core/infrastructure/scripts/codex-skills-sync/validate');
...
describe('Codex Skills Validator', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-codex-validate-'));
  ...
  it('fails when a generated skill is missing', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    fs.rmSync(path.join(skillsDir, 'triviaiox-architect', 'SKILL.md'), { force: true });
  ...
  it('fails when greeting command is removed from a skill', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const target = path.join(skillsDir, 'triviaiox-dev', 'SKILL.md');
  ...
  it('fails in strict mode when orphaned triviaiox-* skill dir exists', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const orphanPath = path.join(skillsDir, 'triviaiox-legacy');
    ...
    expect(result.ok).toBe(false);
    expect(result.orphaned).toContain('triviaiox-legacy');
  });
});
