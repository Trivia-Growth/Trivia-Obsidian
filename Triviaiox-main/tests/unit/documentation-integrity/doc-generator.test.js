...
const {
  ...
  TemplateFiles,
  OutputFiles,
} = require('../../../.triviaiox-core/infrastructure/scripts/documentation-integrity/doc-generator');

describe('Documentation Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-docgen-test-'));
  });
  ...
  describe('renderTemplate', () => {
    ...
    it('should handle multiple variables', () => {
      ...
      const context = { greeting: 'Hello', name: 'User', place: 'TRIVIAIOX' };

      const result = renderTemplate(template, context);

      expect(result).toBe('Hello User, welcome to TRIVIAIOX!');
    });
