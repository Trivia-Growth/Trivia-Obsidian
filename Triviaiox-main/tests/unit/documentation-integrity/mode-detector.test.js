...
const {
  ...
  InstallationMode,
  LegacyProjectType,
} = require('../../../.triviaiox-core/infrastructure/scripts/documentation-integrity/mode-detector');
...
describe('Mode Detector', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-test-'));
  });
  ...
  describe('detectInstallationMode', () => {
    ...
    it('should detect FRAMEWORK_DEV mode for triviaiox-core repository', () => {
      // Create .triviaiox-core directory
      fs.mkdirSync(path.join(tempDir, '.triviaiox-core'));

      // Create package.json with triviaiox-core name
      fs.writeFileSync(
      ...
      expect(result.legacyType).toBe(LegacyProjectType.EXISTING_TRIVIAIOX);
      expect(result.confidence).toBe(100);
      expect(result.markers.isTriviaioxCoreRepo).toBe(true);
    });

    it('should detect BROWNFIELD mode for user project with existing TRIVIAIOX', () => {
      // Create .triviaiox-core directory
      fs.mkdirSync(path.join(tempDir, '.triviaiox-core'));
  ...
  describe('isTriviaioxCoreRepository', () => {
    it('should return true for @triviaiox/core package', () => {
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: '@triviaiox/core' }),
      );

      expect(isTriviaioxCoreRepository(tempDir)).toBe(true);
    });
    ...
    it('should return false for generic monorepo with workspaces pattern only', () => {
      // Generic monorepos should NOT be detected as triviaiox-core
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );

      expect(isTriviaioxCoreRepository(tempDir)).toBe(false);
    });

    it('should return true for workspaces pattern with triviaiox marker', () => {
      // Workspaces + .triviaiox-core/infrastructure marker = triviaiox-core repo
      fs.writeFileSync(
        path.join(tempDir, 'package.json'),
        JSON.stringify({ name: 'something', workspaces: ['packages/*'] }),
      );
      fs.mkdirSync(path.join(tempDir, '.triviaiox-core', 'infrastructure'), { recursive: true });
  ...
  describe('mapLegacyTypeToMode', () => {
    it('should map EXISTING_TRIVIAIOX to BROWNFIELD by default (safer)', () => {
      // Without context, EXISTING_TRIVIAIOX defaults to BROWNFIELD (won't skip project setup)
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_TRIVIAIOX)).toBe(
        InstallationMode.BROWNFIELD,
      );
    });

    it('should map EXISTING_TRIVIAIOX to FRAMEWORK_DEV with isTriviaioxCoreRepo context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_TRIVIAIOX, { isTriviaioxCoreRepo: true })).toBe(
        InstallationMode.FRAMEWORK_DEV,
      );
    });

    it('should map EXISTING_TRIVIAIOX to BROWNFIELD with non-triviaiox-core context', () => {
      expect(mapLegacyTypeToMode(LegacyProjectType.EXISTING_TRIVIAIOX, { isTriviaioxCoreRepo: false })).toBe(
        InstallationMode.BROWNFIELD,
      );
    });
  ...
  describe('validateModeSelection', () => {
    ...
    it('should warn when selecting framework-dev for non-triviaiox-core repo', () => {
      const detected = {
        mode: InstallationMode.BROWNFIELD,
        markers: { isTriviaioxCoreRepo: false },
      };

      const result = validateModeSelection(InstallationMode.FRAMEWORK_DEV, detected);

      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain('triviaiox-core repository');
    });
  ...
  describe('LegacyProjectType enum', () => {
    it('should have all legacy types', () => {
      expect(LegacyProjectType.EXISTING_TRIVIAIOX).toBe('EXISTING_TRIVIAIOX');
