...
const {
  ...
  GitignoreTemplates,
  TechStack,
} = require('../../../.triviaiox-core/infrastructure/scripts/documentation-integrity/gitignore-generator');

describe('Gitignore Generator', () => {
  let tempDir;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-gitignore-test-'));
  });
  ...
  describe('loadGitignoreTemplate', () => {
    it('should load TRIVIAIOX base template', () => {
      const template = loadGitignoreTemplate(GitignoreTemplates.TRIVIAIOX_BASE);

      expect(template).toContain('.triviaiox-core/local/');
    ...
    it('should load brownfield merge template', () => {
      ...
      expect(template).toContain('TRIVIAIOX Integration Section');
    });
  ...
  describe('getTemplatesForStacks', () => {
    it('should always include TRIVIAIOX base', () => {
      const templates = getTemplatesForStacks([]);

      expect(templates).toContain(GitignoreTemplates.TRIVIAIOX_BASE);
    });
  ...
  describe('generateGitignore', () => {
    it('should generate gitignore for Node.js project', () => {
      ...
      expect(content).toContain('.triviaiox-core/local/');
    });
  ...
  describe('mergeGitignore', () => {
    it('should append TRIVIAIOX section to existing content', () => {
      const existing = '# My project\nnode_modules/\n';
      const merged = mergeGitignore(existing);

      expect(merged).toContain('# My project');
      expect(merged).toContain('node_modules/');
      expect(merged).toContain('TRIVIAIOX Integration Section');
    });

    it('should skip if TRIVIAIOX section already exists', () => {
      const existing = '# TRIVIAIOX Integration Section\n.triviaiox-core/\n';
      const merged = mergeGitignore(existing);
  ...
  describe('generateGitignoreFile', () => {
    ...
    it('should merge with existing .gitignore', () => {
      ...
      expect(result.content).toContain('TRIVIAIOX Integration Section');
    });
  ...
  describe('hasTriviaioxIntegration', () => {
    it('should return false for empty directory', () => {
      expect(hasTriviaioxIntegration(tempDir)).toBe(false);
    });

    it('should return true if TRIVIAIOX section exists', () => {
      fs.writeFileSync(
        path.join(tempDir, '.gitignore'),
        '# TRIVIAIOX Integration Section\n.triviaiox-core/\n',
      );

      expect(hasTriviaioxIntegration(tempDir)).toBe(true);
    });

    it('should return true if .triviaiox-core/ pattern exists', () => {
      fs.writeFileSync(path.join(tempDir, '.gitignore'), '.triviaiox-core/local/\n');
  ...
  describe('parseGitignore', () => {
    ...
    it('should identify TRIVIAIOX section', () => {
      const content = `# Header
node_modules/
# TRIVIAIOX Integration Section
.triviaiox-core/
...
`;
      ...
      expect(parsed.triviaioxSection).toBeDefined();
      expect(parsed.triviaioxSection.start).toBeGreaterThan(0);
    });

    it('should return null triviaioxSection when not present', () => {
      const content = 'node_modules/\n';
      const parsed = parseGitignore(content);

      expect(parsed.triviaioxSection).toBeNull();
    });
  });

  describe('GitignoreTemplates enum', () => {
    it('should have all required templates', () => {
      expect(GitignoreTemplates.TRIVIAIOX_BASE).toBe('gitignore-triviaiox-base.tmpl');
