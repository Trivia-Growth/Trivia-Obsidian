...
const VALIDATOR_PATH = path.join(PROJECT_ROOT, 'scripts', 'validate-triviaiox-core-deps.js');

describe('validate-triviaiox-core-deps.js (INS-4.12)', () => {
  ...
  test('.triviaiox-core/package.json includes fast-glob', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fast-glob');
  });

  test('.triviaiox-core/package.json includes fs-extra', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fs-extra');
  });

  test('.triviaiox-core/package.json includes semver', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('semver');
  });

  test('.triviaiox-core/package.json includes ajv', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('ajv');
  });

  test('.triviaiox-core/package.json includes tar', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
describe('validate-publish.js dependency check (INS-4.12)', () => {
  test('validate-publish includes dependency completeness check', () => {
    ...
    expect(content).toContain('validate-triviaiox-core-deps');
  });
});
...
describe('wizard NODE_PATH fix (INS-4.12)', () => {
  test('wizard/index.js sets NODE_PATH for entity registry bootstrap', () => {
    ...
    expect(content).toContain('triviaioxCoreNodeModules');
  });
...
describe('doctor npm-packages check (INS-4.12)', () => {
  test('doctor check validates .triviaiox-core/node_modules/', () => {
    const checkPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'core', 'doctor', 'checks', 'npm-packages.js');
    const content = fs.readFileSync(checkPath, 'utf8');
    expect(content).toContain('triviaioxCoreNodeModules');
    expect(content).toContain('.triviaiox-core');
    expect(content).toContain('INS-4.12');
  });
});
