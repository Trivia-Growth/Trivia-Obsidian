...
const {
  ...
  findLatestBackup,
  listBackups,
} = require('../../.triviaiox-core/cli/commands/migrate/backup');
...
describe('Migration Backup Module', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `triviaiox-backup-test-${testId}`);
  ...
  describe('createBackupDirName', () => {
    it('should create backup directory name with date format', () => {
      ...
      expect(name).toMatch(/^\.triviaiox-backup-\d{4}-\d{2}-\d{2}$/);
    });

    it('should use current date', () => {
      const name = createBackupDirName();
      const today = new Date().toISOString().split('T')[0];
      expect(name).toBe(`.triviaiox-backup-${today}`);
    });
  });
  ...
  describe('createBackup', () => {
    it('should create backup of .triviaiox-core directory', async () => {
      // Create mock .triviaiox-core structure
      ...
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'agents', 'test.md'), 'Agent content');
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'index.js'), 'module.exports = {}');
    ...
    it('should fail if .triviaiox-core does not exist', async () => {
      await expect(createBackup(testDir)).rejects.toThrow(/No .triviaiox-core directory/);
    });

    it('should include backup manifest', async () => {
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      await fs.promises.mkdir(triviaioxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'test.js'), 'test');
  ...
  describe('verifyBackup', () => {
    it('should verify valid backup', async () => {
      ...
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      await fs.promises.mkdir(triviaioxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'test.js'), 'test content');
    ...
    it('should detect corrupted files', async () => {
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      await fs.promises.mkdir(triviaioxCoreDir, { recursive: true });
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'test.js'), 'original');

      const backupResult = await createBackup(testDir);

      // Corrupt a file in backup
      const backedUpFile = path.join(backupResult.backupDir, '.triviaiox-core', 'test.js');
  ...
  describe('findLatestBackup', () => {
    it('should find the most recent backup', async () => {
      ...
      expect(latest.name).toMatch(/^\.triviaiox-backup-/);
    });
