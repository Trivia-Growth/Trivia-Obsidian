...
const {
  ...
  SquadMigratorError,
  MigratorErrorCodes,
} = require('../../../.triviaiox-core/development/scripts/squad');
...
describe('SquadMigrator', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should detect missing triviaiox.type field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);

      expect(analysis.needsMigration).toBe(true);
      expect(analysis.issues.some((i) => i.type === 'MISSING_TRIVIAIOX_TYPE')).toBe(true);
      expect(analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'triviaiox.type')).toBe(
        true,
      );
    });

    it('should detect missing triviaiox.minVersion field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);
      ...
      expect(
        analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'triviaiox.minVersion'),
      ).toBe(true);
    });
    ...
    it('should return needsMigration: false for up-to-date squad', async () => {
      ...
      const manifest = {
        triviaiox: { type: 'squad', minVersion: '2.1.0' },
  ...
  describe('migrate()', () => {
    ...
    it('should add missing triviaiox.type and triviaiox.minVersion fields (AC 5)', async () => {
      // Copy legacy squad to temp dir
      const srcPath = path.join(FIXTURES_PATH, 'legacy-squad');
      ...
      expect(manifest.triviaiox?.type).toBe('squad');
      expect(manifest.triviaiox?.minVersion).toBe('2.1.0');
    });
