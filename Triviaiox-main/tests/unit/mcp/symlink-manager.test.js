...
jest.mock('../../../.triviaiox-core/core/mcp/os-detector', () => ({
  isWindows: jest.fn(),
...
const {
  ...
  createLink,
  removeLink,
} = require('../../../.triviaiox-core/core/mcp/symlink-manager');

const osDetector = require('../../../.triviaiox-core/core/mcp/os-detector');

describe('Symlink Manager', () => {
  const mockProjectRoot = '/mock/project';
  const mockGlobalMcpDir = '/mock/home/.triviaiox/mcp';
  const mockLinkPath = path.join(mockProjectRoot, '.triviaiox-core', 'tools', 'mcp');
  ...
  describe('getProjectMcpPath', () => {
    it('should return correct path for project root', () => {
      ...
      expect(result).toBe(path.join(mockProjectRoot, '.triviaiox-core', 'tools', 'mcp'));
    });

    it('should use current directory when no root provided', () => {
      const cwd = process.cwd();
      const result = getProjectMcpPath();
      expect(result).toBe(path.join(cwd, '.triviaiox-core', 'tools', 'mcp'));
    });
  });
