...
const {
  ...
  formatMigrationPlan,
  analyzeImports,
} = require('../../.triviaiox-core/cli/commands/migrate/analyze');

describe('Migration Analyze Module', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `triviaiox-analyze-test-${Date.now()}`);
  ...
  describe('detectV2Structure', () => {
    ...
    it('should detect v2.1 modular structure', async () => {
      ...
      await fs.promises.mkdir(path.join(triviaioxCoreDir, 'product'), { recursive: true });
      await fs.promises.mkdir(path.join(triviaioxCoreDir, 'infrastructure'), { recursive: true });
    ...
    it('should return error if no .triviaiox-core exists', async () => {
      const result = await detectV2Structure(testDir);
  ...
  describe('analyzeMigrationPlan', () => {
    it('should generate migration plan for v2.0 project', async () => {
      ...
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'tasks', 'build.md'), 'Task');
    ...
    it('should detect conflicts', async () => {
      ...
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      await fs.promises.mkdir(path.join(triviaioxCoreDir, 'agents'), { recursive: true });
      await fs.promises.mkdir(path.join(triviaioxCoreDir, 'core'), { recursive: true }); // Conflict
  ...
  describe('analyzeImports', () => {
    it('should analyze importable files', async () => {
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      await fs.promises.mkdir(path.join(triviaioxCoreDir, 'cli'), { recursive: true });
      await fs.promises.writeFile(path.join(triviaioxCoreDir, 'cli', 'index.js'), 'module.exports = {}');
