...
} = require('../../.triviaiox-core/infrastructure/scripts/validate-parity');

describe('validate-parity', () => {
  function createMockProjectRoot() {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-parity-'));
  ...
  it('fails when docs matrix claim diverges from contract', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-parity-mismatch-'));
  ...
  it('generates diff between contract versions', () => {
    ...
    current.release = 'TRIVIAIOX 4.1.0';
  ...
  it('includes contractDiff in parity result when --diff path is provided', () => {
    ...
    const ok = { ok: true, errors: [], warnings: [] };
    const result = runParityValidation(
      { projectRoot, diffPath: '.triviaiox-core/infrastructure/contracts/compatibility/triviaiox-4.0.3.yaml' },
      {
        runSyncValidate: () => ok,
        validateClaudeIntegration: () => ok,
        validateCodexIntegration: () => ok,
        ...
        loadCompatibilityContract: (contractPath) => {
          if (contractPath.endsWith('triviaiox-4.0.3.yaml')) {
            return {
              release: 'TRIVIAIOX 4.0.3',
              global_required_checks: ['paths'],
              ide_matrix: [
    ...
    expect(result.contractDiff.from_release).toBe('TRIVIAIOX 4.0.3');
    expect(result.contractDiff.to_release).toBe('TRIVIAIOX 4.0.4');
