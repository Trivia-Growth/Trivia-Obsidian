# macOS Testing Suite for TRIVIAIOX
**Story 1.10b - macOS Testing & Validation**

Comprehensive test automation framework for validating TRIVIAIOX installation on macOS (Intel and Apple Silicon).
...
2. **Log Files:**
   - Location: `/tmp/triviaiox-test-*.log`
...
```bash
./run-all-tests.sh
# View report at: /tmp/triviaiox-macos-test-report-*.txt
```
...
2. **Test Logs:**
   - Attach all `/tmp/triviaiox-test-*.log` files
