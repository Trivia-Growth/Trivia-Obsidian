...
LOG_FILE="/tmp/triviaiox-test-perms-$(date +%Y%m%d-%H%M%S).log"
...
test_script_permissions() {
    ...
    # Find TRIVIAIOX scripts
    if [[ -d "$HOME/.triviaiox/bin" ]]; then
        SCRIPTS=$(find "$HOME/.triviaiox/bin" -type f -name "*.sh" 2>/dev/null || true)
...
test_config_permissions() {
    ...
    CONFIG_FILES=(
        "$HOME/.triviaiox/config.json"
        "$HOME/.triviaiox/.triviaioxrc"
    )
...
test_directory_permissions() {
    ...
    if [[ -d "$HOME/.triviaiox" ]]; then
        DIR_PERMS=$(stat -f "%Lp" "$HOME/.triviaiox" 2>/dev/null || stat -c "%a" "$HOME/.triviaiox" 2>/dev/null)
...
test_no_sudo_required() {
    log_info "Test 4: Verifying no sudo required for TRIVIAIOX operations..."

    # Check if current user owns .triviaiox directory
    if [[ -d "$HOME/.triviaiox" ]]; then
        OWNER=$(stat -f "%Su" "$HOME/.triviaiox" 2>/dev/null || stat -c "%U" "$HOME/.triviaiox" 2>/dev/null)

        if [[ "$OWNER" == "$(whoami)" ]]; then
            pass_test "User owns .triviaiox directory (no sudo needed)"
        else
            fail_test ".triviaiox owned by: $OWNER (expected: $(whoami))"
        fi
    fi
}
