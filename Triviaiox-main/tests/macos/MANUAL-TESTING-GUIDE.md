...
This guide provides step-by-step instructions for manually testing TRIVIAIOX on macOS (Intel and Apple Silicon).
...
```bash
# Backup existing TRIVIAIOX installation (if any)
if [ -d "$HOME/.triviaiox" ]; then
    mv "$HOME/.triviaiox" "$HOME/.triviaiox.backup.$(date +%Y%m%d-%H%M%S)"
fi

# Verify backup
ls -la "$HOME/.triviaiox.backup"*
```

### 2. Navigate to Test Directory

```bash
cd path/to/triviaiox-core/tests/macos
chmod +x *.sh
```
...
```bash
npx @triviagrowthai/triviaiox@latest init
```
...
```bash
# Check TRIVIAIOX command availability
which triviaiox
# Expected: /usr/local/bin/triviaiox or similar
...
cat ~/.zshrc | grep -i triviaiox

# Open new terminal window
# Run: triviaiox --version
...
cat ~/.bashrc | grep -i triviaiox
# Or check ~/.bash_profile

# Run TRIVIAIOX command
triviaiox --version
...
```bash
# Check TRIVIAIOX installation path
which triviaiox
# Should use forward slashes: /usr/local/bin/triviaiox
# NOT backslashes: \usr\local\bin\triviaiox
```
...
cd ~/.triviaiox
pwd
# Should show full path: /Users/username/.triviaiox
```

#### 3. Test Symlinks

```bash
# Check if triviaiox command is a symlink
ls -la $(which triviaiox)
...
```bash
# Check line endings of TRIVIAIOX config
file ~/.triviaiox/config.json
# Should show: ASCII text (LF line endings)
# NOT: ASCII text, with CRLF line endings

# Check for carriage returns
grep -r $'\r' ~/.triviaiox/ || echo "No CRLF found - PASS"
```
...
ls -la ~/.triviaiox/bin/*.sh 2>/dev/null
# Should show: -rwxr-xr-x (executable)

# Try running a script without chmod
~/.triviaiox/bin/some-script.sh
...
time npx @triviagrowthai/triviaiox@latest init
...
- [ ] No "unidentified developer" warnings for TRIVIAIOX
...
# Simulate corruption
echo "corrupted" > ~/.triviaiox/config.json

# Try to recover
mv ~/.triviaiox ~/.triviaiox.broken
mv ~/.triviaiox.backup ~/.triviaiox
...
- Permission denied: `chmod 000 ~/.triviaiox`
...
npx @triviagrowthai/triviaiox@latest init --verbose
```

#### Command Not Found

**Symptom:** `bash: triviaiox: command not found`
...
# Check PATH
echo $PATH | grep -o '/[^:]*triviaiox[^:]*'

# Manually add to PATH if needed
export PATH="$HOME/.triviaiox/bin:$PATH"
```
...
chmod +x ~/.triviaiox/bin/*.sh

# Fix ownership
sudo chown -R $(whoami) ~/.triviaiox
...
**Symptom:** `triviaiox health` shows unhealthy MCPs

**Solution:**

```bash
# Reinstall specific MCP
triviaiox mcp reinstall <mcp-name>
...
2. **Test Logs:**
   - All files from `/tmp/triviaiox-test-*.log`
