...
const {
  ...
  syncGeminiCommands,
} = require('../../.triviaiox-core/infrastructure/scripts/ide-sync/gemini-commands');
...
describe('gemini command sync', () => {
  ...
  it('normalizes command slugs and menu names', () => {
    expect(commandSlugForAgent('triviaiox-master')).toBe('master');
    expect(commandSlugForAgent('dev')).toBe('dev');
    expect(menuCommandName('triviaiox-master')).toBe('/triviaiox-master');
    expect(menuCommandName('dev')).toBe('/triviaiox-dev');
  });
  ...
  it('builds menu + one command per agent', () => {
    ...
    expect(files.find((f) => f.filename === 'triviaiox-menu.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'triviaiox-dev.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'triviaiox-architect.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'triviaiox-qa.toml')).toBeDefined();
  ...
  it('derives command description from agent title and whenToUse', () => {
    ...
    const dev = files.find((f) => f.filename === 'triviaiox-dev.toml');
  ...
  it('falls back to generic description when metadata is missing', () => {
    ...
    const dev = files.find((f) => f.filename === 'triviaiox-dev.toml');
    expect(dev.content).toContain('description = "Ativar agente TRIVIAIOX dev"');
  });
  ...
  it('writes command files to .gemini/commands', () => {
    ...
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'triviaiox-menu.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'triviaiox-dev.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'triviaiox-qa.toml'))).toBe(true);
  });
});
