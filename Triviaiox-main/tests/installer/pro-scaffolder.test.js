...
beforeEach(async () => {
  ...
  await fs.writeJson(
    ...
    { name: '@triviaiox-fullstack/pro', version: '2.0.0' }
  );
});
...
describe('scaffoldProContent', () => {
  ...
  it('should copy all pro content to project (AC1, AC2, AC3)', async () => {
    ...
    // AC2: pro-config.yaml exists in .triviaiox-core/
    expect(await fs.pathExists(
      path.join(targetDir, '.triviaiox-core', 'pro-config.yaml')
    )).toBe(true);

    // AC3: feature-registry.yaml exists in .triviaiox-core/
    expect(await fs.pathExists(
      path.join(targetDir, '.triviaiox-core', 'feature-registry.yaml')
    )).toBe(true);
  });
  ...
  it('should be idempotent: 2nd run skips identical files (AC5)', async () => {
    ...
    const configContent = yaml.load(
      await fs.readFile(path.join(targetDir, '.triviaiox-core', 'pro-config.yaml'), 'utf8')
    );
