/**
 * TRIVIAIOX Core Installer Tests
 *
...
const {
  generateFileHashes,
  generateVersionJson,
} = require('../../packages/installer/src/installer/triviaiox-core-installer');

describe('TRIVIAIOX Core Installer - Version Tracking', () => {
  let tempDir;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'triviaiox-installer-test-'));
    await fs.ensureDir(path.join(tempDir, '.triviaiox-core'));
  });
  ...
  describe('generateFileHashes', () => {
    it('should generate hashes for installed files', async () => {
      const triviaioxCoreDir = path.join(tempDir, '.triviaiox-core');

      // Create test files
      await fs.writeFile(path.join(triviaioxCoreDir, 'test1.md'), '# Test File 1');
      await fs.writeFile(path.join(triviaioxCoreDir, 'test2.md'), '# Test File 2');
      await fs.ensureDir(path.join(triviaioxCoreDir, 'agents'));
      await fs.writeFile(path.join(triviaioxCoreDir, 'agents', 'dev.md'), '# Dev Agent');

      const installedFiles = ['test1.md', 'test2.md', 'agents/dev.md'];
      const hashes = await generateFileHashes(triviaioxCoreDir, installedFiles);
    ...
    it('should skip non-existent files', async () => {
      ...
      await fs.writeFile(path.join(triviaioxCoreDir, 'exists.md'), '# Exists');
    ...
    it('should skip directories', async () => {
      ...
      await fs.ensureDir(path.join(triviaioxCoreDir, 'agents'));
      await fs.writeFile(path.join(triviaioxCoreDir, 'file.md'), '# File');
    ...
    it('should generate consistent hashes for same content', async () => {
      ...
      await fs.writeFile(path.join(triviaioxCoreDir, 'file1.md'), 'Same content');
      await fs.writeFile(path.join(triviaioxCoreDir, 'file2.md'), 'Same content');
    ...
    it('should generate different hashes for different content', async () => {
      ...
      await fs.writeFile(path.join(triviaioxCoreDir, 'file1.md'), 'Content A');
      await fs.writeFile(path.join(triviaioxCoreDir, 'file2.md'), 'Content B');
  ...
  describe('generateVersionJson', () => {
    it('should create version.json with correct structure', async () => {
      ...
      // Create test files
      await fs.writeFile(path.join(triviaioxCoreDir, 'test.md'), '# Test');

      const result = await generateVersionJson({
        targetTriviaioxCore: triviaioxCoreDir,
        version: '1.2.0',
    ...
    it('should write version.json to disk', async () => {
      ...
      await fs.writeFile(path.join(triviaioxCoreDir, 'agent.md'), '# Agent');
      ...
      const versionJsonPath = path.join(triviaioxCoreDir, 'version.json');
    ...
    it('should include file hashes in version.json', async () => {
      ...
      await fs.ensureDir(path.join(triviaioxCoreDir, 'agents'));
      await fs.writeFile(path.join(triviaioxCoreDir, 'agents', 'dev.md'), '# Dev');
      await fs.writeFile(path.join(triviaioxCoreDir, 'config.yaml'), 'key: value');
