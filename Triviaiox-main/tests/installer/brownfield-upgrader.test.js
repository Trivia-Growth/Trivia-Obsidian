...
describe('brownfield-upgrader', () => {
  ...
  describe('generateUpgradeReport', () => {
    ...
    it('should identify modified files', () => {
      const triviaioxCoreDir = path.join(targetDir, '.triviaiox-core');
      fs.ensureDirSync(triviaioxCoreDir);
      fs.writeFileSync(path.join(triviaioxCoreDir, 'changed.md'), 'original');
      const originalHash = `sha256:${hashFile(path.join(triviaioxCoreDir, 'changed.md'))}`;
    ...
    it('should identify user-modified files', () => {
      const triviaioxCoreDir = path.join(targetDir, '.triviaiox-core');
      fs.ensureDirSync(triviaioxCoreDir);
      fs.writeFileSync(path.join(triviaioxCoreDir, 'user-changed.md'), 'user modified content');
  ...
  describe('applyUpgrade', () => {
    ...
    it('should install new files', async () => {
      ...
      expect(fs.existsSync(path.join(targetDir, '.triviaiox-core', 'new-file.md'))).toBe(true);
    });
    ...
    it('should perform dry run without modifying files', async () => {
      ...
      expect(result.filesInstalled).toHaveLength(1);
      expect(fs.existsSync(path.join(targetDir, '.triviaiox-core', 'new-file.md'))).toBe(false);
    });
  });
  ...
  describe('updateInstalledManifest', () => {
    it('should create installed manifest file', () => {
      ...
      fs.ensureDirSync(path.join(targetDir, '.triviaiox-core'));
      updateInstalledManifest(targetDir, sourceManifest, 'triviaiox-core@2.1.0');

      const installedPath = path.join(targetDir, '.triviaiox-core', '.installed-manifest.yaml');
      expect(fs.existsSync(installedPath)).toBe(true);

      const content = yaml.load(fs.readFileSync(installedPath, 'utf8'));
      expect(content.installed_version).toBe('2.1.0');
      expect(content.installed_from).toBe('triviaiox-core@2.1.0');
    });
  });
