...
const PKG_DIR = path.resolve(__dirname, '../../../packages/triviaiox-install');
...
describe('Integration - Task 8.3: Local NPX Execution', () => {
  ...
  describe('Package Structure Validation', () => {
    it('should have valid package.json', () => {
      ...
      // Then
      expect(pkgJson.name).toBe('@trivia-growth/triviaiox-install');
      expect(pkgJson.bin).toBeDefined();
      expect(pkgJson.bin['triviaiox-install']).toBe('./bin/triviaiox-install.js');
    ...
    it('should have executable shebang in bin files', () => {
      // Given
      const binFiles = ['bin/triviaiox-install.js', 'bin/edmcp.js'];
  ...
  describe('CLI Execution Tests', () => {
    it('should execute triviaiox-install --version via node', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/triviaiox-install.js');
    ...
    it('should show help for triviaiox-install', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/triviaiox-install.js');
      ...
      // Then
      expect(result).toContain('triviaiox-install');
  ...
  (runNpxIntegration ? describe : describe.skip)('NPX Local Execution Simulation', () => {
    it('should execute via npm exec (simulates npx .)', () => {
      ...
      // When
      const result = execSync('npm exec -- triviaiox-install --version', {
        cwd: PKG_DIR,
        encoding: 'utf8',
        timeout: 90000,
