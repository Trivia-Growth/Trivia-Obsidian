...
describe('license-cache', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-license-test-'));
  });
  ...
  describe('writeLicenseCache', () => {
    ...
    it('should create .triviaiox directory if not exists', () => {
      const data = createTestCacheData();
      const triviaioxDir = getTriviaioxDir(testDir);

      expect(fs.existsSync(triviaioxDir)).toBe(false);

      writeLicenseCache(data, testDir);

      expect(fs.existsSync(triviaioxDir)).toBe(true);
    });
  ...
  describe('readLicenseCache', () => {
    ...
    it('should return null for corrupted JSON', () => {
      ...
      fs.mkdirSync(getTriviaioxDir(testDir), { recursive: true });
  ...
  describe('Pending Deactivation (AC-7b)', () => {
    ...
    describe('markPendingDeactivationSynced', () => {
      ...
      it('should add syncedAt timestamp', () => {
        ...
        const filePath = path.join(getTriviaioxDir(testDir), 'pending-deactivation.json');
