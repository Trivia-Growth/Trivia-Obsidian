...
const { parseArgs, handleWatch, WATCH_FORMAT_MAP } = require('../../.triviaiox-core/core/graph-dashboard/cli');

describe('watch-mode', () => {
  let consoleLogSpy;
  let consoleErrorSpy;
  let watchState;
  const testOutputDir = path.resolve(process.cwd(), '.triviaiox');
  ...
  describe('handleWatch lifecycle', () => {
    ...
    it('should create .triviaiox directory if it does not exist', async () => {
      // .triviaiox should be created by handleWatch
      watchState = await handleWatch({ format: 'dot', interval: 5 });
