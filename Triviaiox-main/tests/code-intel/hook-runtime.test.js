...
const { resolveCodeIntel, formatAsXml, escapeXml, _resetForTesting } = require(
  path.join(__dirname, '..', '..', '.triviaiox-core', 'core', 'code-intel', 'hook-runtime.js'),
);
...
describe('code-intel hook-runtime', () => {
  ...
  describe('resolveCodeIntel', () => {
    ...
    it('handles absolute paths by normalizing to relative', async () => {
      const absPath = path.join(PROJECT_ROOT, '.triviaiox-core', 'development', 'tasks', 'create-next-story.md');
  ...
  describe('formatAsXml', () => {
    ...
    it('generates valid XML with entity data', () => {
      const intel = {
        entity: {
          file: '.triviaiox-core/core/some-module.js',
      ...
      expect(xml).toContain('<path>.triviaiox-core/core/some-module.js</path>');
