/**
 * CLI tests for `triviaiox config` subcommands
...
const { createConfigCommand } = require('../../.triviaiox-core/cli/commands/config');
...
function createTempProject(files = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-cli-test-'));
...
describe('config CLI commands', () => {
  ...
  /**
   * Run `triviaiox config <subArgs>` in-process via Commander.
  ...
  async function runConfigCmd(subArgs) {
    ...
    let exitCode = 0;
    try {
      await program.parseAsync(['node', 'triviaiox', ...subArgs]);
    } catch (err) {
  ...
  describe('triviaiox config show', () => {
    test('shows resolved config as YAML', async () => {
      ...
      try {
        ...
        expect(stdout).toContain('TRIVIAIOX-FullStack');
      } finally {
  ...
  describe('triviaiox config validate', () => {
  ...
  describe('triviaiox config diff', () => {
  ...
  describe('triviaiox config migrate', () => {
    const LEGACY_CONFIG = [
      'project:',
      '  name: "test-project"',
      '  version: "1.0.0"',
      ...
      'mcp:',
      '  enabled: false',
      'toolsLocation: .triviaiox-core/tools',
      'lazyLoading:',
      '  enabled: true',
      '',
    ].join('\n');
    ...
    test('--dry-run shows preview without writing files', async () => {
      const tmpDir = createTempProject({
        '.triviaiox-core/core-config.yaml': LEGACY_CONFIG,
      });
      ...
      try {
        ...
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'framework-config.yaml'))).toBe(false);
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'project-config.yaml'))).toBe(false);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
    ...
    test('full migration creates split files and backup', async () => {
      ...
      try {
        ...
        // Split files created
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'framework-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'project-config.yaml'))).toBe(true);
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'local-config.yaml'))).toBe(true);

        // Backup created
        expect(fs.existsSync(path.join(tmpDir, '.triviaiox-core', 'core-config.yaml.backup'))).toBe(true);
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
  ...
  // -----------------------------------------------------------------------
  // triviaiox config validate — error paths
  // -----------------------------------------------------------------------

  describe('triviaiox config validate — error paths', () => {
  ...
  describe('triviaiox config init-local', () => {
    test('creates local-config.yaml from template', async () => {
      const tmpDir = createTempProject({
        '.triviaiox-core/local-config.yaml.template': 'ide:\n  selected:\n    - vscode\n',
      });
      ...
      try {
        ...
        const localConfig = path.join(tmpDir, '.triviaiox-core', 'local-config.yaml');
    ...
    test('warns if local-config.yaml already exists', async () => {
      const tmpDir = createTempProject({
        '.triviaiox-core/local-config.yaml.template': 'ide:\n  selected:\n    - vscode\n',
        '.triviaiox-core/local-config.yaml': 'existing: true\n',
      });
