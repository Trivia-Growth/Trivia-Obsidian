...
const {
  ...
  ConflictSeverity,
  MergeDecision,
} = require('../../.triviaiox-core/core/execution/semantic-merge-engine');
...
describe('SemanticMergeEngine Module', () => {
  ...
  describe('CustomRulesLoader', () => {
    ...
    test('constructor sets paths', () => {
      ...
      expect(loader.rulesPath).toContain('.triviaiox');
    });
  ...
  describe('SemanticMergeEngine', () => {
    ...
    test('saveReport writes JSON and MD files', async () => {
      const engine = new SemanticMergeEngine({
        rootPath: tmpDir,
        storageDir: path.join(tmpDir, '.triviaiox', 'merge'),
      });
      ...
      const mergeDir = path.join(tmpDir, '.triviaiox', 'merge');
