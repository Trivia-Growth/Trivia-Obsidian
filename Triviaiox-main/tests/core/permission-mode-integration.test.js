...
const { PermissionMode } = require('../../.triviaiox-core/core/permissions/permission-mode');
const { OperationGuard } = require('../../.triviaiox-core/core/permissions/operation-guard');
const {
  createGuard,
  checkOperation,
  getModeBadge,
  ...
  cycleMode,
  enforcePermission,
} = require('../../.triviaiox-core/core/permissions');
...
describe('Permission Mode Integration (Story ACT-4)', () => {
  ...
  beforeEach(() => {
    ...
    const triviaioxDir = path.join(tempDir, '.triviaiox');
    fs.mkdirSync(triviaioxDir, { recursive: true });
    configPath = path.join(triviaioxDir, 'config.yaml');
  });
