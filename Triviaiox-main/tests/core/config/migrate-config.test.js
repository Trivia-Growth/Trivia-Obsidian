...
const {
  ...
  USER_FIELDS,
  PROJECT_FIELDS,
} = require('../../../.triviaiox-core/core/config/migrate-config');
...
describe('extractProjectFields', () => {
  ...
  it('should extract project.type from nested project object', () => {
    // Given
    const legacyConfig = {
      project: { type: 'EXISTING_TRIVIAIOX', version: '2.1.0' },
    };
    ...
    // Then
    expect(projectFields.project_type).toBe('EXISTING_TRIVIAIOX');
  });
  ...
  it('should not override explicit project_type with nested project.type', () => {
    ...
    const legacyConfig = {
      project_type: 'fullstack',
      project: { type: 'EXISTING_TRIVIAIOX' },
    };
...
describe('migrateConfig', () => {
  ...
  it('should be idempotent: re-running does not duplicate', () => {
    ...
    const userConfigPath = path.join(FAKE_HOME, '.triviaiox', 'user-config.yaml');
    setupFileSystem({
      'core-config.yaml': LEGACY_CONFIG_YAML,
...
describe('ensureUserConfigDir', () => {
  it('should create ~/.triviaiox/ directory if it does not exist', () => {
    // Given
    fs.existsSync.mockReturnValue(false);
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith(
      expect.stringContaining('.triviaiox'),
