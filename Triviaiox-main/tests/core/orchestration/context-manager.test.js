...
const ContextManager = require('../../../.triviaiox-core/core/orchestration/context-manager');
...
describe('ContextManager', () => {
  ...
  describe('constructor', () => {
    ...
    test('calcula stateDir como .triviaiox/workflow-state dentro do projectRoot', () => {
      expect(manager.stateDir).toBe(path.join(PROJECT_ROOT, '.triviaiox', 'workflow-state'));
    });

    test('calcula statePath com o workflowId no nome do arquivo', () => {
      expect(manager.statePath).toBe(
        path.join(PROJECT_ROOT, '.triviaiox', 'workflow-state', `${WORKFLOW_ID}.json`),
      );
    });

    test('calcula handoffDir dentro do stateDir', () => {
      expect(manager.handoffDir).toBe(
        path.join(PROJECT_ROOT, '.triviaiox', 'workflow-state', 'handoffs'),
      );
    });

    test('calcula confidenceDir dentro do stateDir', () => {
      expect(manager.confidenceDir).toBe(
        path.join(PROJECT_ROOT, '.triviaiox', 'workflow-state', 'confidence'),
      );
    });
  ...
  describe('_resolveConfidenceThreshold', () => {
    const originalEnv = process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD;

    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD = originalEnv;
      } else {
        delete process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD;
      }
    });
    ...
    test('retorna valor numérico da env quando é um número válido', () => {
      process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD = '85';

      expect(manager._resolveConfidenceThreshold()).toBe(85);
    });

    test('retorna 70 quando env contém valor não numérico', () => {
      process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD = 'invalid';

      expect(manager._resolveConfidenceThreshold()).toBe(70);
    });

    test('retorna 0 quando env é string vazia (Number("") === 0)', () => {
      process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD = '';
    ...
    test('aceita valores decimais', () => {
      process.env.TRIVIAIOX_DELIVERY_CONFIDENCE_THRESHOLD = '75.5';
