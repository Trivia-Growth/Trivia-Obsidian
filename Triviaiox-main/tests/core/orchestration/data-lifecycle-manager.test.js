...
const {
  ...
  STALE_SESSION_DAYS,
  STALE_SNAPSHOT_DAYS,
} = require('../../../.triviaiox-core/core/orchestration/data-lifecycle-manager');
...
describe('DataLifecycleManager', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(TEST_PROJECT_ROOT, '.triviaiox/snapshots'), { recursive: true });
  ...
  describe('cleanupStaleSessions', () => {
    ...
    it('should archive session if last_updated > 30 days (AC8)', async () => {
      ...
      const archiveDir = path.join(TEST_PROJECT_ROOT, '.triviaiox/archive/sessions');
  ...
  describe('cleanupStaleSnapshots', () => {
    it('should return 0 when no snapshots directory exists', async () => {
      ...
      await fs.rm(path.join(TEST_PROJECT_ROOT, '.triviaiox/snapshots'), { recursive: true });
    ...
    it('should not remove snapshot if age < 90 days', async () => {
      ...
      const snapshotPath = path.join(TEST_PROJECT_ROOT, '.triviaiox/snapshots/snapshot-recent.json');
    ...
    it('should remove snapshot if age > 90 days and update index.json (AC10)', async () => {
      ...
      const snapshotPath = path.join(TEST_PROJECT_ROOT, '.triviaiox/snapshots/snapshot-old.json');
      await fs.writeFile(snapshotPath, JSON.stringify({ epic_id: 'test', story_id: '1.0' }));
      ...
      const indexPath = path.join(TEST_PROJECT_ROOT, '.triviaiox/snapshots/index.json');
