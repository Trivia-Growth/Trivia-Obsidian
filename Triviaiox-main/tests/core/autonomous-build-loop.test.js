...
jest.mock('../../.triviaiox-core/core/execution/build-state-manager', () => ({
  BuildStateManager: jest.fn().mockImplementation(() => ({ ...mockStateInstance })),
}));

// Mock optional dependencies to not load
jest.mock('../../.triviaiox-core/infrastructure/scripts/recovery-tracker', () => {
  throw new Error('not available');
});
jest.mock('../../.triviaiox-core/infrastructure/scripts/worktree-manager', () => {
  throw new Error('not available');
});
...
const {
  ...
  SubtaskResult,
  DEFAULT_CONFIG,
} = require('../../.triviaiox-core/core/execution/autonomous-build-loop');

const { BuildStateManager } = require('../../.triviaiox-core/core/execution/build-state-manager');
