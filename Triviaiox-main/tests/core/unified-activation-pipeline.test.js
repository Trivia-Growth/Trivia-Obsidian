...
const mockCoreConfig = {
  ...
  dataLocation: '.triviaiox-core/data',
...
jest.mock('../../.triviaiox-core/development/scripts/agent-config-loader', () => ({
  AgentConfigLoader: jest.fn().mockImplementation(() => ({
    loadComplete: jest.fn().mockResolvedValue({
      config: { dataLocation: '.triviaiox-core/data' },
...
jest.mock('../../.triviaiox-core/core/session/context-loader', () => {
...
jest.mock('../../.triviaiox-core/infrastructure/scripts/project-status-loader', () => ({
  loadProjectStatus: jest.fn().mockResolvedValue(mockProjectStatus),
}));

// Mock git config detector
jest.mock('../../.triviaiox-core/infrastructure/scripts/git-config-detector', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.triviaiox-core/core/permissions', () => ({
...
jest.mock('../../.triviaiox-core/core/config/config-resolver', () => ({
...
jest.mock('../../.triviaiox-core/infrastructure/scripts/validate-user-profile', () => ({
...
jest.mock('../../.triviaiox-core/development/scripts/greeting-preference-manager', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.triviaiox-core/core/session/context-detector', () => {
  return jest.fn().mockImplementation(() => ({
...
jest.mock('../../.triviaiox-core/development/scripts/workflow-navigator', () => {
  return jest.fn().mockImplementation(() => ({
...
// Mock config cache
jest.mock('../../.triviaiox-core/core/config/config-cache', () => ({
  globalConfigCache: {
...
jest.mock('../../.triviaiox-core/infrastructure/scripts/performance-tracker', () => ({
  trackConfigLoad: jest.fn(),
}));

// --- Require modules AFTER mocks ---
const { UnifiedActivationPipeline, ALL_AGENT_IDS, LOADER_TIERS, DEFAULT_PIPELINE_TIMEOUT_MS, FALLBACK_PHRASE } = require('../../.triviaiox-core/development/scripts/unified-activation-pipeline');
const { AgentConfigLoader } = require('../../.triviaiox-core/development/scripts/agent-config-loader');
const SessionContextLoader = require('../../.triviaiox-core/core/session/context-loader');
...
const { PermissionMode } = require('../../.triviaiox-core/core/permissions');
...
describe('UnifiedActivationPipeline', () => {
  ...
  describe('static methods', () => {
    it('getAllAgentIds() should return all 12 agent IDs', () => {
      ...
      expect(ids).toContain('triviaiox-master');
    ...
    it('isValidAgentId() should return true for valid IDs', () => {
      ...
      expect(UnifiedActivationPipeline.isValidAgentId('triviaiox-master')).toBe(true);
    });
  ...
  describe('default icon mapping', () => {
    it('should return correct icons for known agents', () => {
      ...
      expect(pipeline._getDefaultIcon('triviaiox-master')).toBe('\uD83D\uDC51');
    });
  ...
  describe('generate-greeting.js backward compatibility', () => {
    it('should export generateGreeting function', () => {
      const { generateGreeting } = require('../../.triviaiox-core/development/scripts/generate-greeting');
  ...
  describe('ACT-11: configurable timeout', () => {
    ...
    it('should use env var over config value', () => {
      const originalEnv = process.env.TRIVIAIOX_PIPELINE_TIMEOUT;
      process.env.TRIVIAIOX_PIPELINE_TIMEOUT = '800';
      try {
        const timeout = pipeline._resolvePipelineTimeout({ pipeline: { timeout_ms: 300 } });
        expect(timeout).toBe(800);
      } finally {
        if (originalEnv !== undefined) {
          process.env.TRIVIAIOX_PIPELINE_TIMEOUT = originalEnv;
        } else {
          delete process.env.TRIVIAIOX_PIPELINE_TIMEOUT;
        }
      }
    });
    ...
    it('should ignore invalid env var values', () => {
      const originalEnv = process.env.TRIVIAIOX_PIPELINE_TIMEOUT;
      process.env.TRIVIAIOX_PIPELINE_TIMEOUT = 'not-a-number';
      try {
        const timeout = pipeline._resolvePipelineTimeout({});
        expect(timeout).toBe(DEFAULT_PIPELINE_TIMEOUT_MS);
      } finally {
