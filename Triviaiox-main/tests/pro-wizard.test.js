...
beforeEach(() => {
  ...
  delete process.env.TRIVIAIOX_PRO_KEY;
...
describe('showProHeader', () => {
  test('outputs branded header', () => {
    ...
    expect(output).toContain('TRIVIAIOX Pro');
...
describe('stepLicenseGate', () => {
  test('CI mode: fails when TRIVIAIOX_PRO_KEY not set', async () => {
    process.env.CI = 'true';
    delete process.env.TRIVIAIOX_PRO_KEY;
    ...
    expect(result.error).toContain('TRIVIAIOX_PRO_KEY');
  });
  ...
  test('CI mode with valid format key: attempts API validation', async () => {
    process.env.CI = 'true';
    process.env.TRIVIAIOX_PRO_KEY = 'PRO-ABCD-EFGH-IJKL-MNOP';
...
describe('stepInstallScaffold', () => {
  test('resolves pro source from bundled dir or fails gracefully', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-pro-test-'));
  ...
  test('shows summary of scaffolded content', async () => {
    const mockScaffoldResult = {
      copiedFiles: [
        ...
        '.triviaiox-core/pro-config.yaml',
        'pro-version.json',
      ],
    };
...
describe('runProWizard', () => {
  test('fails in CI mode without TRIVIAIOX_PRO_KEY', async () => {
    process.env.CI = 'true';
    delete process.env.TRIVIAIOX_PRO_KEY;
  ...
  test('fails with invalid key format in CI', async () => {
    process.env.CI = 'true';
    process.env.TRIVIAIOX_PRO_KEY = 'INVALID';
  ...
  test('does not show branding in CI mode', async () => {
    process.env.CI = 'true';
    process.env.TRIVIAIOX_PRO_KEY = 'PRO-AAAA-BBBB-CCCC-DDDD';
...
describe('Key Masking Security', () => {
  test('full key never appears in console output during wizard', async () => {
    process.env.CI = 'true';
    const fullKey = 'PRO-ABCD-EFGH-IJKL-MNOP';
    process.env.TRIVIAIOX_PRO_KEY = fullKey;
