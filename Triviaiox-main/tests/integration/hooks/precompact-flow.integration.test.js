...
const { onPreCompact } = require('../../../.triviaiox-core/hooks/unified/runners/precompact-runner');
const proDetector = require('../../../bin/utils/pro-detector');

describe('PreCompact Hook Integration', () => {
  const TEST_PROJECT_DIR = path.join(__dirname, '..', '..', '..', '.triviaiox-test');
  const TEST_DIGESTS_DIR = path.join(TEST_PROJECT_DIR, '.triviaiox', 'session-digests');
  ...
  describe('End-to-End Flow (with triviaiox-pro)', () => {
    it('should create digest file when pro is available', async () => {
      // This test requires actual triviaiox-pro to be present
      const proAvailable = proDetector.isProAvailable();

      if (!proAvailable) {
        console.log('[Integration Test] Skipping: triviaiox-pro not available');
    ...
    it('should handle graceful degradation when pro not available', async () => {
      ...
      if (proAvailable) {
        ...
        console.log('[Test] Skipping graceful degradation test - triviaiox-pro is available');
        return;
      }
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('triviaiox-pro not available'),
      );
  ...
  describe('Performance Benchmarking', () => {
    it('should complete digest extraction within 5 seconds', async () => {
      ...
      if (!proAvailable) {
        console.log('[Performance Test] Skipping: triviaiox-pro not available');
        return;
      }
    ...
    it('should not block compact operation (async fire-and-forget)', async () => {
      ...
      if (!proAvailable) {
        console.log('[Async Test] Skipping: triviaiox-pro not available');
        return;
      }
  ...
  describe('Schema Validation', () => {
    it('should generate digest with valid schema v1.0', async () => {
      ...
      if (!proAvailable) {
        console.log('[Schema Test] Skipping: triviaiox-pro not available');
        return;
      }
