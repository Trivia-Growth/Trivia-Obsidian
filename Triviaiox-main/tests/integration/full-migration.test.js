...
const { createBackup, verifyBackup } = require('../../.triviaiox-core/cli/commands/migrate/backup');
const { detectV2Structure, analyzeMigrationPlan } = require('../../.triviaiox-core/cli/commands/migrate/analyze');
const { executeMigration } = require('../../.triviaiox-core/cli/commands/migrate/execute');
const { updateAllImports, verifyImports } = require('../../.triviaiox-core/cli/commands/migrate/update-imports');
const { validateStructure } = require('../../.triviaiox-core/cli/commands/migrate/validate');
const { executeRollback, canRollback } = require('../../.triviaiox-core/cli/commands/migrate/rollback');

describe('Full Migration Integration', () => {
  let testDir;

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `triviaiox-full-migration-test-${Date.now()}`);
  ...
  async function createMockV20Project(dir) {
    const triviaioxCoreDir = path.join(dir, '.triviaiox-core');

    // Create development directories
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'agents'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'tasks'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'templates'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'checklists'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'scripts'), { recursive: true });

    // Create core directories
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'registry'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'utils'), { recursive: true });
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'config'), { recursive: true });

    // Create product directories
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'cli', 'commands'), { recursive: true });

    // Create infrastructure directories
    await fs.promises.mkdir(path.join(triviaioxCoreDir, 'hooks'), { recursive: true });

    // Create sample files
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'agents', 'dev.md'),
      '# Dev Agent\nDeveloper persona',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'agents', 'qa.md'),
      '# QA Agent\nQuality assurance',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'tasks', 'build.md'),
      '# Build Task\nBuild workflow',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'registry', 'index.js'),
      'const fs = require(\'fs\');\nconst utils = require(\'../utils\');\nmodule.exports = {};',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'utils', 'helpers.js'),
      'module.exports = { helper: () => true };',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'cli', 'index.js'),
      'const registry = require(\'../registry\');\nconst { Command } = require(\'commander\');\nmodule.exports = {};',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'cli', 'commands', 'run.js'),
      'module.exports = { run: () => {} };',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'hooks', 'pre-commit.js'),
      'module.exports = { hook: () => {} };',
    );
    await fs.promises.writeFile(
      path.join(triviaioxCoreDir, 'index.js'),
      'module.exports = require("./registry");',
    );

    // Create config file
    await fs.promises.writeFile(
      path.join(dir, 'triviaiox.config.js'),
      'module.exports = { name: "test-project" };',
    );

    return triviaioxCoreDir;
  }
  ...
  describe('MIG-04: Core Migrated', () => {
    it('should migrate core module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'core', 'registry', 'index.js'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'core', 'utils', 'helpers.js'))).toBe(true);
    });
  });
  ...
  describe('MIG-05: Dev Migrated', () => {
    it('should migrate development module files correctly', async () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'development', 'agents', 'dev.md'))).toBe(true);
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'development', 'tasks', 'build.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-06: Imports Updated', () => {
    it('should verify no broken imports after migration', async () => {
      ...
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      const importResult = await verifyImports(triviaioxCoreDir);
  ...
  describe('MIG-07: Validation Pass', () => {
    it('should validate migrated structure', async () => {
      ...
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      const validation = await validateStructure(triviaioxCoreDir);
  ...
  describe('MIG-08: Rollback Works', () => {
    it('should rollback to original v2.0 state', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'development'))).toBe(true);
      ...
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'agents', 'dev.md'))).toBe(true);
    });
  });
  ...
  describe('MIG-09: Dry Run', () => {
    it('should show plan without making changes in dry run mode', async () => {
      ...
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'development'))).toBe(false);
      expect(fs.existsSync(path.join(testDir, '.triviaiox-core', 'core'))).toBe(false);
    });
  });
  ...
  describe('MIG-11: Conflict Detection', () => {
    it('should detect existing v2.1 directories as conflicts', async () => {
      ...
      // Add a conflict
      await fs.promises.mkdir(path.join(testDir, '.triviaiox-core', 'core'), { recursive: true });
  ...
  describe('MIG-12: Permissions', () => {
    it('should preserve file permissions during migration', async () => {
      ...
      const originalFile = path.join(testDir, '.triviaiox-core', 'cli', 'index.js');
      const originalStats = await fs.promises.stat(originalFile);

      const plan = await analyzeMigrationPlan(testDir);
      await executeMigration(plan, { cleanupOriginals: false });

      const migratedFile = path.join(testDir, '.triviaiox-core', 'product', 'cli', 'index.js');
  ...
  describe('Full Migration Flow', () => {
    it('should complete entire migration workflow', async () => {
      ...
      const triviaioxCoreDir = path.join(testDir, '.triviaiox-core');
      const importResult = await updateAllImports(triviaioxCoreDir, plan);
      expect(importResult.totalFiles).toBeGreaterThan(0);

      // Step 7: Validate structure
      const structureValidation = await validateStructure(triviaioxCoreDir);
