/**
 * Story TRIVIAIOX-DIFF-4.0.5: Onboarding smoke tests in clean environment.
 *
 * Objective:
...
describe('Onboarding smoke flow (TRIVIAIOX-DIFF-4.0.5)', () => {
  const repoRoot = path.resolve(__dirname, '..', '..');
  const cliBin = path.join(repoRoot, 'bin', 'triviaiox.js');
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'triviaiox-onboarding-smoke-'));
  });
  ...
  it('validates Start Here commands are discoverable and executable from clean env', async () => {
    expect(await fs.pathExists(path.join(tempDir, '.triviaiox-core'))).toBe(false);
  ...
  it('validates onboarding docs keep an objective first-value path', async () => {
    ...
    expect(readme).toContain('npx triviaiox-core init');
    expect(readme).toContain('npx triviaiox-core install');

    expect(gettingStarted).toContain('10-Minute Quick Path');
    expect(gettingStarted).toContain('Step 1: Install TRIVIAIOX');
