...
describe('MCP Setup Integration', () => {
  const cliIndexPath = path.join(__dirname, '../../.triviaiox-core/cli/index.js');
  let tempDir;
  let originalHome;
  ...
  describe('CLI Module Loading', () => {
    it('should load mcp command module without errors', () => {
      expect(() => {
        require('../../.triviaiox-core/cli/commands/mcp');
      }).not.toThrow();
    });

    it('should export createMcpCommand function', () => {
      const mcpModule = require('../../.triviaiox-core/cli/commands/mcp');
      expect(typeof mcpModule.createMcpCommand).toBe('function');
    });

    it('should load all mcp subcommand modules', () => {
      expect(() => {
        require('../../.triviaiox-core/cli/commands/mcp/setup');
        require('../../.triviaiox-core/cli/commands/mcp/link');
        require('../../.triviaiox-core/cli/commands/mcp/status');
        require('../../.triviaiox-core/cli/commands/mcp/add');
      }).not.toThrow();
    });
  });
  ...
  describe('Core Module Loading', () => {
    it('should load os-detector module', () => {
      const osDetector = require('../../.triviaiox-core/core/mcp/os-detector');

      expect(osDetector.detectOS).toBeDefined();
      expect(osDetector.isWindows).toBeDefined();
      expect(osDetector.getGlobalTriviaioxDir).toBeDefined();
      expect(osDetector.getGlobalMcpDir).toBeDefined();
    });

    it('should load global-config-manager module', () => {
      const configManager = require('../../.triviaiox-core/core/mcp/global-config-manager');
    ...
    it('should load symlink-manager module', () => {
      const symlinkManager = require('../../.triviaiox-core/core/mcp/symlink-manager');
    ...
    it('should load config-migrator module', () => {
      const configMigrator = require('../../.triviaiox-core/core/mcp/config-migrator');
    ...
    it('should load mcp index module with all exports', () => {
      const mcp = require('../../.triviaiox-core/core/mcp');
  ...
  describe('OS Detection', () => {
    const { detectOS, isWindows, isMacOS, isLinux, getOSInfo } = require('../../.triviaiox-core/core/mcp/os-detector');
  ...
  describe('Server Templates', () => {
    const { getAvailableTemplates, getServerTemplate, SERVER_TEMPLATES } = require('../../.triviaiox-core/core/mcp/global-config-manager');
  ...
  describe('Path Generation', () => {
    ...
    it('should generate correct global TRIVIAIOX directory path', () => {
      const triviaioxDir = getGlobalTriviaioxDir();

      expect(triviaioxDir).toContain('.triviaiox');
      expect(path.isAbsolute(triviaioxDir)).toBe(true);
    });

    it('should generate correct MCP directory path', () => {
      const mcpDir = getGlobalMcpDir();

      expect(mcpDir).toContain('.triviaiox');
  ...
  describe('Link Status Constants', () => {
    const { LINK_STATUS } = require('../../.triviaiox-core/core/mcp/symlink-manager');
  ...
  describe('Migration Options Constants', () => {
    const { MIGRATION_OPTION } = require('../../.triviaiox-core/core/mcp/config-migrator');
  ...
  describe('Default Config Structure', () => {
    const { DEFAULT_CONFIG } = require('../../.triviaiox-core/core/mcp/global-config-manager');
  ...
  describe('CLI Help Output', () => {
    const { createMcpCommand } = require('../../.triviaiox-core/cli/commands/mcp');
...
describe('MCP CLI Command Registration', () => {
  it('should have mcp command in main CLI', () => {
    const { createProgram } = require('../../.triviaiox-core/cli/index');
