 ...
 * Tests for npx triviaiox-core@latest execution
 */
...
describeIntegration('npx Execution', () => {
  ...
  describeIntegration('Package Configuration', () => {
    it('should have correct bin entries in package.json', () => {
      ...
      expect(packageJson.bin['triviaiox']).toBe('./bin/triviaiox.js');
      expect(packageJson.bin['triviaiox-core']).toBe('./bin/triviaiox.js');
    });
    ...
    it('should include required files in package', () => {
      ...
      // Note: .triviaiox-core/ should be included if it exists
      const hasTriviaioxCore = packageJson.files.some(f => 
        f === '.triviaiox-core/' || f === 'triviaiox-core/',
      );
      expect(hasTriviaioxCore).toBe(true);
    });
  ...
  describeIntegration('CLI Execution', () => {
    it('should execute triviaiox command with --version', (done) => {
      const cliPath = path.join(__dirname, '../../bin/triviaiox.js');
    ...
    it('should execute triviaiox command with --help', (done) => {
      const cliPath = path.join(__dirname, '../../bin/triviaiox.js');
      ...
      child.on('close', (code) => {
        ...
        expect(output).toContain('triviaiox-core');
        done();
      });
    }, timeout);

    it('should execute triviaiox info command', (done) => {
      const cliPath = path.join(__dirname, '../../bin/triviaiox.js');
  ...
  describeIntegration('Index.js init Export', () => {
    ...
    it('should maintain TRIVIAIOX class export in source', () => {
      const indexPath = path.join(__dirname, '../../index.js');
      const indexContent = fs.readFileSync(indexPath, 'utf8');

      // Verify TRIVIAIOX class exists in exports
      expect(indexContent).toContain('TRIVIAIOX');
      expect(indexContent).toContain('class TRIVIAIOX');
    });
