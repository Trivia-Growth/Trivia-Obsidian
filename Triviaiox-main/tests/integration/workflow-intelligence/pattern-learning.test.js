...
describe('Pattern Learning Integration', () => {
  ...
  beforeAll(() => {
    learningModule = require('../../../.triviaiox-core/workflow-intelligence/learning');
  });
  ...
  describe('Capture Hook Integration', () => {
    ...
    beforeAll(() => {
      captureHook = require('../../../.triviaiox-core/workflow-intelligence/learning/capture-hook');
    });
    ...
    it('should handle disabled state gracefully', async () => {
      // Save original env
      const originalEnv = process.env.TRIVIAIOX_PATTERN_CAPTURE;

      try {
        process.env.TRIVIAIOX_PATTERN_CAPTURE = 'false';
        captureHook.reset();
        ...
        if (originalEnv !== undefined) {
          process.env.TRIVIAIOX_PATTERN_CAPTURE = originalEnv;
        } else {
          delete process.env.TRIVIAIOX_PATTERN_CAPTURE;
        }
        captureHook.reset();
      }
    });
