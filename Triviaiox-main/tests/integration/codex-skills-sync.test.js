...
const {
  syncSkills,
  buildSkillContent,
} = require('../../.triviaiox-core/infrastructure/scripts/codex-skills-sync/index');
...
describe('Codex Skills Sync', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'triviaiox-codex-skills-'));
    expectedAgentCount = fs.readdirSync(path.join(process.cwd(), '.triviaiox-core', 'development', 'agents'))
  ...
  it('generates one SKILL.md per TRIVIAIOX agent in local .codex/skills', () => {
    const localSkillsDir = path.join(tmpRoot, '.codex', 'skills');
    const result = syncSkills({
      sourceDir: path.join(process.cwd(), '.triviaiox-core', 'development', 'agents'),
      localSkillsDir,
      dryRun: false,
    });

    expect(result.generated).toBe(expectedAgentCount);
    const expected = path.join(localSkillsDir, 'triviaiox-architect', 'SKILL.md');
    expect(fs.existsSync(expected)).toBe(true);

    const content = fs.readFileSync(expected, 'utf8');
    expect(content).toContain('name: triviaiox-architect');
    expect(content).toContain('Activation Protocol');
    expect(content).toContain('.triviaiox-core/development/agents/architect.md');
  ...
  it('supports global installation path when --global mode is enabled', () => {
    ...
    expect(fs.existsSync(path.join(globalSkillsDir, 'triviaiox-dev', 'SKILL.md'))).toBe(true);
  });
  ...
  it('treats globalOnly as global output and skips local writes', () => {
    ...
    expect(result.generated).toBe(expectedAgentCount);
    expect(result.globalSkillsDir).toBe(globalSkillsDir);
    expect(fs.existsSync(path.join(localSkillsDir, 'triviaiox-dev', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(globalSkillsDir, 'triviaiox-dev', 'SKILL.md'))).toBe(true);
  });
  ...
  it('buildSkillContent emits valid frontmatter and starter commands', () => {
    ...
    expect(content).toContain('name: triviaiox-dev');
