...
#
# Usage:
#   .\scripts\sign-manifest.ps1 -SecretKey C:\path\to\triviaiox-core.key
#
# Prerequisites:
...
$ManifestPath = Join-Path $ProjectRoot ".triviaiox-core\install-manifest.yaml"
$SignaturePath = "$ManifestPath.minisig"

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host "  TRIVIAIOX-Core Manifest Signing Tool (Windows)" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host ""
...
if (-not (Test-Path $ManifestPath)) {
    ...
    Write-Host "  node bin\triviaiox.js manifest:generate"
    exit 1
}
...
Push-Location (Join-Path $ProjectRoot ".triviaiox-core")
try {
...
Write-Host "  1. git add .triviaiox-core\install-manifest.yaml.minisig"
