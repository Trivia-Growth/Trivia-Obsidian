...
const EXCLUDE_PATTERNS = [
  ...
  /docs\/standards\/TRIVIAIOX-LIVRO-DE-OURO/,
  /docs\/standards\/TRIVIAIOX-FRAMEWORK-MASTER\.md$/,
...
async function generateManifest() {
  ...
  if (!fs.existsSync(triviaioxCoreDir)) {
    throw new Error(`.triviaiox-core directory not found at ${triviaioxCoreDir}`);
  }
  ...
  console.log(`Generating manifest for triviaiox-core v${version}...`);

  const allFiles = [];

  // Scan folders
  for (const folder of FOLDERS_TO_COPY) {
    const folderPath = path.join(triviaioxCoreDir, folder);
    if (fs.existsSync(folderPath)) {
      scanDirectory(folderPath, triviaioxCoreDir, allFiles);
    }
  }

  // Add root files
  for (const file of ROOT_FILES_TO_COPY) {
    const filePath = path.join(triviaioxCoreDir, file);
  ...
  for (const fullPath of allFiles) {
    try {
      const relativePath = path.relative(triviaioxCoreDir, fullPath).replace(/\\/g, '/');
...
async function writeManifest(manifest) {
  const manifestPath = path.join(__dirname, '..', '.triviaiox-core', 'install-manifest.yaml');
  ...
  // Add header comment
  const header = `# TRIVIAIOX-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('TRIVIAIOX-Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
