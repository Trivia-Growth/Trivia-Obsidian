...
function shouldCheckManifest(stagedFiles) {
  ...
  return stagedFiles.some((file) => {
    if (file === '.triviaiox-core/install-manifest.yaml') return false;
    return file.startsWith('.triviaiox-core/');
  });
}
...
async function main() {
  ...
  if (!shouldCheckManifest(stagedFiles)) {
    console.log('ℹ️ manifest: no .triviaiox-core changes staged, skipping check');
    return;
  }
  ...
  execSync('git add .triviaiox-core/install-manifest.yaml', { stdio: 'inherit' });
