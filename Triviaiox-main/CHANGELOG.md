...
> **Fork notice:** Este projeto é um fork de [SynkraAI/aiox-core](https://github.com/SynkraAI/aiox-core) v5.0.3 (MIT).
> O histórico abaixo refere-se ao projeto upstream. O histórico do **Triviaiox** começa a partir deste fork.

All notable changes to TriviaGrowth Triviaiox will be documented in this file.
...
- Pro wizard (`npx triviaiox-core install`) now auto-installs `@triviaiox-fullstack/pro` package during Step 2, fixing "Pro package not found" error in greenfield and brownfield projects.
...
- `docs/glossary.md` with official TRIVIAIOX taxonomy terms:
  - `squad`
  - `flow-state`
  - `confidence gate`
  - `execution profile`
...
This release marks the **Open-Source Community Readiness** milestone, preparing TRIVIAIOX for public contribution while introducing the **Squad System** for extensibility.
...
- **Rebranding to TriviaGrowth** (OSR-9)
  - Brand investigation complete
  - Namespace updated to Trivia-Growth
...
- **Scripts Path Consolidation** (6.16)
  - Standardized script locations under `.triviaiox-core/scripts/`
...
- **Agent Command Rationalization** (Story 6.1.2.3)
  - Command consolidation: `triviaiox-master` 44→30 commands (32% reduction)
...
- **Scripts Location** - Consolidated under `.triviaiox-core/scripts/` for consistency
...
### Removed
- **Private squads** - Moved to separate private repository (`triviaiox-squads`)
  - Removed `squads/creator/` (CreatorOS)
  - Removed `squads/innerlens/`
  - Removed `squads/mmos-mapper/`
  - Removed `squads/triviaiox-infrastructure-devops/`
  - Removed `squads/meeting-notes/`
  - Repository: https://github.com/Trivia-Growth/triviaiox-squads (PRIVATE)
- **Internal development tools** - Moved to separate private repository (`triviaiox-dev-tools`)
  ...
  - Repository: https://github.com/Trivia-Growth/triviaiox-dev-tools (PRIVATE)
- **hybrid-ops squad** - Moved to separate repository for independent maintenance
  ...
  - Repository: https://github.com/Trivia-Growth/triviaiox-hybrid-ops-pedro-valerio
...
- **Squad naming convention** - Applied consistent `{agent-id}-` prefix to agent-specific tasks across all 6 squads
  ...
  - Triviaiox-infrastructure-devops pack: 2 tasks already renamed (pre-existing)
...
- Breaking Change: hybrid-ops no longer bundled with triviaiox-core
...
- PRIMARY detection layer in `tools/triviaiox-npx-wrapper.js` using `__dirname`
- SECONDARY fallback detection in `tools/installer/bin/triviaiox.js` using `process.cwd()`
...
- Prevents confusion when running `npx triviaiox-core install` from home directory

### Changed
- Early exit with `process.exit(1)` when NPX temporary context detected
- Help message provides actionable solution: `cd /path/to/your/project && npx triviaiox-core install`
