# CLAUDE.md - TriviaGrowth Triviaiox

Este arquivo configura o comportamento do Claude Code ao trabalhar neste repositório.

---

<!-- FRAMEWORK-OWNED: Generated by TRIVIAIOX installer, do not customize -->
## Constitution
...
The installer writes this automatically during `npx triviaiox-core install`. No language config in `core-config.yaml`.
...
O TriviaGrowth Triviaiox segue uma hierarquia clara de prioridades que deve guiar **TODAS** as decisões:
...
├── bin/                     # CLI executables (triviaiox-init.js, triviaiox.js)
├── docs/                    # Documentação
...
|--------|----------------------|
| `@dev` | `packages/`, `.triviaiox-core/core/`, `bin/` |
...
- Agent memory persists in `.triviaiox-core/development/agents/{id}/MEMORY.md`
...
### TRIVIAIOX
```bash
npx triviaiox-core install       # Instalar TRIVIAIOX
...
Prefer native Claude Code tools over MCP for common operations. The tool-registry at `.triviaiox-core/data/tool-registry.yaml` defines the 3-Tier Tool Mesh:
...
**Tool Examples (TOK-4B):** Concrete input examples improve tool selection accuracy by +18pp. See `.claude/rules/tool-examples.md` for examples of top-10 tools. Full registry: `.triviaiox-core/data/mcp-tool-examples.yaml`. ADR-5: examples for always-loaded tools only, search for deferred tools.

**Runtime capabilities:** `.triviaiox/runtime-capabilities.json` (generated by capability-detection.js)
...
*TriviaGrowth Triviaiox Claude Code Configuration v4.0*
*CLI First | Observability Second | UI Third*
