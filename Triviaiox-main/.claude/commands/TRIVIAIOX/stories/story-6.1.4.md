...
- STEP 3: |
    ...
    1. Execute: node .triviaiox-core/scripts/generate-greeting.js {agent-id}
...
Add to `.triviaiox-core/scripts/agent-config-loader.js`:
...
1. Add deprecation warning to `.triviaiox-core/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .triviaiox-core/
grep -r "from.*config-loader" .triviaiox-core/
```
...
1. `.triviaiox-core/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.triviaiox-core/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.triviaiox-core/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.triviaiox-core/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .triviaiox-core/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .triviaiox-core/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .triviaiox-core/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .triviaiox-core/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .triviaiox-core/scripts/batch-integrate-greeting-builder.js ] && rm .triviaiox-core/scripts/batch-integrate-greeting-builder.js
   [ -f .triviaiox-core/scripts/update-activation-instructions.js ] && rm .triviaiox-core/scripts/update-activation-instructions.js
   ```
...
Modify `.triviaiox-core/scripts/greeting-builder.js`:
...
Create `.triviaiox-core/scripts/generate-greeting.js`:
...
- STEP 3: |
    ...
    1. Execute: node .triviaiox-core/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
1. **New Session Test:**
   ```bash
   # Clear session state
   rm .triviaiox/session-state.json
...
3. **Direct Script Test:**
   ...
   node .triviaiox-core/scripts/generate-greeting.js qa
...
4. **Error Handling Test:**
   ...
   node .triviaiox-core/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .triviaiox-core/scripts/generate-greeting.js qa
...
7. `data-engineer.md`
8. `devops.md`
9. `triviaiox-master.md`
10. `ux-design-expert.md`

**Implementation Script:**

Create script `.triviaiox/temp-update-remaining-agents.js`:
...
const REMAINING_AGENTS = [
  ...
  'data-engineer', 'devops', 'triviaiox-master', 'ux-design-expert'
];
...
for agent in dev po sm pm architect analyst data-engineer devops triviaiox-master ux-design-expert; do
  echo "Testing $agent..."
  node .triviaiox-core/scripts/generate-greeting.js $agent
  echo "---"
done
```
...
Create `.triviaiox-core/scripts/command-execution-hook.js`:
...
Create `.triviaiox-core/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.triviaiox-core/scripts/generate-greeting');
...
- `.triviaiox-core/scripts/generate-greeting.js` (Unified greeting generator)
- `.triviaiox-core/scripts/command-execution-hook.js` (Session update hook)
- `.triviaiox-core/docs/session-update-pattern.md` (Documentation)
- `tests/unit/generate-greeting.test.js` (Unit tests)
- `tests/integration/greeting-system-integration.test.js` (Integration tests)

### Files Modified

- `.triviaiox-core/scripts/agent-config-loader.js` (Expanded with `loadAgentDefinition()` and `loadComplete()`)
- `.triviaiox-core/scripts/greeting-builder.js` (Modified to accept pre-loaded context)
- `.triviaiox-core/scripts/config-loader.js` (Deprecated warning added)
- `.triviaiox-core/agents/qa.md` (Pilot - STEP 3 updated first)
- `.triviaiox-core/agents/*.md` (Remaining 10 agents - STEP 3 updated after QA validation)

### Files Deleted

- `.triviaiox-core/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.triviaiox-core/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.triviaiox-core/scripts/update-activation-instructions.js` (Temporary migration script)
- `.triviaiox-core/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.triviaiox-core/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.triviaiox-core/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.triviaiox-core/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.triviaiox-core/scripts/context-detector.js` (Used by greeting-builder.js)
- `.triviaiox-core/scripts/workflow-navigator.js` (Used by greeting-builder.js)
