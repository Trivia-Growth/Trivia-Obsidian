# Integracao TRIVIAIOX com Codex CLI (Estado Atual)

Este documento descreve o estado operacional atual da integracao TRIVIAIOX + Codex CLI no TRIVIAIOX `4.2.11`.
O foco aqui nao e historico: e operacao pratica, compatibilidade real e como manter sem regressao.

## Resumo Executivo

O Codex hoje e alvo de primeira classe no TRIVIAIOX:
...
Em termos praticos: Codex esta no mesmo trilho arquitetural das integracoes principais do TRIVIAIOX, sem caminho paralelo legado.

## Status de Compatibilidade (TRIVIAIOX 4.2)

| O que você quer fazer | Funciona no Codex? | Como fazer |
| --- | --- | --- |
| Ativar agentes TRIVIAIOX | Works | `/skills` depois escolha `triviaiox-<agent-id>` |
| Sincronizar e validar arquivos TRIVIAIOX | Works | `npm run sync:ide:codex` e `npm run validate:codex-sync` |
...
- Agentes canonicos: `.triviaiox-core/development/agents/*.md`
- Regras de projeto Codex: `AGENTS.md`

### Pipeline de ativacao

- Runtime: `.triviaiox-core/development/scripts/activation-runtime.js`
- Entrada de greeting: `.triviaiox-core/development/scripts/generate-greeting.js`
...
- Skills locais: `.codex/skills/triviaiox-*/SKILL.md`
...
4. No Codex, usar `/skills` e escolher `triviaiox-<agent-id>` como ativacao padrao.
...
- Integracao estrutural: `.triviaiox-core/infrastructure/scripts/validate-codex-integration.js`
- Skills: `.triviaiox-core/infrastructure/scripts/codex-skills-sync/validate.js`
- Paths/contratos: `.triviaiox-core/infrastructure/scripts/validate-paths.js`
...
- `.codex/skills/triviaiox-*/SKILL.md` existente
- validadores sem erro

## Recursos Recentes do Codex (TRIVIAIOX 4.0)
...
No TRIVIAIOX, o caminho recomendado continua: `AGENTS.md` + `/skills` + MCP + scripts de sync/validacao.
...
- menor automacao de eventos de ciclo de sessao (`SessionStart/SessionEnd`) no padrao TRIVIAIOX
...
Mitigacao operacional no TRIVIAIOX:
...
- garantir skill apontando para agente canonico em `.triviaiox-core/development/agents/`
