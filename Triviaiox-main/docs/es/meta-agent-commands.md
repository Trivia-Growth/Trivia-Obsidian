...
Guia de referencia completa para todos los comandos del meta-agente de TriviaGrowth Triviaiox.
...
Actualiza TriviaGrowth Triviaiox.
...
TRIVIAIOX> create-agent helper
TRIVIAIOX> activate helper
TRIVIAIOX> helper translate "Hello" --to spanish
TRIVIAIOX> exit
```
...
```bash
TRIVIAIOX_AI_PROVIDER=openai          # Proveedor de IA
TRIVIAIOX_AI_MODEL=gpt-4             # Modelo de IA
TRIVIAIOX_LOG_LEVEL=debug            # Nivel de log
TRIVIAIOX_TELEMETRY=disabled         # Configuracion de telemetria
TRIVIAIOX_TIMEOUT=30000             # Timeout de comando (ms)
TRIVIAIOX_MEMORY_CACHE=true         # Cache de memoria
```
