...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de TRIVIAIOX (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.triviaiox/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.triviaiox/cache/` | Sí           |
...
- id: PC-001
  description: '.triviaiox/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .triviaiox/config.yaml (respaldo: .triviaiox/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .triviaiox/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .triviaiox-core/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx triviaiox task validate deploy'
...
Pasos para resolver:
  1. Abrir .triviaiox-core/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx triviaiox task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .triviaiox/)
...
safeToAutoFix:
  paths:
    - '.triviaiox/**' # Archivos del workspace TRIVIAIOX
    - '.triviaiox-core/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "files": [
    {
      "original": ".triviaiox/config.yaml",
      "backup": "config.yaml",
      "action": "recreate_config",
  ...
  "rollbackCommand": "npx triviaiox health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.triviaiox', 'backups', backupId);
...
npx triviaiox health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx triviaiox health-check --list-backups

# Revertir último respaldo
npx triviaiox health-check --rollback-last
```
...
```
.triviaiox-core/core/health-check/
├── healers/
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.pathExists('.triviaiox/config.yaml'));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.triviaiox-core/templates/config-template.yaml');
    await fs.writeFile('.triviaiox/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.pathExists('.triviaiox/config.yaml');
  }
}
```
...
```javascript
// .triviaiox/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.triviaiox/logs/self-healing.log';
    const logEntry = {
