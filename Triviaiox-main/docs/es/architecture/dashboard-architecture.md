# 🏛️ TRIVIAIOX Dashboard - Arquitectura Completa
...
El TRIVIAIOX Dashboard es una aplicación Next.js que proporciona una interfaz visual para monitorear y gestionar el sistema TRIVIAIOX. Se comunica con el CLI/TRIVIAIOX a través de archivos de estado en el sistema de archivos y Server-Sent Events (SSE).

### Principios Arquitectónicos

1. **CLI-First**: Dashboard es complementario al CLI, no sustituto
2. **File-Based Communication**: Estado a través de `.triviaiox/dashboard/status.json`
...
│  │  │  useTriviaioxStatus │ │                │ │  (SSE + Polling)       ││   │
│  │  └────────────────┘ └────────────────┘ └────────────────────────┘│   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│  ┌──────────────────────────────────────────────────────────────────┐   │
...
│                              FILESYSTEM                                  │
│  ┌──────────────────────────┐  ┌──────────────────────────────────────┐ │
│  │ .triviaiox/dashboard/         │  │ docs/stories/                        │ │
...
│   │   │   ├── status/route.ts       # Polling de estado TRIVIAIOX
...
│   │   ├── use-triviaiox-status.ts        # Estado con SWR
...
| ---------------- | ----------------------------- | -------------------------------- |
| `story-store`    | `triviaiox-stories`            | `storyOrder` (orden de las columnas) |
| `ui-store`       | `triviaiox-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `triviaiox-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `triviaiox-dashboard-settings` | Todo el objeto `settings`         |
...
```typescript
// Retorna estado actual de TRIVIAIOX
// Lee de: .triviaiox/dashboard/status.json

interface TriviaioxStatus {
  version: string;
  updatedAt: string;
  connected: boolean;
...
```
┌─────────────┐                              ┌──────────────────┐
│   CLI/TRIVIAIOX  │                              │    Dashboard     │
│   (Claude)  │                              │    (Next.js)     │
└──────┬──────┘                              └────────┬─────────┘
       │                                              │
       │  1. Agente activado                         │
       │  ─────────────────────────────────────────▶  │
       │     Escribe .triviaiox/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
const { status, isLoading, isConnected, statusError, mutate } = useTriviaioxStatus({
...
_Documentación generada por @architect (Aria) - TRIVIAIOX Core v2.0_
