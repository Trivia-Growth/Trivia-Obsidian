# 🔴 TRIVIAIOX Dashboard - Arquitectura de Observabilidad en Tiempo Real
...
Este documento describe la arquitectura para **observabilidad en tiempo real** del TRIVIAIOX Dashboard, permitiendo que los usuarios monitoreen comandos ejecutados en la CLI con máximo detalle visual.
...
│                                   ▼                                      │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │               .triviaiox/dashboard/events.jsonl (append-only)         │    │
...
```
.triviaiox-core/core/events/dashboard-emitter.ts
```

### Interfaz

```typescript
// .triviaiox-core/core/events/types.ts
...
const EVENTS_DIR = '.triviaiox/dashboard';
...
```typescript
// .triviaiox-core/integrations/claude-code/hooks.ts
...
```
.triviaiox/dashboard/events.jsonl
```
...
const TRIVIAIOX_DIR = process.env.TRIVIAIOX_PROJECT_ROOT || process.cwd();
const STATUS_FILE = join(TRIVIAIOX_DIR, '.triviaiox/dashboard/status.json');
const EVENTS_FILE = join(TRIVIAIOX_DIR, '.triviaiox/dashboard/events.jsonl');
...
│                                                                              │
│  .triviaiox/dashboard/events.jsonl (SOLO ALTO NIVEL)                             │
│  ─────────────────────────────────────────────────                          │
