...
Este documento define los modos de ejecución para el Sistema de Verificación de Salud de TRIVIAIOX (HCS), basado en investigación de mejores prácticas de la industria de Kubernetes, VS Code, Terraform, npm/yarn, y patrones CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Al instalar/bootstrap**    | `npx triviaiox install` | 60-120s  | Esperado                 | Validación de setup         | ✅ **Requerido**  |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx triviaiox health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path: .triviaiox/reports/health-check-*.md
```
...
**Veredicto:** Requerido. Parte de `npx triviaiox install` y `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Post to Slack (on failure)
        ...
        with:
          payload: |
            {
              "text": "⚠️ TRIVIAIOX Health Check Falló",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx triviaiox health-check --mode=quick
```
...
{
  "triviaiox.healthCheck.enabled": true,
  "triviaiox.healthCheck.interval": 300,
  "triviaiox.healthCheck.mode": "quick",
  "triviaiox.healthCheck.showNotifications": true
}
```
