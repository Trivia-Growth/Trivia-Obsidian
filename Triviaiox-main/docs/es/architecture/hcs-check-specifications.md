...
**Propósito:** Verificar que los archivos del framework TRIVIAIOX están correctamente configurados y son consistentes.

### Verificaciones

| ID     | Nombre                      | Severidad | Nivel | Modo  | Timeout | Descripción                                         |
| ------ | --------------------------- | --------- | ----- | ----- | ------- | --------------------------------------------------- |
| PC-001 | Config existe               | CRITICAL  | 1     | quick | 100ms   | `.triviaiox/config.yaml` existe y es YAML válido         |
...
| PC-006 | Sin archivos huérfanos      | LOW       | 3     | full  | 5s      | Todos archivos en `.triviaiox-core/` están referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    const configPath = path.join(context.projectRoot, '.triviaiox', 'config.yaml');

    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.triviaiox/config.yaml no encontrado',
  ...
  async heal(context) {
    const templatePath = '.triviaiox-core/infrastructure/templates/core-config/config-template.yaml';
    const configPath = path.join(context.projectRoot, '.triviaiox', 'config.yaml');
...
const requiredSections = [
  'Project Overview', // o 'TRIVIAIOX-FULLSTACK Development Rules'
...
```yaml
# .triviaiox-core/health-check/checks/ide-checks.yaml
checks:
  - id: IDE-VSCODE
    ...
    validation:
      type: json-schema
      schema: '.triviaiox-core/schemas/vscode-settings.json'
    autoFix:
      tier: 2
      action: 'create-from-template'
      template: '.triviaiox-core/infrastructure/templates/ide/vscode-settings.json'
...
Los usuarios pueden definir verificaciones específicas del proyecto en `.triviaiox/custom-checks.yaml`:

```yaml
# .triviaiox/custom-checks.yaml
version: 1.0
...
checks:
  ...
  - id: CUSTOM-001
    ...
    autoFix:
      action: create-from-template
      template: '.triviaiox-core/infrastructure/templates/project-docs/readme-template.md'
  ...
  - id: CUSTOM-005
    ...
    type: json-schema
    path: 'package.json'
    schema: '.triviaiox-core/schemas/package-json.json'
    severity: CRITICAL
    tier: 3
    mode: quick
```
