...
TriviaGrowth Triviaiox puede instalarse via NPX para una configuración rápida sin instalación global. Esta guía cubre el uso correcto y la resolución de problemas para instalaciones basadas en NPX.

## Inicio Rápido

### Uso Correcto

Siempre ejecute `npx triviaiox-core install` **desde el directorio de su proyecto**:
...
NPX ejecuta paquetes en **directorios temporales** (por ejemplo, `/private/var/folders/.../npx-xxx/` en macOS). Cuando TriviaGrowth Triviaiox se ejecuta desde estas ubicaciones temporales, no puede:
...
A partir de la versión 4.31.1, TriviaGrowth Triviaiox detecta automáticamente cuando está ejecutándose desde un directorio temporal NPX y muestra un mensaje de error útil:

```
⚠️  NPX Temporary Directory Detected

NPX executes in a temporary directory, which prevents
TRIVIAIOX from detecting your IDE correctly.

Solution:
  cd /path/to/your/project
  npx triviaiox-core install

See: https://triviaiox-core.dev/docs/npx-install
```
...
TriviaGrowth Triviaiox detecta estos patrones y previene instalaciones incorrectas.
...
TriviaGrowth Triviaiox v4.31.1+ implementa detección de dos capas:

1. **Capa PRIMARIA** (`tools/triviaiox-npx-wrapper.js`):
...
2. **Capa SECUNDARIA** (`tools/installer/bin/triviaiox.js`):
...
- Documentación: https://triviaiox-core.dev/docs
