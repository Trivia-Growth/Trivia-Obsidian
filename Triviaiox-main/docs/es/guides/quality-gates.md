...
# Guía del Sistema de Quality Gates de TRIVIAIOX

> 🌐 [EN](../../guides/quality-gates.md) | [PT](../../pt/guides/quality-gates.md) | **ES**

---

> Guía completa del sistema de quality gates de 3 capas para TriviaGrowth Triviaiox.
...
El Sistema de Quality Gates de TRIVIAIOX proporciona aseguramiento de calidad automatizado a través de tres capas progresivas de validación. Cada capa detecta diferentes tipos de problemas en la etapa apropiada del desarrollo.
...
```yaml
# .triviaiox-core/core/quality-gates/quality-gate-config.yaml
layer1:
  enabled: true
...
triviaiox qa run --layer=1

# Ejecutar verificación específica
triviaiox qa run --layer=1 --check=lint
triviaiox qa run --layer=1 --check=test
triviaiox qa run --layer=1 --check=typecheck

# Ejecutar con salida detallada
triviaiox qa run --layer=1 --verbose
```
...
layer2:
  ...
  quinn:
    enabled: true
    autoReview: true
    agentPath: '.claude/commands/TRIVIAIOX/agents/qa.md'
    severity:
      block: ['CRITICAL']
      warn: ['HIGH', 'MEDIUM']
```
...
triviaiox qa run --layer=2

# Ejecutar solo CodeRabbit
triviaiox qa run --layer=2 --tool=coderabbit

# Ejecutar revisión de Quinn (@qa)
triviaiox qa run --layer=2 --tool=quinn
```
...
const QualityGateManager = require('./.triviaiox-core/core/quality-gates/quality-gate-manager');
...
triviaiox qa request-review --pr=123

# Aprobar la revisión
triviaiox qa signoff --pr=123 --reviewer="@architect"

# Verificar estado de aprobación
triviaiox qa signoff-status --pr=123
```
...
triviaiox qa run --layer=1
triviaiox qa run --layer=2
triviaiox qa run --layer=3

# Ejecutar con opciones
triviaiox qa run --verbose          # Salida detallada
triviaiox qa run --fail-fast        # Detener en primera falla
triviaiox qa run --continue-on-fail # Continuar a pesar de fallas
```
...
triviaiox qa status --layer=1

# Obtener estado para PR
triviaiox qa status --pr=123
```
...
# Exportar a archivo
triviaiox qa report --output=qa-report.json
triviaiox qa report --format=markdown --output=qa-report.md
```

### `triviaiox qa configure`
...
triviaiox qa configure --layer1.coverage.minimum=90
triviaiox qa configure --layer2.coderabbit.enabled=false
triviaiox qa configure --layer3.requireSignoff=true
```
...
jobs:
  ...
  layer3:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: triviaiox qa request-review --pr=${{ github.event.pull_request.number }}
```
...
triviaiox qa run --layer=1 --fail-fast
```
...
status:
  location: '.triviaiox/qa-status.json'
  updateOnChange: true
...
_Guía del Sistema de Quality Gates de TriviaGrowth Triviaiox v4_
