...
# Guía de Configuración Global de MCP en TRIVIAIOX

> 🌐 [EN](../../guides/mcp-global-setup.md) | [PT](../../pt/guides/mcp-global-setup.md) | **ES**

---

> Configura servidores MCP (Model Context Protocol) globales para TriviaGrowth Triviaiox.
...
El Sistema Global de MCP te permite configurar servidores MCP una vez y compartirlos en todos los proyectos TRIVIAIOX. Esto elimina la necesidad de configurar los mismos servidores en cada proyecto.
...
```
C:\Users\<username>\.triviaiox\mcp\global-config.json
C:\Users\<username>\.triviaiox\mcp\servers\
C:\Users\<username>\.triviaiox\credentials\
```

### macOS

```
/Users/<username>/.triviaiox/mcp/global-config.json
/Users/<username>/.triviaiox/mcp/servers/
/Users/<username>/.triviaiox/credentials/
```

### Linux

```
/home/<username>/.triviaiox/mcp/global-config.json
/home/<username>/.triviaiox/mcp/servers/
/home/<username>/.triviaiox/credentials/
```
...
- `~/.triviaiox/` - Directorio global de TRIVIAIOX
- `~/.triviaiox/mcp/` - Directorio de configuración MCP
- `~/.triviaiox/mcp/global-config.json` - Archivo de configuración principal
- `~/.triviaiox/mcp/servers/` - Configuraciones individuales de servidor
- `~/.triviaiox/mcp/cache/` - Caché de respuestas
- `~/.triviaiox/credentials/` - Almacenamiento seguro de credenciales
...
Location: ~/.triviaiox/mcp/global-config.json
Status:   ✓ Configured

Servers: 0 configured
Cache:   Empty

Run 'triviaiox mcp add <server>' to add servers.
```
...
TRIVIAIOX incluye plantillas para servidores MCP populares:

```bash
# Add from template
triviaiox mcp add context7
triviaiox mcp add exa
triviaiox mcp add github
triviaiox mcp add puppeteer
triviaiox mcp add filesystem
triviaiox mcp add memory
triviaiox mcp add desktop-commander
```
...
triviaiox mcp add my-server --config='{"command":"npx","args":["-y","my-mcp-server"]}'

# Add from config file
triviaiox mcp add my-server --config-file=./my-server-config.json
```
...
triviaiox mcp setup --force

# Specify custom location
triviaiox mcp setup --path=/custom/path
```
...
triviaiox mcp add custom-server --config='{"command":"npx","args":["-y","package"]}'

# Add with environment variables
triviaiox mcp add exa --env='EXA_API_KEY=your-key'
```

### `triviaiox mcp remove`

Elimina un servidor MCP.

```bash
# Remove server
triviaiox mcp remove context7

# Remove with confirmation skip
triviaiox mcp remove context7 --yes
```
...
# List with details
triviaiox mcp list --verbose

# List only enabled
triviaiox mcp list --enabled
```
...
### `triviaiox mcp enable/disable`

Habilita o deshabilita servidores.

```bash
# Disable server
triviaiox mcp disable github

# Enable server
triviaiox mcp enable github

# Toggle
triviaiox mcp toggle github
```
...
triviaiox mcp sync --servers=context7,exa
```
...
```json
// ~/.triviaiox/mcp/servers/context7.json
{
  "type": "sse",
  "url": "https://mcp.context7.com/sse",
  "enabled": true
...
Las credenciales se almacenan en `~/.triviaiox/credentials/` con un `.gitignore` para prevenir commits accidentales.

```bash
# Add credential
triviaiox mcp credential set EXA_API_KEY "your-api-key"

# Get credential
triviaiox mcp credential get EXA_API_KEY

# List credentials (masked)
triviaiox mcp credential list
```

### Formato del Archivo de Credenciales

```json
// ~/.triviaiox/credentials/api-keys.json
{
...
const {
  ...
  removeServer,
  listServers,
} = require('./.triviaiox-core/core/mcp/global-config-manager');
...
const {
  ...
  getGlobalMcpDir,
  getGlobalConfigPath,
} = require('./.triviaiox-core/core/mcp/os-detector');

// Get OS type
console.log(detectOS()); // 'windows' | 'macos' | 'linux'

// Get paths
console.log(getGlobalMcpDir()); // ~/.triviaiox/mcp/
console.log(getGlobalConfigPath()); // ~/.triviaiox/mcp/global-config.json
```
...
| Directorio existe  | Usar `triviaiox mcp setup --force` para recrear                               |
...
# Clear cache
rm -rf ~/.triviaiox/mcp/cache/*

# Verify config
triviaiox mcp status --verbose
...
```json
{
  "triviaiox.mcp.useGlobal": true,
  "triviaiox.mcp.globalPath": "~/.triviaiox/mcp/global-config.json"
}
```
...
_TriviaGrowth Triviaiox v4 Guía de Configuración Global de MCP_
