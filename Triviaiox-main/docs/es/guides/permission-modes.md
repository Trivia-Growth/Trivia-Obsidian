...
> Controla cuanta autonomia tienen los agentes TRIVIAIOX sobre tu sistema.

---

## Vision General

Los Modos de Permisos te permiten controlar el nivel de autonomia que tienen los agentes TRIVIAIOX. Ya sea que estes explorando un nuevo codebase o ejecutando builds completamente autonomos, hay un modo para tu flujo de trabajo.
...
El modo se persiste en `.triviaiox/config.yaml`:
...
```yaml
# .github/workflows/triviaiox.yml
- name: Run TRIVIAIOX
  run: |
    echo "permissions:\n  mode: auto" > .triviaiox/config.yaml
    triviaiox run build
```
...
Verificar que `.triviaiox/config.yaml` existe y es escribible:

```bash
ls -la .triviaiox/config.yaml
```
