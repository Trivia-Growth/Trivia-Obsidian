# Guía de Hardening de Seguridad de TRIVIAIOX

> [EN](../../guides/security-hardening.md) | [PT](../../pt/guides/security-hardening.md) | **ES**

---

> Guía completa para fortalecer la seguridad en despliegues de TriviaGrowth Triviaiox - desde desarrollo hasta producción.
...
TriviaGrowth Triviaiox opera en una capa privilegiada entre los modelos de IA y tu sistema. Esta guía cubre estrategias de hardening específicas para entornos de desarrollo orquestados por IA.
...
### Preocupaciones de Seguridad Específicas de TRIVIAIOX
...
TRIVIAIOX implementa múltiples capas de protección:
...
Las claves API son los secretos más críticos en TRIVIAIOX. Las claves comprometidas pueden llevar a uso no autorizado, brechas de datos e impacto financiero significativo.
...
const secrets = await SecretManager.loadSecrets({
  ...
  secretName: 'triviaiox/production/api-keys',
...
```javascript
// .triviaiox-core/core/security/key-validator.js
const requiredKeys = [
...
```bash
# ============================================================
# CONFIGURACIÓN DE ENTORNO TRIVIAIOX
# ============================================================
...
AUDIT_LOG_PATH=/var/log/triviaiox/audit.log
...
mkdir -p ~/.triviaiox/secrets
chmod 700 ~/.triviaiox/secrets

# Crear archivo de secretos cifrado
# Nunca almacenar secretos en texto plano
openssl enc -aes-256-cbc -salt -pbkdf2 \
  -in secrets.txt \
  -out ~/.triviaiox/secrets/encrypted.dat

# Establecer permisos apropiados
chmod 600 ~/.triviaiox/secrets/*
...
### Permisos de Estructura de Directorios TRIVIAIOX
...
# Directorios de configuración TRIVIAIOX
chmod 700 .triviaiox/              # Solo el propietario puede acceder
chmod 700 .triviaiox-core/         # Fuente del framework
chmod 700 .claude/            # Configuración de Claude

# Archivos de configuración sensibles
chmod 600 .env                # Variables de entorno
chmod 600 .triviaiox/config.yaml   # Configuración principal
chmod 600 .triviaiox/users.json    # Base de datos de usuarios
chmod 600 .triviaiox/sessions.json # Sesiones activas
...
# Archivos temporales
chmod 700 .triviaiox/temp/
chmod 600 .triviaiox/temp/*
```

### Control de Acceso a Directorios

```yaml
# .triviaiox/config.yaml - Configuración de directorios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificación de Permisos de Seguridad TRIVIAIOX"
echo "==========================================="
...
check_permission ".triviaiox" "700"
check_permission ".triviaiox/config.yaml" "600"
...
TRIVIAIOX usa contenedores Docker para aislar los servidores MCP del sistema host:
...
const { OperationGuard } = require('./.triviaiox-core/core/permissions');
...
```javascript
// .triviaiox-core/core/security/input-sanitizer.js
...
```yaml
# .triviaiox/config.yaml - Configuración de auditoría
audit:
  enabled: true
  ...
  retention:
    days: 90
    archivePath: .triviaiox/logs/archive
```
...
```javascript
// .triviaiox-core/core/security/audit-logger.js
...
```yaml
# .triviaiox/config.development.yaml
security:
  debug: true
...
```yaml
# .triviaiox/config.production.yaml
security:
  debug: false
...
function validateProductionSecurity() {
  ...
  if (process.env.TRIVIAIOX_DEBUG === 'true') {
    errors.push('TRIVIAIOX_DEBUG debe ser false en producción');
  }
...
Si descubres una vulnerabilidad de seguridad en TriviaGrowth Triviaiox, por favor sigue las prácticas de divulgación responsable:

### Proceso de Reporte

1. **NO** crees un issue público en GitHub para vulnerabilidades de seguridad
2. Reporta preocupaciones de seguridad via [GitHub Security Advisories](https://github.com/Trivia-Growth/Triviaiox/security/advisories)
...
Actualmente, TriviaGrowth Triviaiox no tiene un programa formal de bug bounty. Sin embargo, las contribuciones significativas de seguridad son reconocidas y pueden recibir licencias de TRIVIAIOX Pro u otro reconocimiento.
...
_Guía de Hardening de Seguridad de TriviaGrowth Triviaiox v4.0_
