...
Sincroniza agentes, tasks, workflows y checklists de TRIVIAIOX a traves de multiples configuraciones de IDE.

## Vision General

La task `*command` automatiza la sincronizacion de componentes TRIVIAIOX a todos los directorios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operaciones de copia manuales.
...
```bash
cp .triviaiox-core/infrastructure/templates/triviaiox-sync.yaml.template .triviaiox-sync.yaml
```

### 2. Configurar IDEs

Editar `.triviaiox-sync.yaml` para habilitar tus IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Leer configuracion .triviaiox-sync.yaml               │
...
```
tu-proyecto/
├── .triviaiox-sync.yaml           # Configuracion de sincronizacion
...
**Solucion**: Agregar el alias a `.triviaiox-sync.yaml`:
...
- [Arquitectura TRIVIAIOX](../core-architecture.md)
