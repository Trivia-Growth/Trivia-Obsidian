...
**@docs (Ajax)** es un agente de IA especializado en crear **documentacion educativa** para TRIVIAIOX. A diferencia de la documentacion tecnica (JSDoc/Typedoc), Ajax se enfoca en **tutoriales, guias de incorporacion, explicadores visuales y materiales de capacitacion para socios**.
...
whenToUse: |
  ...
  - Tutoriales explicando como usar funciones de TRIVIAIOX
...
Entrada:
  ...
  - campo: agent_definition_path
    ...
    obligatorio: false
    example: "squads/triviaiox/agents/dev/agent.yaml"
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Usa estilo consistente (paleta de colores TRIVIAIOX)"
      blocker: false
...
whenToUse: |
  ...
  - Nuevo socio se une al ecosistema TRIVIAIOX
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Enlaces a documentacion TRIVIAIOX relevante"
      blocker: false
...
[Nombre del Agente] es el agente [rol] de TRIVIAIOX, especializado en [experiencia principal].
...
**Notas de Plantilla:**
- Usar paleta de colores TRIVIAIOX (Azul primario, Purpura secundario, Rosa terciario)
...
```bash
#!/bin/bash
# Hook de Auto-Actualizacion TRIVIAIOX @docs
...
if echo "$changed_files" | grep -qE "^(src/|triviaiox-core/)"; then
  echo "Cambios de codigo detectados. Analizando impacto en documentacion..."

  # Invocar tarea de actualizacion de @docs
  triviaiox agent invoke @docs --task update-educational-docs \
    --files "$changed_files" \
    --mode interactive
...
jobs:
  check-docs:
    ...
    steps:
      ...
      - name: Instalar TRIVIAIOX CLI
        run: npm install -g @triviaiox/core
  ...
  qa-docs:
    ...
    steps:
      ...
      - name: Ejecutar QA de Documentacion
        id: qa
        run: |
          triviaiox agent invoke @docs --task qa-documentation \
            --paths "docs/**/*.md" \
...
- [ ] Crear `squads/triviaiox/agents/docs/agent.yaml`
...
- [ ] **Reconocimiento de Industria:** Docs de TRIVIAIOX citados como "mejor en espacio de agentes IA"
...
Con la adicion de @docs (Ajax), TRIVIAIOX ahora tiene **13 agentes nombrados**:
...
| 13 | @triviaiox-master | Orion | Comandante | Aries | Cian |
