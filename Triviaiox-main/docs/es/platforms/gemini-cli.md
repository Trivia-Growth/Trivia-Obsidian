...
# Guía de TRIVIAIOX para Gemini CLI
...
### ¿Por qué usar TRIVIAIOX con Gemini CLI?
...
```markdown
# Reglas de TriviaGrowth Triviaiox para Gemini CLI
...
## Activación
Incluye "Como el agente TRIVIAIOX dev" en tu prompt.
...
### Activando Agentes de TRIVIAIOX

Incluye contexto de agentes en los prompts:

```bash
# Agente de desarrollador
gemini "Como el agente TRIVIAIOX dev, implementa la característica de login"

# Agente de QA
gemini "Como el agente TRIVIAIOX qa, revisa este código por seguridad"

# Agente de arquitecto
gemini "Como el agente TRIVIAIOX architect, diseña el esquema de base de datos"
```
...
cat src/auth.ts | gemini "Como TRIVIAIOX qa, revisa este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como TRIVIAIOX qa, revisa este archivo" >> reviews.md
done
```
...
**Frase de Activación:** "Como el agente TRIVIAIOX dev"

## Rol
Eres un Desarrollador Full Stack Senior trabajando con TriviaGrowth Triviaiox.
...
1. Instalar TRIVIAIOX para Gemini CLI:
   ```bash
   npx @anthropic/triviaiox init --ide gemini-cli
   ```
2. Los agentes se transforman a formato basado en prompts

### De Gemini CLI a Otros IDEs

1. Los agentes de TRIVIAIOX en `.triviaiox-core/` son portátiles
...
*TriviaGrowth Triviaiox - Guía de Plataforma Gemini CLI v1.0*
