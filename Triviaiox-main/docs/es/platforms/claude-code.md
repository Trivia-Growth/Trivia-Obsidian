...
# Guía TRIVIAIOX para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | [PT](../../pt/platforms/claude-code.md) | **ES**

---

> **CLI Oficial de Anthropic** - Plataforma Recomendada para TRIVIAIOX
...
### ¿Por qué usar TRIVIAIOX con Claude Code?

Claude Code ofrece la **mejor integración** con TRIVIAIOX debido a:
...
### Paso 3: Instalar TRIVIAIOX

```bash
# Navega a tu proyecto
cd tu-proyecto

# Inicializa TRIVIAIOX
npx @anthropic/triviaiox init
...
# Verifica que los archivos TRIVIAIOX fueron creados
ls -la .claude/
```
...
- Comprensión del marco TRIVIAIOX
...
export TRIVIAIOX_DEBUG="true"                        # Habilitar registro de depuración
```
...
```mermaid
flowchart LR
    Source["TRIVIAIOX Core<br/>.triviaiox-core/development/agents"] --> Parser["Analizador de Agentes"]
    Parser --> Transform["Transformador Claude"]
    Transform --> Output[".claude/commands/TRIVIAIOX/agents"]
```
...
1. Los cambios locales se respaldan en `.claude/commands/TRIVIAIOX/agents/.backup/`
...
# Verificar registros de TRIVIAIOX
cat .triviaiox-core/logs/sync.log
```
...
### Preguntas Específicas de TRIVIAIOX
...
R: En `.claude/commands/TRIVIAIOX/agents/` después de la sincronización.

**P: ¿Con qué frecuencia debo sincronizar agentes?**
R: Después de actualizar TRIVIAIOX o cuando los agentes se modifiquen en el núcleo.
...
2. Inicializa TRIVIAIOX con Claude Code:
   ```bash
   npx @anthropic/triviaiox init --ide claude-code
   ```
...
1. Tus agentes TRIVIAIOX se almacenan en `.triviaiox-core/development/agents/`
2. Inicializa TRIVIAIOX para el IDE objetivo
...
*TriviaGrowth Triviaiox - Guía de Plataforma Claude Code v1.0*
