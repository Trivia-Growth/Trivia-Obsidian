...
Complete reference guide for all TriviaGrowth Triviaiox meta-agent commands.
...
```bash
TRIVIAIOX_AI_PROVIDER=openai          # AI provider
TRIVIAIOX_AI_MODEL=gpt-4             # AI model
TRIVIAIOX_LOG_LEVEL=debug            # Log level
TRIVIAIOX_TELEMETRY=disabled         # Telemetry setting
TRIVIAIOX_TIMEOUT=30000             # Command timeout (ms)
TRIVIAIOX_MEMORY_CACHE=true         # Memory caching
```
