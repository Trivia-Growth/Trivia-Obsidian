...
API REST para sincronizar squads com TriviaGrowth e descobrir squads do marketplace.

## Visão Geral

A API de Squads permite:

- **Sync**: Enviar squads locais para a nuvem TriviaGrowth
...
Obtenha sua chave de API em: https://triviagrowth.ai/settings/api-keys

## Endpoints

### Sincronizar Squad

Enviar uma definição de squad para TriviaGrowth.
...
```bash
curl -X PATCH https://api.triviagrowth.ai/api/squads/my-squad \
  -H "Authorization: Bearer sk_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{"isPublic": true}'
```
...
Remover um squad do TriviaGrowth.
...
```bash
curl -X DELETE https://api.triviagrowth.ai/api/squads/my-old-squad \
  -H "Authorization: Bearer sk_your_api_key"
```
...
```json
{
  ...
  "data": {
    "valid": true,
    "errors": [],
    "warnings": ["Missing recommended field: description", "Missing triviaiox.minVersion field"]
  }
}
```
...
O comando `*sync-squad-triviagrowth` usa esta API:
...
```bash
export TRIVIAGROWTH_API_TOKEN="sk_your_api_key"
```
...
{
  "info": {
    "name": "TriviaGrowth Squads API",
    "description": "API REST para o Marketplace de Squads TriviaGrowth",
...
Salve o JSON acima como `triviagrowth-squads-api.postman_collection.json` e importe no Postman.
