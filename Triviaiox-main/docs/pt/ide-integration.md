...
Guia para integrar o TRIVIAIOX com IDEs e plataformas de desenvolvimento com IA suportadas.
...
O TRIVIAIOX suporta 6 plataformas de desenvolvimento com IA. Escolha a que melhor se adapta ao seu fluxo de trabalho.
...
| IDE | Paridade de Hooks vs Claude | O que degrada sem hooks completos | Mitigação no TRIVIAIOX |
| --- | --- | --- | --- |
| Claude Code | Completa | Nenhum (comportamento de referência) | Hooks nativos + pipeline completo do TRIVIAIOX |
...
**Nível de Recomendação:** Melhor integração com TRIVIAIOX
...
1. TRIVIAIOX cria automaticamente o diretório `.claude/` durante a inicialização
...
4. Use `/skills` e selecione `triviaiox-<agent-id>`
...
1. TRIVIAIOX cria o diretório `.cursor/` durante a inicialização
...
2. TRIVIAIOX cria `.github/copilot-instructions.md`
...
1. TRIVIAIOX cria o diretório `.antigravity/`
...
O TRIVIAIOX mantém uma única fonte de verdade para definições de agentes e as sincroniza com todas as IDEs configuradas:
...
O TRIVIAIOX pode ser configurado para sincronizar automaticamente quando houver mudanças nos agentes:
...
├── Sim --> Claude Code (Melhor integração com TRIVIAIOX)
└── Não
...
_Guia de Integração com IDEs do TriviaGrowth Triviaiox v4.0_
