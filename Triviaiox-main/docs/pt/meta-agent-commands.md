...
Guia de referência completo para todos os comandos do meta-agente TriviaGrowth Triviaiox.
...
Atualiza o TriviaGrowth Triviaiox.
...
```bash
TRIVIAIOX_AI_PROVIDER=openai          # Provedor de IA
TRIVIAIOX_AI_MODEL=gpt-4             # Modelo de IA
TRIVIAIOX_LOG_LEVEL=debug            # Nível de log
TRIVIAIOX_TELEMETRY=disabled         # Configuração de telemetria
TRIVIAIOX_TIMEOUT=30000             # Timeout de comando (ms)
TRIVIAIOX_MEMORY_CACHE=true         # Cache de memória
```
