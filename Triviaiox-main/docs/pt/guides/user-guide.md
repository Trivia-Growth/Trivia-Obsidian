...
# Guia do Usuário TRIVIAIOX

> **PT-BR**

---

Guia completo para usar o TriviaGrowth Triviaiox - o Sistema Orquestrado por IA para Desenvolvimento Full Stack.
...
Antes de usar o TRIVIAIOX, certifique-se de ter:
...
O TRIVIAIOX fornece estrutura orquestrada enquanto permite flexibilidade na comunicação. Isso significa:
...
### A Diferença do TRIVIAIOX

| Desenvolvimento Tradicional com IA | TRIVIAIOX                                        |
| ---------------------------------- | ------------------------------------------- |
...
O TRIVIAIOX inclui 11 agentes especializados, cada um com papel e personalidade distintos:
...
| **Pax**   | `@triviaiox-master`   | Orquestrador | Orquestração do framework |
...
@triviaiox-master        # Ativar Pax (Orquestrador)
...
Tasks são o ponto de entrada principal no TRIVIAIOX. Tudo é uma task.
...
| `brownfield-integration` | Adicionar TRIVIAIOX a existente | dev, architect     |
...
Squads são equipes modulares de agentes de IA que estendem a funcionalidade do TRIVIAIOX.
...
Nível 2: TRIVIAIOX-SQUADS  --> github.com/Trivia-Growth (Público/Gratuito)
...
# Baixar um squad
triviaiox squads download etl-squad
...
```
my-project/
├── .triviaiox-core/                # Configuração do framework
...
A configuração principal está em `.triviaiox-core/core/config/`:
...
O TRIVIAIOX suporta múltiplas IDEs. A configuração é sincronizada entre:
...
cat .triviaiox-core/development/tasks/{task-name}.md
...
```bash
# Limpar cache
rm -rf .triviaiox-core/core/cache/*
...
O TRIVIAIOX implementa quality gates em 3 camadas:
...
```bash
triviaiox squads search {keyword}
```
...
_Guia do Usuário TriviaGrowth Triviaiox v4.0_
