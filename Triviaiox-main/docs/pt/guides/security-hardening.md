# Guia de Hardening de Segurança do TRIVIAIOX

> [EN](../../guides/security-hardening.md) | **PT** | [ES](../../es/guides/security-hardening.md)

---

> Guia completo para fortalecer a segurança de implantações do TriviaGrowth Triviaiox - do desenvolvimento à produção.
...
O TriviaGrowth Triviaiox opera em uma camada privilegiada entre modelos de IA e seu sistema. Este guia cobre estratégias de hardening específicas para ambientes de desenvolvimento orquestrados por IA.
...
### Preocupações de Segurança Específicas do TRIVIAIOX
...
O TRIVIAIOX implementa múltiplas camadas de proteção:
...
Chaves de API são os segredos mais críticos no TRIVIAIOX. Chaves comprometidas podem levar a uso não autorizado, violações de dados e impacto financeiro significativo.
...
```bash
# ============================================================
# CONFIGURAÇÃO DE AMBIENTE DO TRIVIAIOX
# ============================================================
...
### Permissões da Estrutura de Diretórios do TRIVIAIOX
...
# Diretórios de configuração do TRIVIAIOX
chmod 700 .triviaiox/              # Apenas o proprietário pode acessar
chmod 700 .triviaiox-core/         # Fonte do framework
chmod 700 .claude/            # Configuração do Claude

# Arquivos de configuração sensíveis
chmod 600 .env                # Variáveis de ambiente
chmod 600 .triviaiox/config.yaml   # Configuração principal
chmod 600 .triviaiox/users.json    # Banco de dados de usuários
chmod 600 .triviaiox/sessions.json # Sessões ativas
...
```yaml
# .triviaiox/config.yaml - Configuração de diretórios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificação de Permissões de Segurança do TRIVIAIOX"
echo "==============================================="
...
O TRIVIAIOX usa containers Docker para isolar servidores MCP do sistema host:
...
```yaml
# .triviaiox/config.yaml - Configuração de auditoria
audit:
  enabled: true
...
function validateProductionSecurity() {
  ...
  if (process.env.TRIVIAIOX_DEBUG === 'true') {
    errors.push('TRIVIAIOX_DEBUG deve ser false em produção');
  }
...
Se você descobrir uma vulnerabilidade de segurança no TriviaGrowth Triviaiox, por favor siga práticas de divulgação responsável:

### Processo de Relatório

1. **NÃO** crie uma issue pública no GitHub para vulnerabilidades de segurança
2. Reporte preocupações de segurança via [GitHub Security Advisories](https://github.com/Trivia-Growth/Triviaiox/security/advisories)
...
Atualmente, o TriviaGrowth Triviaiox não possui um programa formal de bug bounty. No entanto, contribuições significativas de segurança são reconhecidas e podem receber licenças TRIVIAIOX Pro ou outro reconhecimento.
...
_Guia de Hardening de Segurança do TriviaGrowth Triviaiox v4.0_
