...
# Guia de Configuração Global MCP do TRIVIAIOX

> 🌐 [EN](../../guides/mcp-global-setup.md) | **PT** | [ES](../../es/guides/mcp-global-setup.md)

---

> Configure servidores MCP (Model Context Protocol) globais para TriviaGrowth Triviaiox.
...
O Sistema Global MCP permite que você configure servidores MCP uma única vez e os compartilhe entre todos os projetos TRIVIAIOX. Isso elimina a necessidade de configurar os mesmos servidores em cada projeto.
...
- `~/.triviaiox/` - Diretório global do TRIVIAIOX
- `~/.triviaiox/mcp/` - Diretório de configuração MCP
- `~/.triviaiox/mcp/global-config.json` - Arquivo de configuração principal
- `~/.triviaiox/mcp/servers/` - Configurações individuais de servidores
- `~/.triviaiox/mcp/cache/` - Cache de respostas
- `~/.triviaiox/credentials/` - Armazenamento seguro de credenciais
...
O TRIVIAIOX inclui templates para servidores MCP populares:
...
Credenciais são armazenadas em `~/.triviaiox/credentials/` com um `.gitignore` para prevenir commits acidentais.
...
| Diretório já existe    | Use `triviaiox mcp setup --force` para recriar                                 |
...
_TriviaGrowth Triviaiox v4 Guia de Configuração Global MCP_
