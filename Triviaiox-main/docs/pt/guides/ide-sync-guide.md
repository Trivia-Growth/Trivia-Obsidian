...
Sincronize agentes, tasks, workflows e checklists do TRIVIAIOX entre múltiplas configurações de IDE.

## Visão Geral

A task `*command` automatiza a sincronização de componentes do TRIVIAIOX para todos os diretórios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operações manuais de cópia.
...
Edite `.triviaiox-sync.yaml` para habilitar suas IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Ler configuração .triviaiox-sync.yaml                 │
...
```
seu-projeto/
├── .triviaiox-sync.yaml           # Configuração de sincronização
...
**Solução**: Adicione o alias em `.triviaiox-sync.yaml`:
...
- [Arquitetura TRIVIAIOX](../core-architecture.md)
