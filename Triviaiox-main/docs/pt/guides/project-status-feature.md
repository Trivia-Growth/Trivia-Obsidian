...
A funcionalidade de Status do Projeto exibe automaticamente seu contexto de trabalho atual quando qualquer agente TRIVIAIOX é ativado. Isso inclui:
...
- **Framework TRIVIAIOX-FullStack** instalado
...
3. Criar arquivo de cache `.triviaiox/project-status.yaml`
...
1. Edite `.triviaiox-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Localização: `.triviaiox-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .triviaiox/project-status.yaml  # Localização do arquivo de cache
...
1. **Verifica cache** - Procura por `.triviaiox/project-status.yaml`
...
- **Localização do Cache:** `.triviaiox/project-status.yaml`
...
Todos os 11 agentes TRIVIAIOX exibem status do projeto:
...
11. **@triviaiox-master** (Orion - Orchestrator)
...
3. O arquivo `.triviaiox-core/infrastructure/scripts/project-status-loader.js` existe?
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.triviaiox-core/infrastructure/scripts/project-status-loader.js');
...
const { clearCache } = require('./.triviaiox-core/infrastructure/scripts/project-status-loader.js');
await clearCache();
```
...
```bash
# Remove script
rm .triviaiox-core/infrastructure/scripts/project-status-loader.js

# Remove task
rm .triviaiox-core/tasks/init-project-status.md

# Remove cache
rm .triviaiox/project-status.yaml

# Remove tests
rm .triviaiox-core/infrastructure/scripts/__tests__/project-status-loader.test.js
...
R: Sim, todos os agentes usam o mesmo arquivo de cache (`.triviaiox/project-status.yaml`).

---

## Documentação Relacionada

- **Story:** `docs/stories/triviaiox migration/story-6.1.2.4-project-status-context.md`
- **Config:** `.triviaiox-core/core-config.yaml` (seção projectStatus)
- **Script:** `.triviaiox-core/infrastructure/scripts/project-status-loader.js`
- **Task de Init:** `.triviaiox-core/tasks/init-project-status.md`
- **Testes:** `.triviaiox-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
