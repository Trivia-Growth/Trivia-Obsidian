# 🏛️ TRIVIAIOX Dashboard - Arquitetura Completa
...
O TRIVIAIOX Dashboard é uma aplicação web Next.js que fornece uma interface visual para monitorar e gerenciar o sistema TRIVIAIOX. Ele se comunica com o CLI/TRIVIAIOX através de arquivos de status no filesystem e Server-Sent Events (SSE).

### Princípios Arquiteturais

1. **CLI-First**: Dashboard é complementar ao CLI, não substituto
2. **File-Based Communication**: Status via `.triviaiox/dashboard/status.json`
...
│   │   │   ├── status/route.ts       # TRIVIAIOX status polling
...
│   │   ├── use-triviaiox-status.ts        # Status with SWR
...
| ---------------- | ------------------------- | -------------------------------- |
| `story-store`    | `triviaiox-stories`            | `storyOrder` (ordem das colunas) |
| `ui-store`       | `triviaiox-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `triviaiox-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `triviaiox-dashboard-settings` | Todo o objeto `settings`         |
...
```typescript
// Retorna status atual do TRIVIAIOX
// Lê de: .triviaiox/dashboard/status.json
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agent ativado                            │
       │  ─────────────────────────────────────────▶  │
       │     Escreve .triviaiox/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
_Documentação gerada por @architect (Aria) - TRIVIAIOX Core v2.0_
