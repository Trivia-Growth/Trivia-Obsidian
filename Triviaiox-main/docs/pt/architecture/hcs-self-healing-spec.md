...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do TRIVIAIOX (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.triviaiox/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.triviaiox/cache/`     | Sim           |
...
- id: PC-001
  description: '.triviaiox/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .triviaiox/config.yaml (backup: .triviaiox/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .triviaiox/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx triviaiox task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx triviaiox task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .triviaiox/)
...
safeToAutoFix:
  paths:
    - '.triviaiox/**' # Arquivos de workspace do TRIVIAIOX
    - '.triviaiox-core/**/*.yaml' # YAML do framework (cuidado)
