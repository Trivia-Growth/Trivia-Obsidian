...
**Propósito:** Verificar se os arquivos do framework TRIVIAIOX estão corretamente configurados e consistentes.

### Verificações

| ID     | Nome                           | Severidade | Nível | Modo  | Timeout | Descrição                                            |
| ------ | ------------------------------ | ---------- | ----- | ----- | ------- | ---------------------------------------------------- |
| PC-001 | Config existe                  | CRITICAL   | 1     | quick | 100ms   | `.triviaiox/config.yaml` existe e é YAML válido           |
...
| PC-006 | Sem arquivos órfãos            | LOW        | 3     | full  | 5s      | Todos os arquivos em `.triviaiox-core/` são referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    ...
    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.triviaiox/config.yaml não encontrado',
...
const requiredSections = [
  'Project Overview', // ou 'TRIVIAIOX-FULLSTACK Development Rules'
...
Usuários podem definir verificações específicas do projeto em `.triviaiox/custom-checks.yaml`:
