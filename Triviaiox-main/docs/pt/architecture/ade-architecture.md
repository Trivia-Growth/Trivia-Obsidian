...
O **Motor de Desenvolvimento Autônomo (ADE)** é a infraestrutura do TRIVIAIOX para workflows de desenvolvimento autônomo. Ele permite que agentes de IA trabalhem de forma independente através de pipelines inteligentes, loops auto-recuperáveis e aprendizado persistente.
...
Todo o estado do ADE é persistido em `.triviaiox/` para:
...
Arquivo de Estado: .triviaiox/project-status.yaml
```
...
| ------------------- | ---------------------------------- | --------------------------------- |
| Padrões de Código   | Padrões reutilizáveis da base     | .triviaiox/patterns/code-patterns.json |
| Gotchas             | Problemas conhecidos e soluções   | .triviaiox/patterns/gotchas.json       |
| Insights de Sessão  | Descobertas durante sessões       | .triviaiox/sessions/                   |
| Mapa de Codebase    | Análise de estrutura do projeto   | .triviaiox/codebase-map.json           |
...
ADE integra-se com agentes TRIVIAIOX através de:
...
Todos os componentes do ADE atualizam `.triviaiox/status.json`:
...
Localizada em `.triviaiox-core/core-config.yaml`:
...
_Este é um padrão oficial do framework TRIVIAIOX documentando o Motor de Desenvolvimento Autônomo._
