...
Este documento define limites claros de responsabilidade para todos os agentes TRIVIAIOX, com foco particular em:
...
1. **Git Pre-Push Hook** (Aplicação Primária)
   ...
   - Verificações: Variável de ambiente `$TRIVIAIOX_ACTIVE_AGENT`
   - Ação: Bloqueia push se agent != "github-devops"

2. **Variáveis de Ambiente** (Detecção em Runtime)
   ```bash
   export TRIVIAIOX_ACTIVE_AGENT="github-devops"
   export TRIVIAIOX_GIT_PUSH_ALLOWED="true"
   ```
...
- [ ] **Criar Git Pre-Push Hook**
  - Localização: `.git/hooks/pre-push`
  - Conteúdo: Verificar `$TRIVIAIOX_ACTIVE_AGENT`, bloquear se != "github-devops"
...
- [ ] **Atualizar Scripts de Ativação de Agentes**
  - Adicionar configuração de variável de ambiente: `TRIVIAIOX_ACTIVE_AGENT={agent_id}`
  - Configurar `TRIVIAIOX_GIT_PUSH_ALLOWED` apropriadamente
