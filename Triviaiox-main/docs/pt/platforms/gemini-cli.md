# Guia TRIVIAIOX para Gemini CLI
...
### Por que usar TRIVIAIOX com Gemini CLI?
...
```markdown
# Regras TriviaGrowth Triviaiox para Gemini CLI
...
## Ativação
Inclua "Como agente TRIVIAIOX dev" no seu prompt.
...
```bash
# Agente desenvolvedor
gemini "Como agente TRIVIAIOX dev, implemente a feature de login"

# Agente QA
gemini "Como agente TRIVIAIOX qa, revise este código para segurança"

# Agente arquiteto
gemini "Como agente TRIVIAIOX architect, projete o schema do banco de dados"
```
...
cat src/auth.ts | gemini "Como TRIVIAIOX qa, revise este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como TRIVIAIOX qa, revise este arquivo" >> reviews.md
done
```
...
**Frase de Ativação:** "Como agente TRIVIAIOX dev"
...
1. Instale TRIVIAIOX para Gemini CLI:
   ```bash
...
1. Agentes TRIVIAIOX em `.triviaiox-core/` são portáveis
...
*TriviaGrowth Triviaiox - Guia da Plataforma Gemini CLI v1.0*
