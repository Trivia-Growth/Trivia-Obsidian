...
O TriviaGrowth Triviaiox pode ser instalado via NPX para uma configuração rápida sem instalação global. Este guia cobre o uso correto e a resolução de problemas para instalações baseadas em NPX.

## Início Rápido

### Uso Correto

Sempre execute `npx triviaiox-core install` **a partir do diretório do seu projeto**:
...
O NPX executa pacotes em **diretórios temporários** (ex: `/private/var/folders/.../npx-xxx/` no macOS). Quando o TriviaGrowth Triviaiox é executado a partir desses locais temporários, ele não consegue:
...
A partir da versão 4.31.1, o TriviaGrowth Triviaiox detecta automaticamente quando está sendo executado a partir de um diretório temporário do NPX e exibe uma mensagem de erro útil:

```
⚠️  Diretório Temporário do NPX Detectado

O NPX executa em um diretório temporário, o que impede
o TRIVIAIOX de detectar sua IDE corretamente.

Solução:
  cd /path/to/your/project
  npx triviaiox-core install

Veja: https://triviaiox-core.dev/docs/npx-install
```
...
O TriviaGrowth Triviaiox detecta esses padrões e previne instalação incorreta.
...
O TriviaGrowth Triviaiox v4.31.1+ implementa detecção em duas camadas:

1. **Camada PRIMÁRIA** (`tools/triviaiox-npx-wrapper.js`):
...
2. **Camada SECUNDÁRIA** (`tools/installer/bin/triviaiox.js`):
...
- GitHub Issues: https://github.com/Trivia-Growthinc/triviaiox-core/issues
- Documentação: https://triviaiox-core.dev/docs
