...
- **FR9:** O sistema deve funcionar como CLI command (`triviaiox graph`) com sub-comandos: `--deps`, `--blast <file>`, `--stats`, `--watch`
...
O CLI Graph Dashboard será adicionado ao monorepo existente `triviaiox-core`, como um novo módulo em `.triviaiox-core/core/graph-dashboard/` com entrypoint em `bin/triviaiox-graph.js`.
...
### Epic 1: MVP — ASCII Text Output (`triviaiox graph`)
...
**Goal:** Estabelecer o CLI command `triviaiox graph` com output ASCII do dependency graph e entity stats. Entrega valor imediato com zero dependências TUI pesadas — apenas `asciichart` para line charts simples. Este é o fundamento sobre o qual os epics seguintes construirão.

### Story 1.1: CLI Entrypoint e Dependency Tree ASCII

**As a** developer using TRIVIAIOX,
**I want** to run `triviaiox graph --deps` in the terminal,
**so that** I can see the dependency tree of code-intel entities as ASCII text.

#### Acceptance Criteria

1. Comando `npx triviaiox-core graph` existe e é executável
...
5. Output é válido para pipe (`triviaiox graph --deps | grep helper`)
6. Testes unitários cobrem: tree rendering, fallback data, empty graph

### Story 1.2: Entity Stats e Cache Metrics ASCII

**As a** developer using TRIVIAIOX,
**I want** to run `triviaiox graph --stats` to see entity statistics and cache metrics,
...
1. Sem flags adicionais, `triviaiox graph` mostra summary view (dependency tree + stats + provider status)
...
1. `triviaiox graph --interactive` abre dashboard blessed-contrib fullscreen
...
**Goal:** Adicionar modo watch para updates automáticos, blast radius interativo para qualquer arquivo, e comandos de agente (`*graph`) para integração no workflow dos agentes TRIVIAIOX.
...
1. `triviaiox graph --blast <file>` mostra blast radius de um arquivo específico
...
> @architect — Crie a arquitetura técnica detalhada para o CLI Graph Dashboard usando este PRD como input. Foco em: (1) estrutura de módulos dentro de `.triviaiox-core/core/graph-dashboard/`, (2) data source adapters para code-intel e entity-registry, (3) widget composition pattern com blessed-contrib, (4) CLI command routing via `bin/triviaiox-graph.js`, (5) fallback strategy quando Code Graph MCP está offline. Reference: `docs/research/2026-02-21-cli-graph-dashboard/03-recommendations.md` para stack recomendado.
