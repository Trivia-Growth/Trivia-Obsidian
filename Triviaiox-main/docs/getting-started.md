# Getting Started with TriviaGrowth Triviaiox

> **EN** | [PT](./pt/getting-started.md) | [ES](./es/getting-started.md)

---

Welcome to TriviaGrowth Triviaiox. This guide is optimized for first-value in 10 minutes.
...
### Step 1: Install TRIVIAIOX

```bash
# New project
npx triviaiox-core init my-first-project
cd my-first-project
...
- Gemini CLI: `/triviaiox-menu` then `/triviaiox-<agent>`
- Codex CLI: `/skills` then `triviaiox-<agent-id>`
...
1. You activate one TRIVIAIOX agent.
...
# Start using TRIVIAIOX agents in your IDE
...
npx triviaiox-core init my-project --template enterprise
...
npx triviaiox-core init my-project --force

# 4. Skip dependency installation (manual install later)
npx triviaiox-core init my-project --skip-install
```
...
```
my-first-project/
├── .triviaiox-core/                 # TRIVIAIOX framework core
...
TRIVIAIOX configuration lives in `.triviaiox-core/core/config/`. The installer handles initial setup. To verify your installation:
...
TRIVIAIOX agents are activated through your IDE. Once activated, agents respond to commands prefixed with `*`:
...
Not all IDEs support TRIVIAIOX features equally. See the full comparison at [`docs/ide-integration.md`](./ide-integration.md).
...
| Gemini CLI | Works | `/triviaiox-menu` then `/triviaiox-<agent>` |
| Codex CLI | Limited | `/skills` then `triviaiox-<agent-id>` |
...
Already have a codebase? TRIVIAIOX handles brownfield projects with a dedicated workflow.
...
# Install TRIVIAIOX (non-destructive, preserves your config)
npx triviaiox-core install
...
When you first activate an TRIVIAIOX agent in an existing project:

1. **Detection**: TRIVIAIOX detects code but no TRIVIAIOX docs
...
- **Non-destructive**: TRIVIAIOX creates files, never overwrites existing
- **Rollback**: `git checkout HEAD~1 -- .` restores pre-TRIVIAIOX state
- **Config preservation**: Your `.eslintrc`, `tsconfig.json`, etc. stay intact

### Resources

- **[Working in the Brownfield Guide](.triviaiox-core/working-in-the-brownfield.md)** - Complete brownfield documentation
- **[Compatibility Checklist](.triviaiox-core/development/checklists/brownfield-compatibility-checklist.md)** - Pre/post migration checks
- **[Risk Report Template](.triviaiox-core/product/templates/brownfield-risk-report-tmpl.yaml)** - Phase-by-phase risk assessment
...
All TRIVIAIOX development follows stories in `docs/stories/`. Each story contains:
...
Squads extend TRIVIAIOX beyond software development into any domain. See [Squads Guide](./guides/squads-guide.md).
...
- **[Squads Guide](./guides/squads-guide.md)** - Extend TRIVIAIOX to any domain
- **[Troubleshooting](./troubleshooting.md)** - Common issues and solutions

---

_TriviaGrowth Triviaiox Getting Started Guide v4.2.11_
