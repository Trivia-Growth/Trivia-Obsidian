...
This document explains how to set up and use the cryptographic signing system for TRIVIAIOX-Core install manifests.

## Overview

TRIVIAIOX-Core uses **Ed25519 digital signatures** (via minisign format) to verify the integrity and authenticity of the `install-manifest.yaml` file. This ensures that:
...
│  │ (triviaiox-core.key)  │    │         -s triviaiox-core.key          │  │
│  │ NEVER SHARE!     │    └───────────────────────────────────┘  │
│  └──────────────────┘                    │                       │
│                                          ▼                       │
│                          ┌───────────────────────────────────┐  │
...
minisign -G -p triviaiox-core.pub -s triviaiox-core.key

# You will be prompted for a password to protect the secret key
# CHOOSE A STRONG PASSWORD!

# Output:
#   triviaiox-core.pub  - PUBLIC key (safe to share, will be hardcoded)
#   triviaiox-core.key  - SECRET key (NEVER share, store securely)
```

### 3. View Public Key

```bash
cat triviaiox-core.pub
# Output example:
# untrusted comment: minisign public key TRIVIAIOX0001
...
const PINNED_PUBLIC_KEY = {
  ...
  keyId: 'TRIVIAIOX0001',
  // Base64 public key (the second line of triviaiox-core.pub)
...
- Store `triviaiox-core.key` in a secure location (password manager, HSM, etc.)
...
1. **Generate/Update Manifest**
   ...
   ```bash
   node bin/triviaiox.js manifest:generate
   # Creates .triviaiox-core/install-manifest.yaml with all file hashes
   ```
...
2. **Sign the Manifest**
   ...
   ```bash
   cd .triviaiox-core
   minisign -Sm install-manifest.yaml -s /path/to/triviaiox-core.key
...
3. **Verify Signature (Optional but Recommended)**
   ...
   ```bash
   minisign -Vm install-manifest.yaml -p /path/to/triviaiox-core.pub
...
4. **Commit Both Files**
   ...
   ```bash
   git add .triviaiox-core/install-manifest.yaml
   git add .triviaiox-core/install-manifest.yaml.minisig
...
```bash
minisign -Sm .triviaiox-core/install-manifest.yaml -s /path/to/triviaiox-core.key
```
