...
| `bin/triviaiox-init.js`                               | **Modified** | Added `requireSignature` option          |
...
1. Generate a key pair: `minisign -G -p triviaiox-core.pub -s triviaiox-core.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s triviaiox-core.key`
...
3. **Sign manifest** before each release:
   ...
   ```bash
   minisign -Sm .triviaiox-core/install-manifest.yaml -s triviaiox-core.key
   ```
