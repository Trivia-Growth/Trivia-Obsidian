...
TriviaGrowth Triviaiox 所有元代理命令的完整参考指南。
...
```bash
TRIVIAIOX_AI_PROVIDER=openai          # AI 提供商
TRIVIAIOX_AI_MODEL=gpt-4             # AI 模型
TRIVIAIOX_LOG_LEVEL=debug            # 日志级别
TRIVIAIOX_TELEMETRY=disabled         # 遥测设置
TRIVIAIOX_TIMEOUT=30000             # 命令超时（毫秒）
TRIVIAIOX_MEMORY_CACHE=true         # 内存缓存
```
