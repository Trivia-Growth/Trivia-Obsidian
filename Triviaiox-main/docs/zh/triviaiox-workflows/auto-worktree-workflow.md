...
```mermaid
graph TB
    subgraph TRIVIAIOX["TRIVIAIOX 核心"]
    ...
    subgraph FS["文件系统"]
        WT[.triviaiox/worktrees/{storyId}/<br/>工作树目录]
        LOG[.triviaiox/logs/merges/<br/>合并审计日志]
    end
...
**创建的结构:**
```
.triviaiox/worktrees/{storyId}/     # 工作目录
...
| 属性 | 值 |
|------|-----|
| **位置** | `.triviaiox-core/development/tasks/create-worktree.md` |
| **代理** | @devops (Gage) |
| **版本** | 1.0 |
| **故事** | 1.3 |
...
| **TRIVIAIOX 核心** | 已安装 | 检查 `.triviaiox-core/` |
...
| `WorktreeManager not found` | TRIVIAIOX 不完整 | 重新安装 TRIVIAIOX |
...
3. 增加限制 (如需): 在 `.triviaiox/config.yaml` 中
...
| 文件 | 路径 |
|------|------|
| **工作流定义** | `.triviaiox-core/development/workflows/auto-worktree.yaml` |
| **任务创建** | `.triviaiox-core/development/tasks/create-worktree.md` |
