...
**迁移通知**: 此文档将在 Q2 2026 迁移到 `Trivia-Growth/Triviaiox` 存储库（参见决策 005）
...
- [概述](#概述)
- [当前结构 (triviaiox-core Brownfield)](#当前结构-triviaiox-core-brownfield)
...
## 当前结构 (triviaiox-core Brownfield)

```
triviaiox-core/                             # 根目录 (brownfield 项目)
├── .triviaiox-core/                        # 框架核心 (可移植)
│   ├── core/                          # 框架必要组件 (v4)
│   │   ├── config/                    # 配置系统
│   │   ├── data/                      # 核心知识库
│   │   ├── docs/                      # 核心文档
...
│
├── bin/                               # CLI 可执行文件
│   ├── @trivia-growth/triviaiox-core.js              # 主 CLI 入口点
│   └── triviaiox-minimal.js                # 最小 CLI
│
├── tools/                             # 构建和实用工具
│   ├── cli.js                         # CLI 构建器
...
**目的**: 可在任何 TRIVIAIOX 项目中工作的可移植框架组件。
...
Squad 是添加专用功能到 TRIVIAIOX 的模块化扩展。不同于已弃用的 Squad，Squad 遵循标准化模板结构。
...
### 存储库 1: Trivia-Growth/Triviaiox (MIT)
...
### 存储库 2: Trivia-Growth/squads (MIT)

```
squads/
├── verified/                          # TRIVIAIOX 审核的 Squad
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### 存储库 3: Trivia-Growth/mcp-ecosystem (Apache 2.0)
...
### 存储库 4: Trivia-Growth/certified-partners (私有)
...
### 存储库 5: Trivia-Growth/mmos (私有 + NDA)
...
```yaml
# 我在创建新代理:
位置: .triviaiox-core/agents/{agent-name}.md
示例: .triviaiox-core/agents/security-expert.md

# 我在创建新任务:
位置: .triviaiox-core/tasks/{task-name}.md
示例: .triviaiox-core/tasks/deploy-to-production.md

# 我在创建新工作流:
位置: .triviaiox-core/workflows/{workflow-name}.yaml
示例: .triviaiox-core/workflows/continuous-deployment.yaml

# 我在创建新模板:
位置: .triviaiox-core/product/templates/{template-name}-tmpl.{yaml|md}
示例: .triviaiox-core/product/templates/deployment-plan-tmpl.yaml

# 我在编写故事:
位置: docs/stories/{epic-context}/{story-file}.md
示例: docs/stories/triviaiox migration/story-6.1.2.6.md
...
# 我在创建实用脚本:
位置: .triviaiox-core/utils/{utility-name}.js
示例: .triviaiox-core/utils/performance-monitor.js
...
| 1.1  | 2025-12-14 | 更新 org 为 Trivia-Growth，用 Squad 系统替换 Squads [Story 6.10]    | Dex (dev)       |

---

_这是 TRIVIAIOX 框架的官方模式。所有文件放置应遵循此结构。_
