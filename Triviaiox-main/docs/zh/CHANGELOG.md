...
TriviaGrowth Triviaiox 的所有重要变更都将记录在此文件中。
...
- **TRIVIAIOX 自主开发引擎 (ADE)**：完整的自主开发系统，包含 7 个 Epic：
...
- **代理同步**：所有代理现在在 `.triviaiox-core/development/agents/` 和 `.claude/commands/TRIVIAIOX/agents/` 之间同步
...
- **目录结构**：`.legacy-core/` 重命名为 `.triviaiox-core/`
...
- **IDE 配置**：Claude Code 代理移至 `.claude/commands/TRIVIAIOX/agents/`
- **文件位置**：
...
- 手动安装流程（改用 `npx @trivia-growth/triviaiox-core install`）
...
1. 目录重命名：`.legacy-core/` → `.triviaiox-core/`
...
- [GitHub 仓库](https://github.com/Trivia-Growth/Triviaiox)
- [Issue 追踪器](https://github.com/Trivia-Growth/Triviaiox/issues)
