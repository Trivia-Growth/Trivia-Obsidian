...
# TriviaGrowth Triviaiox 入门指南

> 🌐 [EN](../getting-started.md) | [PT](../pt/getting-started.md) | [ES](../es/getting-started.md) | **ZH**

---

欢迎使用 TriviaGrowth Triviaiox。本指南针对 10 分钟内实现首次价值进行了优化。
...
- Gemini CLI：`/triviaiox-menu` 然后 `/triviaiox-<agent>`
- Codex CLI：`/skills` 然后 `triviaiox-<agent-id>`
- Cursor/Copilot/AntiGravity：遵循 `docs/ide-integration.md` 中的约束

### 步骤 3：验证首次价值

当满足以下 3 个条件时，即实现首次价值：
1. 您激活了一个 TRIVIAIOX 代理。
2. 您收到了有效的问候/激活响应。
...
TRIVIAIOX 配置位于 `.triviaiox-core/core/config/`。安装程序处理初始设置。要验证您的安装：
...
TRIVIAIOX 代理通过您的 IDE 激活。激活后，代理响应以 `*` 为前缀的命令：
...
并非所有 IDE 都同等支持 TRIVIAIOX 功能。请参阅 [`docs/ide-integration.md`](./ide-integration.md) 中的完整比较。
...
| Gemini CLI | 可用 | `/triviaiox-menu` 然后 `/triviaiox-<agent>` |
| Codex CLI | 有限 | `/skills` 然后 `triviaiox-<agent-id>` |
...
当您首次在现有项目中激活 TRIVIAIOX 代理时：

1. **检测**：TRIVIAIOX 检测到代码但没有 TRIVIAIOX 文档
...
- **非破坏性**：TRIVIAIOX 创建文件，从不覆盖现有文件
- **回滚**：`git checkout HEAD~1 -- .` 恢复 TRIVIAIOX 之前的状态
- **配置保留**：您的 `.eslintrc`、`tsconfig.json` 等保持不变

### 资源

- **[棕地项目工作指南](.triviaiox-core/working-in-the-brownfield.md)** - 完整的棕地项目文档
- **[兼容性检查清单](.triviaiox-core/development/checklists/brownfield-compatibility-checklist.md)** - 迁移前/后检查
- **[风险报告模板](.triviaiox-core/product/templates/brownfield-risk-report-tmpl.yaml)** - 分阶段风险评估
...
所有 TRIVIAIOX 开发都遵循 `docs/stories/` 中的故事。每个故事包含：
- 带复选框的验收标准
- 映射到特定 AC 的任务
- CodeRabbit 集成用于自动审查
- 质量门控分配
...
小队将 TRIVIAIOX 扩展到软件开发之外的任何领域。请参阅[小队指南](./guides/squads-guide.md)。
...
- **[小队指南](./guides/squads-guide.md)** - 将 TRIVIAIOX 扩展到任何领域
- **[故障排除](./troubleshooting.md)** - 常见问题和解决方案

---

_TriviaGrowth Triviaiox 入门指南 v4.2.11_
