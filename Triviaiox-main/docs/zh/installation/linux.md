...
# TriviaGrowth Triviaiox Linux 安装指南
...
2. 命令安装到 `.claude/commands/TRIVIAIOX/`
3. 使用 `/agent-name` 激活代理

### VS Code（使用 Continue 扩展）

1. 安装 Continue 扩展
2. 在 `.continue/` 中配置 TRIVIAIOX 规则
...
TriviaGrowth Triviaiox 遵守 XDG 目录：

```bash
# 数据文件：~/.local/share/triviaiox/
# 配置文件：~/.config/triviaiox/
# 缓存：~/.cache/triviaiox/
# 状态：~/.local/state/triviaiox/
```
