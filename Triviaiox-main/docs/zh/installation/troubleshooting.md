...
### 问题 1："npx @trivia-growth/triviaiox-core is not recognized"
...
TriviaGrowth Triviaiox should be installed in your project directory,
...
```
Error: TriviaGrowth Triviaiox requires Node.js 18.0.0 or higher
...
```
npm ERR! code E404
npm ERR! 404 Not Found - GET https://registry.npmjs.org/@trivia-growth/triviaiox-core
```
...
npm ERR! network request to https://registry.npmjs.org/@trivia-growth/triviaiox-core failed
```
...
# 以详细日志重试
npm install @trivia-growth/triviaiox-core --verbose
```
...
```
Error: ENOTEMPTY: directory not empty, rmdir '.triviaiox-core'
```
...
```bash
# 备份现有安装
mv .triviaiox-core .triviaiox-core.backup

# 使用强制标志运行安装程序
npx @trivia-growth/triviaiox-core install --force-upgrade

# 如果需要，从备份恢复自定义文件
cp .triviaiox-core.backup/custom-files/* .triviaiox-core/
```
...
2. 验证文件已创建：
   ...
   ```bash
   # Claude Code
   ls .claude/commands/TRIVIAIOX/agents/
...
```
Error: Agent 'dev' not found in .triviaiox-core/agents/
```
...
cat .triviaiox-core/core-config.yaml
```
...
```bash
# 验证 YAML 语法
npx yaml-lint .triviaiox-core/agents/dev.md
...
# 重新安装以获取干净的代理文件
mv .triviaiox-core/agents/dev.md .triviaiox-core/agents/dev.md.backup
npx @trivia-growth/triviaiox-core install --full
```
...
# 显示详细日志
npx @trivia-growth/triviaiox-core install --verbose
```
...
```bash
# 验证 .triviaiox-core 结构
find .triviaiox-core -type f | wc -l
# 预期：200+ 文件

# 检查损坏的 YAML
for f in .triviaiox-core/**/*.yaml; do npx yaml-lint "$f"; done

# 验证权限
ls -la .triviaiox-core/
```
...
1. 运行 `npx @trivia-growth/triviaiox-core status` 并记录输出
2. 查看此故障排除指南
3. 搜索现有 [GitHub 问题](https://github.com/Trivia-Growth/Triviaiox/issues)
...
- **GitHub Issues**: [@trivia-growth/triviaiox-core/issues](https://github.com/Trivia-Growth/Triviaiox/issues)
