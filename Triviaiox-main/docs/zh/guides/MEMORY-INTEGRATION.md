...
集成遵循 TRIVIAIOX Open Core 模型:
- **triviaiox-core:** UnifiedActivationPipeline 中的扩展点 (本指南)
- **triviaiox-pro:** 记忆智能实现 (检索, 评分, 学习)
...
```javascript
isProAvailable() === true
// 但 .triviaiox/session-digests/ 为空
```
...
```javascript
isProAvailable() === true
// 且 .triviaiox/session-digests/ 包含记忆摘要
```
...
**解决方案:**
1. 减少摘要数量 (将旧摘要归档到 `.triviaiox/session-digests/archive/`)
