...
> **架构:** MIS 遵循 TRIVIAIOX 的 **Open Core** 模型。所有记忆智能都位于私有仓库 `triviaiox-pro`（子模块 `pro/`）中。`triviaiox-core`（开源）仅提供扩展点，并且在没有智能记忆的情况下 100% 正常运行 — 与现在完全一样。
...
当前记忆系统在**两个断开的层**（Claude Code 原生 + TRIVIAIOX 框架）上运行，存在关键缺口：
...
| 层 | 功能 | 仓库 | Story |
|--------|--------|-------------|-------|
| **清理** | 移除死代码，准备基础 | **triviaiox-core** | MIS-2 |
| **捕获** | 通过 hooks 捕获会话知识 | **triviaiox-pro** | MIS-3 |
| **存储** | 使用带 YAML frontmatter 的 Markdown 存储 | **triviaiox-pro** | MIS-3, MIS-4 |
| **检索** | 按相关性渐进式检索 | **triviaiox-pro** + **triviaiox-core** 扩展点 | MIS-4, MIS-6 |
| **演化** | 自动学习和演化规则 | **triviaiox-pro** | MIS-5, MIS-7 |

### 关键原则 (vs 当前状态)

| 方面 | 当前状态 | 使用 MIS (triviaiox-pro 激活) | 没有 MIS (仅 triviaiox-core) |
|---------|-------------|--------------------------|--------------------------|
...
> **triviaiox-core 在没有 MIS 的情况下 100% 正常运行** — 与现在的运行方式完全相同。当 `triviaiox-pro` 存在时（子模块 `pro/`），MIS 通过扩展点自动连接。无需手动配置。

### Open Core 模型

```mermaid
flowchart LR
    subgraph CORE["triviaiox-core (开源, MIT)"]
        direction TB
    ...
    subgraph PRO["triviaiox-pro (私有, 子模块 pro/)"]
        direction TB
...
triviaiox-core 已有模块 `bin/utils/pro-detector.js` (Story PRO-5):
...
#### triviaiox-core (开源) — 仅扩展点

| 文件 | 类型 | 描述 |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | 已存在 | `isProAvailable()`, `loadProModule()` |
| `.triviaiox-core/development/scripts/unified-activation-pipeline.js` | 修改 | 在 Tier 2 添加 `if (isProAvailable())` 用于记忆加载 |
| `.triviaiox-core/development/scripts/greeting-builder.js` | 修改 | 如果 pro 可用，在 greeting 中添加记忆统计 |
| `.triviaiox-core/core/memory/gotchas-memory.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
| `.triviaiox-core/core/session/context-loader.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
| `.claude/settings.json` | 修改 | 调用 `pro/` 脚本的 Hook 桩（如果不存在则优雅失败） |

#### triviaiox-pro (私有) — 所有智能
...
Claude Code 的 hooks 在 `.claude/settings.json` (triviaiox-core) 中配置。指向的脚本位于 `pro/`:
...
```javascript
// .triviaiox-core/hooks/pro-hook-runner.js (在 core 中, ~20 行)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["triviaiox-core (开源)"]
        direction TB
        EXT["管道中的<br/>扩展点"]
    ...
    subgraph PRO_BOUNDARY["triviaiox-pro (私有 — pro/)"]
...
> **仓库:** `triviaiox-pro` (pro/memory/)
> **core 中的扩展点:** `.triviaiox-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (triviaiox-core — 已提交)
{
  "hooks": {
    "PreCompact": [
      {
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["triviaiox-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("持久化到<br/>.triviaiox/memories/")]
    end
...
> **仓库:** `triviaiox-pro` (pro/memory/) 管理存储
> **存储位置:** `.triviaiox/memories/` (gitignored, 由 pro 创建)
...
```
.triviaiox/memories/                          # 由 pro/memory/ 创建和管理
  shared/                                # 跨代理记忆
    durable/                             # 永久知识
...
> **仓库:** `triviaiox-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **仓库:** `triviaiox-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
> **仓库:** `triviaiox-pro` (pro/memory/agent-memory-api.js)
...
每个代理访问：
1. 自己的私有记忆 (`.triviaiox/memories/{agent}/`)
2. 共享记忆 (`.triviaiox/memories/shared/`)
3. **绝不** 其他代理的私有记忆
...
> **100% 在 triviaiox-pro 中** — core 中没有自学习组件
...
**没有 pro:** 仅 `[CORE]` 和 `[原生]` 存在。`[PRO]` (.triviaiox/memories/) 永远不会创建。

---

## 系统文件映射

### triviaiox-core (开源) — 扩展点

| 文件 | 类型 | Story | 描述 |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | 已存在 (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.triviaiox-core/hooks/pro-hook-runner.js` | **新建** | MIS-3 | 包装器: 检查 pro, 委托 hook (~20 行) |
| `.triviaiox-core/development/scripts/unified-activation-pipeline.js` | **修改** | MIS-6 | 在 Tier 2 添加 `if (isProAvailable())` |
| `.triviaiox-core/development/scripts/greeting-builder.js` | **修改** | MIS-6 | 如果 pro 可用，在 greeting 中显示记忆统计 |
| `.triviaiox-core/core/memory/gotchas-memory.js` | **路径修复** | MIS-2 | 修正 `.triviaiox/error-tracking.json` |
| `.triviaiox-core/core/session/context-loader.js` | **路径修复** | MIS-2 | 修正 `.triviaiox/session-state.json` |
...
### 删除的文件 (MIS-2, triviaiox-core)

| 文件 | 行数 | 原因 |
|---------|--------|-------|
| `.triviaiox-core/core/memory/timeline-manager.js` | 746 | 0 个消费者 (孤立) |
| `.triviaiox-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 个消费者 (孤立) |
| `.triviaiox-core/core/memory/context-snapshot.js` | 648 | 0 个消费者 (孤立) |
| `.triviaiox-core/elicitation/session-manager.js` | — | 死代码 |
| `.triviaiox/compound-analysis/` | ~14K | 0 个读取者 (孤立数据) |

---

## 差异对比: 当前状态 vs MIS

| 方面 | 当前 triviaiox-core | triviaiox-core + triviaiox-pro (MIS) | 没有 pro 的 triviaiox-core |
|---------|---------------|---------------------------|------------------|
...
| Story | 标题 | 仓库 | 依赖 | 工时 |
|-------|--------|-------------|------------|-------|
| MIS-1 | 调研与架构设计 | docs (triviaiox-core) | — | 12h (完成) |
| MIS-2 | 死代码清理与路径修复 | **triviaiox-core** | MIS-1 | 4h |
| MIS-3 | 会话摘要 (PreCompact Hook) | **triviaiox-core** (hook runner) + **triviaiox-pro** (digest) | MIS-1 | 14h |
| MIS-4 | 渐进式记忆检索 | **triviaiox-pro** | MIS-3 | 16h |
| MIS-5 | 自学习引擎 | **triviaiox-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | 管道集成与代理记忆 API | **triviaiox-core** (扩展点) + **triviaiox-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md 与规则自动演化 | **triviaiox-pro** | MIS-5 | 8h |
| **总计** | | | | **~78h** |
...
| Epic PRO 架构 | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-triviaiox-pro-architecture/EPIC-PRO-INDEX.md) |
...
*TRIVIAIOX 记忆智能系统 — 架构愿景 v4.0.4 (Core/Pro 分离)*
*Epic MIS 完成后的目标状态 (7 个 stories, ~78 小时)*
*triviaiox-core: 扩展点 + 死代码清理*
*triviaiox-pro: 所有记忆智能 (捕获, 存储, 检索, 演化)*
*@architect (Aria) — 架构未来*
