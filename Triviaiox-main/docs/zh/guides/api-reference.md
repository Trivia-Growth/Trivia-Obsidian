...
TriviaGrowth Triviaiox 完整 API 参考 - AI 编排全栈开发系统。
...
TRIVIAIOX 通过两个主要机制提供统一 API 来与专业 AI 代理交互：
...
| `@triviaiox-master`   | Orion  | Orchestrator| 框架编排、元操作             |

### 激活行为

激活代理时：

1. 从 `.triviaiox-core/development/agents/{id}.md` 加载代理定义文件
2. 采用角色（语气、词汇、问候）
3. 基于会话类型显示上下文问候
4. 代理停止并等待用户输入
...
| 类别                 | 描述                         | 解决方案                           |
| -------------------- | ---------------------------- | ---------------------------------- |
| `AGENT_NOT_FOUND`    | 缺少代理定义                 | 检查 `.triviaiox-core/development/agents/` |
...
/triviaiox-master  → 激活 @triviaiox-master 代理
```
...
_TriviaGrowth Triviaiox API 参考 v4.0.4_
