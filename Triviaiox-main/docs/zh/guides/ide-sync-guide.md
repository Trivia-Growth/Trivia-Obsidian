...
在多个IDE配置中同步TRIVIAIOX代理、任务、工作流和检查清单。

## 概述

`*command` 任务自动化TRIVIAIOX组件到所有配置的IDE目录（`.claude/`、`.cursor/`、`.gemini/` 等）的同步，省去手动复制操作。
...
编辑 `.triviaiox-sync.yaml` 启用您的IDE:
...
**解决方案**: 将别名添加到 `.triviaiox-sync.yaml`:
...
- [TRIVIAIOX架构](../core-architecture.md)
