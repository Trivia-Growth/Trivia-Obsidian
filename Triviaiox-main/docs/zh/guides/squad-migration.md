...
如何将旧版Squad迁移到TRIVIAIOX 2.1格式。

## 概述

TRIVIAIOX 2.1引入了新的Squad格式，具有以下特性：
- 任务优先的架构
- JSON Schema验证
- 三层级分发
- 标准化清单 (`squad.yaml`)
...
| TRIVIAIOX类型字段 | 缺失 | `triviaiox.type: squad` |
| 最低版本 | 缺失 | `triviaiox.minVersion: "2.1.0"` |
| 结构 | Agent优先 | Task优先 |
...
- [ ] `squad.yaml` 存在且有效
- [ ] `triviaiox.type` 是 `"squad"`
- [ ] `triviaiox.minVersion` 是 `"2.1.0"` 或更高
