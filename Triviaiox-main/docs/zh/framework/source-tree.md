...
- [概述](#概述)
- [模块化架构](#模块化架构)
- [框架核心 (.triviaiox-core/)](#框架核心-triviaiox-core)
- [模块详情](#模块详情)
- [文档 (docs/)](#文档-docs)
- [Squads 系统](#squads-系统)
...
TRIVIAIOX 使用**模块化架构**，具有清晰的关注点分离：

1. **框架核心** (`.triviaiox-core/`) - 按领域组织的可移植框架组件
...
│
├── data/                              # 共享数据
│   ├── triviaiox-kb.md                     # TRIVIAIOX 知识库（@triviaiox-master，延迟加载）
...
### 共享数据文件 (.triviaiox-core/data/)

| 文件 | 所有者 | 填充规则 | 更新触发器 | 使用者 |
|------|--------|----------|------------|--------|
| `triviaiox-kb.md` | @triviaiox-master | 框架重大变更时更新 | 手动编辑 | @triviaiox-master (延迟) |
...
### 产品数据文件 (.triviaiox-core/product/data/)
...
│
├── stories/                           # 开发故事
│   ├── triviaiox migration/                # TRIVIAIOX 迁移故事
│   │   ├── story-6.1.2.1.md
│   │   ├── story-6.1.2.2.md
│   │   ├── story-6.1.2.3.md
│   │   ├── story-6.1.2.4.md
...
Squads 是为 TRIVIAIOX 添加专门功能的模块化扩展。与已弃用的 Squads 不同，Squads 遵循标准化的模板结构。
...
### 仓库1: Trivia-Growth/Triviaiox (MIT)

```
triviaiox-core/
├── .triviaiox-core/                        # 框架资产（模块化 v2.0）
│   ├── cli/                           # CLI 命令和实用工具
│   ├── core/                          # 框架基础
│   │   ├── config/                    # 配置系统
│   │   ├── quality-gates/             # 质量验证器
...
### 仓库2: Trivia-Growth/squads (MIT)

```
squads/
├── verified/                          # TRIVIAIOX 认证的 squads
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### 仓库3: Trivia-Growth/mcp-ecosystem (Apache 2.0)
...
### 仓库4: Trivia-Growth/certified-partners (私有)
...
ADE 通过 `.triviaiox-core/core-config.yaml` 配置：
...
| 1.1 | 2025-12-14 | 更新组织为 Trivia-Growth，用 Squads 系统替换 Squads [Story 6.10] | Dex (dev) |
...
| 3.1 | 2026-02-06 | 添加数据文件治理部分：记录7个缺失的数据文件及其所有者、填充规则和更新触发器。扩展 .triviaiox-core/data/ 和 product/data/ 树列表。[Story ACT-8] | Dex (dev) |

---

_这是官方 TRIVIAIOX 框架标准。所有文件放置必须遵循此结构。_
