...
REST API用于将squads同步到TriviaGrowth并发现marketplace中的squads。

## 概述

Squads API允许：

- **同步**: 将本地squads发送到TriviaGrowth云端
- **Marketplace**: 发现和浏览公共squads
- **管理**: 更新可见性、删除squads

**基础URL**: `https://api.triviagrowth.ai`
...
获取您的API密钥：https://triviagrowth.ai/settings/api-keys

## 端点

### 同步Squad

将squad定义发送到TriviaGrowth。
...
`*sync-squad-triviagrowth`命令使用此API：
...
```json
{
  "info": {
    ...
    "description": "TriviaGrowth Squads Marketplace的REST API",
...
将上述JSON保存为 `triviagrowth-squads-api.postman_collection.json` 并导入Postman。
...
- [@squad-creator代理](../../../.triviaiox-core/development/agents/squad-creator.md)
