# TRIVIAIOX Agent Execution Traces - Index

> **Story:** TRIVIAIOX-TRACE-001 | **Traced from source code, not documentation.**

## Overview

This directory contains comprehensive runtime execution trace documentation for all 12 TRIVIAIOX agents. Each document shows **exactly** what files are loaded, in what order, and why - from agent activation through every command execution.
...
| @triviaiox-master | Nova (Orchestrator) | 33 | Direct | [triviaiox-master-execution-trace.md](./triviaiox-master-execution-trace.md) |
...
graph TD
    subgraph "Orchestration Layer"
        MASTER[@triviaiox-master<br/>Nova]
    end
...
|-------|----------|-----------------|--------------|-------------|
| @triviaiox-master | Critical | dataLocation, registry | triviaiox-kb.md (lazy) | <30ms |
...
Source: `.triviaiox-core/data/agent-config-requirements.yaml`
...
| File | Purpose | Lines |
|------|---------|-------|
| `.triviaiox-core/development/scripts/greeting-builder.js` | Main activation pipeline | 949 |
| `.triviaiox-core/development/scripts/agent-config-loader.js` | Config loading per agent | ~400 |
| `.triviaiox-core/development/scripts/generate-greeting.js` | CLI wrapper (3 agents) | 173 |
| `.triviaiox-core/core/session/context-detector.js` | Session type detection | ~100 |
| `.triviaiox-core/infrastructure/scripts/project-status-loader.js` | Git/project status | ~524 |
| `.triviaiox-core/infrastructure/scripts/git-config-detector.js` | Git config detection | ~294 |
| `.triviaiox-core/development/scripts/greeting-preference-manager.js` | Greeting level preference | ~146 |
| `.triviaiox-core/development/scripts/workflow-navigator.js` | Workflow suggestions | ~200 |
| `.triviaiox-core/core/permissions/index.js` | Permission mode badge | ~100 |
| `.triviaiox-core/data/agent-config-requirements.yaml` | Per-agent config requirements | 369 |
...
**Resolution:** The `AgentConfigLoader` resolves dependencies by searching multiple paths: `development/`, `product/`, and root `.triviaiox-core/` directories. The path mismatch does not cause runtime failures but is documented for accuracy.
...
| @triviaiox-master | add-tech-doc.md | `*add-tech-doc` command non-functional |
