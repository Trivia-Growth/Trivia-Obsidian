...
The integration follows the TRIVIAIOX Open Core model:
- **triviaiox-core:** Extension point in UnifiedActivationPipeline (this guide)
- **triviaiox-pro:** Memory intelligence implementation (retrieval, scoring, learning)
...
```yaml
# .triviaiox-core/development/agents/dev.md
agent:
  id: dev
  config:
...
```javascript
isProAvailable() === true
// but .triviaiox/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .triviaiox/session-digests/ contains memory digests
```
...
const fs = require('fs');
const digests = fs.readdirSync('.triviaiox/session-digests');
...
**Solutions:**
1. Reduce digest count (archive old digests to `.triviaiox/session-digests/archive/`)
