...
TRIVIAIOX v3.12+ introduces a 4-level configuration hierarchy that replaces the single `core-config.yaml` file:
...
| **Pro** Extension | `pro/pro-config.yaml` | Submodule | Premium features (triviaiox-pro) |
| **L3** App | `apps/<name>/triviaiox-app.config.yaml` | Committed | Per-app overrides |
...
If `${...}` patterns are found in L1 or L2, `triviaiox config validate` will warn — these files are committed to git and should not contain environment-specific values.
...
To suppress deprecation warnings: `TRIVIAIOX_SUPPRESS_DEPRECATION=1`
...
triviaiox config show --level L2
```
...
triviaiox config validate --level L4
```
...
| Command | Description |
|---------|-------------|
| `triviaiox config show` | Show fully resolved configuration |
| `triviaiox config show --level L2` | Show single level (raw, no merge) |
| `triviaiox config show --debug` | Show with source annotations per value |
| `triviaiox config diff --levels L1,L2` | Compare two levels |
| `triviaiox config migrate` | Migrate monolithic to layered |
| `triviaiox config migrate --dry-run` | Preview migration without changes |
| `triviaiox config validate` | Validate YAML syntax and patterns |
| `triviaiox config init-local` | Create local-config.yaml from template |
...
**Q: How does triviaiox-pro config work?**
