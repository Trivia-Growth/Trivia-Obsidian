...
4. [Camada 2: TRIVIAIOX Framework](#camada-2-triviaiox-framework)
...
O sistema de memoria do TRIVIAIOX opera em **duas camadas independentes** que coexistem mas **nao se comunicam entre si**:

| Camada | Gerenciado Por | Escopo |
|--------|---------------|--------|
| **Camada 1: Claude Code Nativo** | Claude Code CLI | Auto Memory, CLAUDE.md, Session Transcripts |
| **Camada 2: TRIVIAIOX Framework** | Scripts JS em `.triviaiox-core/` | Gotchas, Session State, Context Snapshots, Timeline |
...
- **Cada camada persiste de forma independente** — `.claude/` vs `.triviaiox/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>triviaiox-core/.claude/CLAUDE.md"]
    ...
    subgraph TRIVIAIOX_FRAMEWORK["Camada 2: TRIVIAIOX Framework"]
        direction TB
        GOTCHAS["Gotchas Memory<br/>.triviaiox-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot<br/>.triviaiox-core/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker<br/>.triviaiox-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.triviaiox-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.triviaiox-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.triviaiox-core/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.triviaiox-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.triviaiox-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.triviaiox-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_TRIVIAIOX["Storage TRIVIAIOX (.triviaiox/)"]
        S_GOTCHAS[".triviaiox/gotchas.json + .md"]
        S_ERRORS[".triviaiox/error-tracking.json"]
        S_SNAPSHOTS[".triviaiox/snapshots/"]
        S_TIMELINE[".triviaiox/timeline/"]
        S_FILE_EVO[".triviaiox/file-evolution/"]
        S_SESSION[".triviaiox/session-state.json"]
    ...
    style TRIVIAIOX_FRAMEWORK fill:#FFF3E0,stroke:#E65100
    style STORAGE_CLAUDE fill:#E8F5E9,stroke:#2E7D32
    style STORAGE_TRIVIAIOX fill:#FCE4EC,stroke:#C62828
```
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.triviaiox-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.triviaiox-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.triviaiox-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.triviaiox-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.triviaiox-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. triviaiox-core/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. triviaiox-core/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart LR
    subgraph AGENT_MEM[".claude/agent-memory/"]
        AM1["triviaiox-architect/MEMORY.md"]
        AM2["triviaiox-dev/MEMORY.md"]
    ...
    NOTE["NOTA: Apenas agentes<br/>definidos em .claude/agents/<br/>usam esta memoria.<br/>Agentes TRIVIAIOX (.triviaiox-core/<br/>development/agents/)<br/>NAO usam."]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.triviaiox/gotchas.json<br/>.triviaiox/gotchas.md<br/>.triviaiox/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.triviaiox/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.triviaiox/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.triviaiox/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .triviaiox/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.triviaiox/session-state.json<br/>TTL: 1 hora")]
...
sequenceDiagram
    ...
    Note over CC: NENHUMA memoria TRIVIAIOX<br/>e carregada automaticamente<br/>no system prompt.<br/>Apenas contexto de sessao<br/>aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".triviaiox/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".triviaiox/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".triviaiox/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".triviaiox/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".triviaiox/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".triviaiox/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .triviaiox/session-state.json
    ...
    CL->>FS: Escreve .triviaiox/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .triviaiox/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .triviaiox/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .triviaiox/gotchas.md"]
...
classDiagram
    class GotchasStore {
        +string schema = "triviaiox-gotchas-memory-v1"
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.triviaiox/file-evolution/"]
        SRC2["ContextSnapshot<br/>.triviaiox/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".triviaiox/timeline/<br/>unified-timeline.json")]
    end
...
```mermaid
flowchart LR
    subgraph TRIVIAIOX_EVENTS["Eventos TRIVIAIOX"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
|---------|--------|------------|--------|
| `.triviaiox-core/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.triviaiox-core/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.triviaiox-core/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.triviaiox-core/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.triviaiox-core/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.triviaiox-core/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.triviaiox-core/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.triviaiox-core/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.triviaiox-core/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.triviaiox-core/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.triviaiox-core/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |

### Hooks (persistencia de sessao)

| Arquivo | CLI | Funcao |
|---------|-----|--------|
| `.triviaiox-core/hooks/gemini/session-start.js` | Gemini | Carrega contexto TRIVIAIOX no inicio da sessao |
| `.triviaiox-core/hooks/gemini/session-end.js` | Gemini | Persiste sumario da sessao em `.triviaiox/sessions/` |
| `.triviaiox-core/hooks/gemini/before-agent.js` | Gemini | Pre-processamento antes de agente |
| `.triviaiox-core/hooks/gemini/before-tool.js` | Ambos | Pre-processamento antes de tool |
| `.triviaiox-core/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.triviaiox-core/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.triviaiox-core/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.triviaiox-core/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `triviaiox-core/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `triviaiox-core/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
```
.triviaiox/                                        # Runtime state (gitignored)
...
├── triviaiox-architect/MEMORY.md
├── triviaiox-dev/MEMORY.md
...
Os modulos TRIVIAIOX (gotchas, snapshots, timeline) **nao fazem flush** automatico ao final da sessao. Dados em memoria podem se perder se o processo terminar abruptamente.
...
Claude auto-memory (`MEMORY.md`) e TRIVIAIOX memory (`.triviaiox/`) nunca se sincronizam. Gotchas capturadas pelo TRIVIAIOX nao aparecem no MEMORY.md e vice-versa.
...
| Gotchas Memory Script | `.triviaiox-core/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.triviaiox-core/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.triviaiox-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.triviaiox-core/core/memory/timeline-manager.js` |
| Session Context Loader | `.triviaiox-core/core/session/context-loader.js` |
| Context Detector | `.triviaiox-core/core/session/context-detector.js` |
| Session State | `.triviaiox-core/core/orchestration/session-state.js` |
| Unified Hook Interface | `.triviaiox-core/hooks/unified/hook-interface.js` |
| Hook Registry | `.triviaiox-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.triviaiox-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.triviaiox-core/hooks/gemini/session-end.js` |
| Core Config | `.triviaiox-core/core-config.yaml` |
| Claude Settings | `~/.claude/settings.json` |
| Story Development Cycle | `docs/guides/workflows/STORY-DEVELOPMENT-CYCLE-WORKFLOW.md` |

---

*TRIVIAIOX Memory System Architecture Guide v1.0*
