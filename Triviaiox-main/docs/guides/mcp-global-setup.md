# TRIVIAIOX MCP Global Setup Guide

> **EN** | [PT](../pt/guides/mcp-global-setup.md) | [ES](../es/guides/mcp-global-setup.md)

---

> Configure global MCP (Model Context Protocol) servers for TriviaGrowth Triviaiox.
...
The MCP Global System allows you to configure MCP servers once and share them across all TRIVIAIOX projects. This eliminates the need to configure the same servers in every project.
...
- `~/.triviaiox/` - Global TRIVIAIOX directory
- `~/.triviaiox/mcp/` - MCP configuration directory
- `~/.triviaiox/mcp/global-config.json` - Main config file
- `~/.triviaiox/mcp/servers/` - Individual server configs
- `~/.triviaiox/mcp/cache/` - Response cache
- `~/.triviaiox/credentials/` - Secure credential storage
...
TRIVIAIOX includes templates for popular MCP servers:
...
Credentials are stored in `~/.triviaiox/credentials/` with a `.gitignore` to prevent accidental commits.
...
| Directory exists  | Use `triviaiox mcp setup --force` to recreate                          |
...
_TriviaGrowth Triviaiox v4 MCP Global Setup Guide_
