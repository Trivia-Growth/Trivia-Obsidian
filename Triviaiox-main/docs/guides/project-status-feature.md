...
The Project Status feature automatically displays your current work context when any TRIVIAIOX agent activates. This includes:
...
- **TRIVIAIOX-FullStack** framework installed
...
3. Create `.triviaiox/project-status.yaml` cache file
...
1. Edit `.triviaiox-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
projectStatus:
  ...
  statusFile: .triviaiox/project-status.yaml  # Cache file location
...
1. **Checks cache** - Looks for `.triviaiox/project-status.yaml`
...
- **Cache Location:** `.triviaiox/project-status.yaml`
...
All 11 TRIVIAIOX agents display project status:
...
3. Does `.triviaiox-core/infrastructure/scripts/project-status-loader.js` exist?
...
A: Yes, all agents use the same cache file (`.triviaiox/project-status.yaml`).
