# TRIVIAIOX Pro Developer Workflow

This guide covers the two developer workflows for working with the TRIVIAIOX Open Core architecture.

## Architecture Overview

TRIVIAIOX uses a dual-repository Open Core model:

- **triviaiox-core** (public) — Open-source framework, available to everyone
- **triviaiox-pro** (private) — Premium features, available to team members and licensees

The `pro/` directory in triviaiox-core is a git submodule pointing to the triviaiox-pro repository.
...
├── src/
├── packages/
├── pro/ ─── git submodule ──► Trivia-Growth/triviaiox-pro (private)
├── squads/ (community)
└── package.json
```
...
- You do NOT need access to Trivia-Growth/triviaiox-pro

---

## Workflow 2: Pro Developer (Team Member with Access)

Team members with access to the private triviaiox-pro repository can work on both repos.
...
2. Push `triviaiox-core` changes second: `cd .. && git push`

This ensures the submodule pointer in triviaiox-core references a valid commit in triviaiox-pro.
...
if (isProAvailable()) {
  console.log('TRIVIAIOX Pro v' + getProVersion() + ' detected');
...
|------------|----------|-------|---------|
| **triviaiox-core** | Without submodules | Core-only (pro/ absent) | npm (excludes pro/) |
| **triviaiox-pro** | With triviaiox-core cloned | Integration (pro/ symlinked) | GitHub Packages |
...
You need access to `Trivia-Growth/triviaiox-pro`. Contact the team lead.
...
- [Story PRO-5: Repository Bootstrap](../../stories/epics/epic-pro-triviaiox-pro-architecture/story-pro-5-repo-bootstrap.md)
