...
Synchronize TRIVIAIOX agents, tasks, workflows, and checklists across multiple IDE configurations.

## Overview

The `*command` task automates the synchronization of TRIVIAIOX components to all configured IDE directories (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminating manual copy operations.
...
Edit `.triviaiox-sync.yaml` to enable your IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Read .triviaiox-sync.yaml configuration               │
...
```
your-project/
├── .triviaiox-sync.yaml           # Sync configuration
...
**Solution**: Add the alias to `.triviaiox-sync.yaml`:
...
- [TRIVIAIOX Architecture](../core-architecture.md)
