...
> **ARQUITETURA:** O MIS segue o modelo **Open Core** do TRIVIAIOX. Toda a inteligencia de memoria reside no repositorio privado `triviaiox-pro` (submodule `pro/`). O `triviaiox-core` (open source) fornece apenas extension points e funciona 100% sem memoria inteligente — exatamente como hoje.
...
O sistema de memoria atual opera em **duas camadas desconectadas** (Claude Code nativo + TRIVIAIOX Framework) com gaps criticos:
...
|--------|--------|-------------|-------|
| **Cleanup** | Remover dead code, preparar base | **triviaiox-core** | MIS-2 |
| **Capture** | Captura conhecimento de sessao via hooks | **triviaiox-pro** | MIS-3 |
| **Storage** | Armazena em Markdown com frontmatter YAML | **triviaiox-pro** | MIS-3, MIS-4 |
| **Retrieval** | Recupera por relevancia com progressive disclosure | **triviaiox-pro** + extension points em **triviaiox-core** | MIS-4, MIS-6 |
| **Evolution** | Aprende e evolui regras automaticamente | **triviaiox-pro** | MIS-5, MIS-7 |

### Principios Chave (vs Estado Atual)

| Aspecto | Estado Atual | Com MIS (triviaiox-pro ativo) | Sem MIS (triviaiox-core only) |
|---------|-------------|--------------------------|--------------------------|
...
> **triviaiox-core funciona 100% sem o MIS** — exatamente como funciona hoje. Quando `triviaiox-pro` esta presente (submodule `pro/`), o MIS se conecta automaticamente via extension points. Zero configuracao manual.

### Modelo Open Core

```mermaid
flowchart LR
    subgraph CORE["triviaiox-core (Open Source, MIT)"]
        direction TB
    ...
    subgraph PRO["triviaiox-pro (Privado, Submodule pro/)"]
        direction TB
...
O triviaiox-core ja possui o modulo `bin/utils/pro-detector.js` (Story PRO-5):
...
#### triviaiox-core (open source) — Extension Points Only

| Arquivo | Tipo | Descricao |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | Existente | `isProAvailable()`, `loadProModule()` |
| `.triviaiox-core/development/scripts/unified-activation-pipeline.js` | Modificacao | Adiciona `if (isProAvailable())` no Tier 2 para memory loading |
| `.triviaiox-core/development/scripts/greeting-builder.js` | Modificacao | Adiciona memory stats no greeting se pro disponivel |
| `.triviaiox-core/core/memory/gotchas-memory.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.triviaiox-core/core/session/context-loader.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.claude/settings.json` | Modificacao | Hook stubs que chamam scripts em `pro/` (graceful fail se ausente) |

#### triviaiox-pro (privado) — Toda a Inteligencia
...
Os hooks do Claude Code sao configurados em `.claude/settings.json` (triviaiox-core). O script apontado reside em `pro/`:
...
```json
{
  "hooks": {
    "PreCompact": [
      {
        "type": "command",
        "command": "node .triviaiox-core/hooks/pro-hook-runner.js PreCompact",
        "timeout": 10000
      }
    ]
  }
...
```javascript
// .triviaiox-core/hooks/pro-hook-runner.js (no core, ~20 linhas)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
### 8. Open Core (TRIVIAIOX PRO pattern)
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["triviaiox-core (Open Source)"]
        direction TB
        EXT["Extension Points<br/>in Pipeline"]
        HOOK_RUNNER[".triviaiox-core/hooks/<br/>pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js<br/>(standalone)"]
        CTX_LOADER["context-loader.js<br/>(standalone)"]
    end

    subgraph PRO_BOUNDARY["triviaiox-pro (Privado — pro/)"]
        ...
        subgraph STORAGE["CAMADA 2: STORAGE"]
            ST1[".triviaiox/memories/shared/"]
            ST2[".triviaiox/memories/{agent}/"]
            ST3[".triviaiox/memories/index.json"]
        end
...
### Fluxo Sem Pro (triviaiox-core standalone)
...
```mermaid
flowchart LR
    subgraph CORE["triviaiox-core"]
        HR[".triviaiox-core/hooks/<br/>pro-hook-runner.js"]
        UAP["unified-activation-<br/>pipeline.js"]
        GM["gotchas-memory.js"]
        PD["bin/utils/<br/>pro-detector.js"]
    end

    subgraph PRO["triviaiox-pro (pro/)"]
...
> **Repositorio:** `triviaiox-pro` (pro/memory/)
> **Extension point no core:** `.triviaiox-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (triviaiox-core — committed)
{
  "hooks": {
    "PreCompact": [
      {
    ...
    "Stop": [
      {
        "type": "command",
        "command": "node .triviaiox-core/hooks/pro-hook-runner.js Stop",
        "timeout": 5000
      }
    ],
    "PostToolUseFailure": [
      {
        "type": "command",
        "command": "node .triviaiox-core/hooks/pro-hook-runner.js PostToolUseFailure",
        "timeout": 3000
      }
    ]
  }
...
```mermaid
flowchart TD
    subgraph CORE_HOOKS["triviaiox-core"]
        HR["pro-hook-runner.js"]
    end

    subgraph PRO_CAPTURE["triviaiox-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to<br/>.triviaiox/memories/")]
    end
...
> **Repositorio:** `triviaiox-pro` (pro/memory/) gerencia o storage
> **Storage location:** `.triviaiox/memories/` (gitignored, criado pelo pro)
...
```
.triviaiox/memories/                          # Criado e gerenciado por pro/memory/
...
> **Repositorio:** `triviaiox-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **Repositorio:** `triviaiox-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js<br/>(triviaiox-core)
    participant PD as pro-detector.js<br/>(triviaiox-core)
    participant SD as session-digest.js<br/>(triviaiox-pro)
    participant FS as .triviaiox/memories/
...
> **Repositorio:** `triviaiox-pro` (pro/memory/agent-memory-api.js)
...
Cada agente acessa:
1. Suas proprias memorias privadas (`.triviaiox/memories/{agent}/`)
2. Memorias compartilhadas (`.triviaiox/memories/shared/`)
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline<br/>(triviaiox-core)
    participant PD as pro-detector.js<br/>(triviaiox-core)
    participant ML as memory-loader.js<br/>(triviaiox-pro)
    participant MR as memory-retriever.js<br/>(triviaiox-pro)
    participant GB as GreetingBuilder<br/>(triviaiox-core)
...
### Especificacao (arquivos em .triviaiox/memories/)
...
# Always Use Absolute Imports in TRIVIAIOX
...
> **100% em triviaiox-pro** — nenhum componente de self-learning no core
...
→ PROPOSTA: Adicionar a rules/: "Warning: .triviaiox/ permissions"
...
**Sem pro:** Apenas `[CORE]` e `[NATIVO]` existem. `[PRO]` (.triviaiox/memories/) nunca e criado.

---

## Mapa de Arquivos do Sistema

### triviaiox-core (Open Source) — Extension Points

| Arquivo | Tipo | Story | Descricao |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | Existente (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.triviaiox-core/hooks/pro-hook-runner.js` | **Novo** | MIS-3 | Wrapper: verifica pro, delega hook (~20 linhas) |
| `.triviaiox-core/development/scripts/unified-activation-pipeline.js` | **Modificacao** | MIS-6 | Adiciona `if (isProAvailable())` no Tier 2 |
| `.triviaiox-core/development/scripts/greeting-builder.js` | **Modificacao** | MIS-6 | Memory stats no greeting se pro disponivel |
| `.triviaiox-core/core/memory/gotchas-memory.js` | **Path fix** | MIS-2 | Corrige `.triviaiox/error-tracking.json` |
| `.triviaiox-core/core/session/context-loader.js` | **Path fix** | MIS-2 | Corrige `.triviaiox/session-state.json` |
...
### Arquivos Removidos (MIS-2, triviaiox-core)

| Arquivo | Linhas | Razao |
|---------|--------|-------|
| `.triviaiox-core/core/memory/timeline-manager.js` | 746 | 0 consumers (orphan) |
| `.triviaiox-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 consumers (orphan) |
| `.triviaiox-core/core/memory/context-snapshot.js` | 648 | 0 consumers (orphan) |
| `.triviaiox-core/elicitation/session-manager.js` | — | Dead code |
| `.triviaiox/compound-analysis/` | ~14K | 0 readers (orphan data) |

---

## Diferencas: Estado Atual vs MIS

| Aspecto | triviaiox-core hoje | triviaiox-core + triviaiox-pro (MIS) | triviaiox-core sem pro |
|---------|---------------|---------------------------|------------------|
...
|-------|--------|-------------|------------|-------|
| MIS-1 | Investigation & Architecture Design | docs (triviaiox-core) | — | 12h (Done) |
| MIS-2 | Dead Code Cleanup & Path Repair | **triviaiox-core** | MIS-1 | 4h |
| MIS-3 | Session Digest (PreCompact Hook) | **triviaiox-core** (hook runner) + **triviaiox-pro** (digest) | MIS-1 | 14h |
| MIS-4 | Progressive Memory Retrieval | **triviaiox-pro** | MIS-3 | 16h |
| MIS-5 | Self-Learning Engine | **triviaiox-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | Pipeline Integration & Agent Memory API | **triviaiox-core** (ext points) + **triviaiox-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md & Rules Auto-Evolution | **triviaiox-pro** | MIS-5 | 8h |
| **Total** | | | | **~78h** |
...
| Epic PRO Architecture | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-triviaiox-pro-architecture/EPIC-PRO-INDEX.md) |
...
*TRIVIAIOX Memory Intelligence System — Architecture Vision v4.0.4 (Core/Pro Split)*
*Target state apos Epic MIS completo (7 stories, ~78 horas)*
*triviaiox-core: extension points + dead code cleanup*
*triviaiox-pro: toda a inteligencia de memoria (Capture, Storage, Retrieval, Evolution)*
