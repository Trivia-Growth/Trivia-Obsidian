...
### Why it matters for TRIVIAIOX

The TRIVIAIOX Unified Activation Pipeline runs `git status` during agent activation to detect modified files and project state. On large repositories, this can cause the `projectStatus` loader to timeout (~60% timeout rate observed). Enabling fsmonitor dramatically reduces this latency.
...
Run `triviaiox doctor` to check fsmonitor status:
...
_This is an official TRIVIAIOX framework standard._
