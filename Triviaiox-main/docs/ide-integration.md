...
Guide for integrating TRIVIAIOX with supported IDEs and AI development platforms.
...
## Compatibility Contract (TRIVIAIOX 4.2.11)

The IDE matrix is enforced by a versioned contract:

- Contract file: `.triviaiox-core/infrastructure/contracts/compatibility/triviaiox-4.2.11.yaml`
...
TRIVIAIOX supports multiple AI-powered development platforms. Choose the one that best fits your workflow.

### Quick Status Matrix (TRIVIAIOX 4.2.11)
...
| Gemini CLI | Works | `/triviaiox-menu` then `/triviaiox-<agent>` | Works (minor differences in event handling) | -- |
| Codex CLI | Limited | `/skills` then `triviaiox-<agent-id>` | Limited (some checks need manual sync) | Run `npm run sync:ide:codex` and follow `/skills` flow |
...
Legend:
- `Works`: fully recommended for new users in TRIVIAIOX 4.2.11.
...
**Recommendation Level:** Best TRIVIAIOX integration

```yaml
config_file: .claude/CLAUDE.md
agent_folder: .claude/commands/TRIVIAIOX/agents
...
1. TRIVIAIOX automatically creates `.claude/` directory on init
...
# Verify setup
ls -la .claude/commands/TRIVIAIOX/agents/
```
...
special_features:
  ...
  - /skills activators (triviaiox-<agent-id>)
...
4. Use `/skills` and choose `triviaiox-architect`, `triviaiox-dev`, etc.
...
1. TRIVIAIOX creates `.cursor/` directory on init
...
2. TRIVIAIOX creates `.github/copilot-instructions.md`
...
1. TRIVIAIOX creates `.antigravity/` directory
...
agent_folder: .gemini/rules/TRIVIAIOX/agents
...
2. TRIVIAIOX creates:
   - `.gemini/rules.md`
   - `.gemini/rules/TRIVIAIOX/agents/*.md`
   - `.gemini/commands/*.toml` (`/triviaiox-menu`, `/triviaiox-<agent>`)
   - `.gemini/hooks/*.js`
...
4. Quick agent activation (recommended):
   - `/triviaiox-menu` to list shortcuts
   - `/triviaiox-dev`, `/triviaiox-architect`, `/triviaiox-qa`, etc.
   - `/triviaiox-agent <agent-id>` for generic launcher
...
TRIVIAIOX maintains a single source of truth for agent definitions and synchronizes them to all configured IDEs:

```
┌─────────────────────────────────────────────────────┐
│                    TRIVIAIOX Core                         │
│  .triviaiox-core/development/agents/  (Source of Truth)  │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
TRIVIAIOX can be configured to automatically sync on agent changes:

```yaml
# .triviaiox-core/core/config/sync.yaml
auto_sync:
  enabled: true
  watch_paths:
...
ls .triviaiox-core/development/agents/
...
ls .claude/commands/TRIVIAIOX/agents/       # Claude Code
ls .gemini/rules/TRIVIAIOX/agents/          # Gemini CLI
```
...
├── Yes --> Claude Code (Best TRIVIAIOX integration)
└── No
...
# Verify migration
diff -r ./rules-backup/ .claude/commands/TRIVIAIOX/agents/
```
...
_TriviaGrowth Triviaiox IDE Integration Guide v4.2.11_
