...
| E43 | Jornadas & Fluxos — TODO follow-up automático de lead num lugar só: fundação do motor de flows (save órfã enrollments, gatilhos fantasma, retry/quiet-hours/crons), entradas (new_lead/lp_lead, lista/massa, engajamento de e-mail), réguas+jornada de lançamento, routing+SLA+cadência, pressão de contato global | 📋 Draft (aberto 12/07 — 12 stories 43.1–43.12 no repo; base = mapeamento profundo c/ verificação adversarial) |
| E44 | Autoria Profissional (construtores de e-mail & LP) — casco multi-marca, preview/test-send fiéis, brand kit + modelos da casa, biblioteca pro; LP: duplicar/salvar-como-modelo/rollback UI/form rico/DTR+A/B | 📋 Draft (aberto 12/07 — 7 stories 44.1–44.7 no repo) |
| E45 | Aquisição & Medição (mini) — CAPI Lead no submit de LP (pixel bloqueado por design), obrigado c/ repasse de UTMs/fbclid, UTM builder, ROI de e-mail | 📋 Draft (aberto 12/07 — 4 stories 45.1–45.4 no repo) |
| E46 | Identidade Microsoft & Colaboração — login no OS c/ "Entrar com Microsoft" (Entra + MFA Authenticator) servindo chat da equipe (Teams real, identidade própria), calendário Outlook e reuniões Teams no OS; cofre de tokens cifrado (corrige texto puro do meetings); alertas migrados (webhooks Teams MORTOS 05/2026 — verificar TEAMS_WEBHOOK_URL!) | 📋 Draft v2 (reescopado 12/07 — 14 stories 46.1–46.14 no repo, padrão profundo "qualquer dev implementa"; API US$ 0) |
