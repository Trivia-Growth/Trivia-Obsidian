...
### 2026-06-21 — João
- **Story 6.15 (Fila não-atribuídas + distribuição) — Done** (PR #73, mergeado; **migration aplicada + functions deployadas**): tabela `conversation_assignments` (auditoria) + trigger SECURITY DEFINER + função `crm-conversation-distribute` (ródizio ponderado sob demanda, papel manager/admin, número dedicado, UPDATE anti-corrida, dry_run). Webhooks Meta/Z-API atribuem número dedicado. UI: botão "Distribuir N da fila" (reusa filtro da 6.13) + badge de sugestão. **Security gate adversarial: PASS** (7 vetores). Fila → **6.16** (escalonamento IA→humano, última da Onda 2).
