...
### 2026-06-21 — João
- **🧹 Pendências em aberto resolvidas (PR #75, em `develop`)** — fechei 3 dos 5 follow-ups; os 2 restantes dependem do painel/secret:
  ...
  - **⏳ Cron do sweep (6.16) — pronto, falta wirar** (precisa do `CRON_SECRET`, ação @devops). SQL pronto pra colar no SQL editor do Supabase (mesmo padrão dos jobs 2/3), trocando `<CRON_SECRET>` pelo valor real (Dashboard → Edge Functions → Secrets → `CRON_SECRET`):
    ```sql
    select cron.schedule('crm-ai-escalate-sweep-10min', '*/10 * * * *', $$
      select net.http_post(
        url     := 'https://ouvfthknhqcciuothrqb.supabase.co/functions/v1/crm-ai-escalate',
        headers := jsonb_build_object('Content-Type','application/json','x-cron-secret','<CRON_SECRET>'),
        body    := jsonb_build_object('mode','sweep')
      );
    $$);
    ```
  - **⏳ Rotação de secrets expostos (chip do vault) — ação sua, JG.** Continua pendente: os segredos mascarados no vault (Supabase DB pw/PAT, Tray, Cloudfy, Auvo, Literarius) + o `SUPABASE_ACCESS_TOKEN` exposto no chat ([[project_heziomos_crons]]) precisam ser **rotacionados no painel** — não dá pra fazer em código.
