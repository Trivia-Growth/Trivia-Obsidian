...
## 5. Lista Flowbiz "LP - Bíblia 120 anos" (ID 27562) — STATUS

A função grava o lead via API Mailclick (`Subscriber.Subscribe`). **Configuração concluída** (lista + AutoResponder do cupom IPP120 ✅).

**Campos da lista 27562** (todos globais, confirmados via API):

| ID | Campo | Usado na LP? |
|---|---|---|
| 40612 | Nome | ✅ sim (gravado) |
| 40613–40617 | Cidade, Estado, CEP, Data de Nascimento, Gênero | não (LP só coleta e-mail + nome) |
| 50847 | Telefone | não |
| 50848 | E-mail | (e-mail principal) |

> ⚠️ **A lista NÃO tem campo "Fonte"/"Combo".** Os IDs `58000/58001` eram da lista do Plano Bomba e foram **removidos** do `leads.js` (estavam sendo descartados pela Flowbiz). A **origem do lead** (UTM/fonte) segue capturada no **Meta CAPI** (`content_category`) e no **GA4** (`event_source`) — onde de fato importa para otimização de campanha.
>
> **Se quiser a origem também no CRM (segmentação por e-mail):** criar um campo custom "Fonte" na lista 27562 e mapear o ID no `leads.js` (há comentário no ponto exato). Caso contrário, nada a fazer.
