...
- `2026-06-11` — **STORY-108** — **caixa do texto não era salva**. E2E no banco revelou `uppercase_mode` **NULL em todos os briefs** (inclusive salvos hoje), enquanto `cover_uppercase` gravava. Causa: o upsert do hook `useImageBrief.saveBrief` listava colunas explícitas e **esquecia `uppercase_mode`** → campo descartado → composição caía no fallback legado (internos sempre maiúsculos). Fix no hook (upsert + interface + leitura). Lógica do `buildEditorialSpec` já estava certa/testada. Coluna confirmada persistindo (write+read).
- `2026-06-12` — **STORY-109** — **Fotográfico Premium/editorial em imagem única dava 400**. Logs: `compose-editorial-carousel` 400 sem logar → validação precoce. Banco: conteúdo era `static` (imagem única), `generated_content` sem array `slides` (só `headline`/`copy`/...). O compositor exigia `gc.slides` (feito p/ carrossel). Fix: monta **1 slide (capa)** a partir de headline+copy quando não há slides → estilos compostos por código funcionam em imagem única; carrosséis inalterados.
- `2026-06-12` — **STORY-110** — **Fotográfico Realista (post estático) renderizava só o título**. Logs: `Conteúdo single: texto extraído (35 chars)`. Causa: na extração de texto do `with_text`, o caminho single lia o corpo de `body`/`text` (vazios) — o gerador salva o corpo em **`copy`** (+ `cta`). Fix: corpo = `body||copy||text||subtitle` + anexa `cta` → imagem traz título+copy+cta. Carrosséis (slides[]) inalterados.
- `2026-06-12` — **STORY-111** — **Fotográfico Premium ganhou composição própria** (antes era idêntico ao editorial). Direção do piloto: foto full-bleed, **foco à direita / texto à esquerda**. Novo layout `photo_left` no render (scrim horizontal escurecendo a esquerda + coluna de texto à esquerda centralizada na vertical, sem cabeçalho/contador/progresso); coluna estreita (540) no auto-ajuste do título (`lineW`); corpo como parágrafos; prompt da foto pede foco à direita/zona calma à esquerda. 32 testes verdes. **Aguardando validação visual** do piloto.
- `2026-06-25` — **STORY-122** — **rejeição bancária em assinatura recorrente bloqueia acesso**. A Heziom teve a recorrência recusada pelo banco (AppMax, R$997, 22/06) e seguiu acessando. Causa: a AppMax envia `SubscriptionDelayedEvent` tanto p/ atraso quanto p/ rejeição bancária; o STORY-059 mapeou esse evento p/ `subscriptionStatus: null` (atraso ≠ recusa) e o `resolveEvent` retornava no bloco canônico **antes** do fallback que trata "recusado". Fix: quando o evento canônico é `null` mas o `statusHint` (`data.status`) contém recusa (`recus/reject/not_authorized`), eleva p/ `payment_failed`; + entradas diretas no mapa canônico e PT-BR. Heziom corrigida no banco (`status=payment_failed`, acesso cortado na hora). `appmax-webhook` deployada.
- `2026-06-25` — **STORY-123** — **geração de imagem 100% quebrada (content_filter)**. Logs do OpenRouter: TODAS as gerações do `gemini-3-pro-image-preview` (Nano Banana Pro) terminando em `finish_reason: content_filter`, 0 tokens, $0 — enquanto texto (Gemini 2.5) seguia normal via Vertex. Causa: o **Google AI Studio** passou a recusar 100% das imagens (mudança de política num modelo "preview"), e `openrouter-provider.ts` forçava **só** AI Studio sem fallback. O fallback de provider do OpenRouter só dispara em **erro** (5xx) — `content_filter` volta como HTTP 200, então não caía sozinho. Fix: `ai-gateway` detecta o bloqueio e re-tenta a MESMA requisição fixada no `google-vertex`; provider dos modelos de imagem afrouxado p/ `order=[ai-studio,vertex]+allow_fallbacks`. Helper compartilhado `retryImageOnVertexIfFiltered` estende o mesmo retry às funções de fetch próprio (demo-image, style-example, blog-cover). 7 funções de imagem redeployadas. **E2E pendente**: confirmar numa geração real que o provider vira Google Vertex e a imagem volta.
