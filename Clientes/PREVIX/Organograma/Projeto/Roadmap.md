...
- ✅ **STORY-019 — Defesa total contra ciclos no `secondary_manager_id`.** Implementada em 2026-05-08. Função SQL `check_secondary_manager_cycle` (CTE recursiva descendente) + trigger `trg_pessoas_secondary_no_cycle` (BEFORE INSERT/UPDATE em `pessoas`). Edge Function `validate-and-set-secondary-manager` (admin/editor only) chama a RPC e dá resposta amigável antes do trigger. PessoasPage migrado pra salvar `secondary_manager_id` em passo separado via essa EF. Smoke test inline na migration confirma rejeição de ciclo depth-1 e depth-2.
