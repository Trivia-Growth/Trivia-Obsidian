...
**Arquitetura:** Bulletproof React adaptado para Astro (pastas `src/components`, `src/layouts`, `src/pages`, `src/content`, `src/lib`, `src/types`, `src/config`). Dois repositórios: `Trivia-Obsidian/Clientes/PREVIX/Site/` (vault — specs) e `previx-site-app/` (código).
